...
// CreateDeleteProperties - The properties that control how Scaling will manage the size of the hostpool by creating and deleting
// hosts.
type CreateDeleteProperties struct {
	...
	// this limit is met. This requires the assigned hostpool to have a session host
	// config property.
...
// DiffDiskProperties - Specifies the ephemeral Disk Settings for the operating system disk used by the virtual machine.
type DiffDiskProperties struct {
	...
	// one is configured for the VM size otherwise ResourceDisk is used. Refer to the
	// VM size documentation for Windows VM at https://docs.microsoft.com/azure/virtual-machines/windows/sizes and Linux VM at
	// https://docs.microsoft.com/azure/virtual-machines/linux/sizes to check which VM
	// sizes exposes a cache disk. Possible values include: 'CacheDisk', 'ResourceDisk'
...
// HostPoolProvisioningControlParameter - Represents properties for a hostpool provisioning control request.
type HostPoolProvisioningControlParameter struct {
	// REQUIRED; Action types for controlling hostpool provisioning.
	Action *HostPoolProvisioningAction
...
// ManagedDiskProperties - Managed Disk information.
...
// RegistrationInfo - Represents a RegistrationInfo definition. This is not returned on HostPool GET. In order to get the
// registration token use the retrieveRegistrationToken or listRegistrationTokens POST calls.
...
// RegistrationInfoPatch - Represents a RegistrationInfo definition. This is not returned on HostPool GET. In order to get
// the registration token use the retrieveRegistrationToken or listRegistrationTokens POST calls.
...
// ScalingPlanPersonalSchedulePatchProperties - A ScalingPlanPersonalSchedule.
type ScalingPlanPersonalSchedulePatchProperties struct {
...
type ScalingPlanPersonalScheduleProperties struct {
	// REQUIRED; Set of days of the week on which this schedule is active.
	DaysOfWeek []*DayOfWeek

	// REQUIRED; Starting time for off-peak period.
	OffPeakStartTime *Time

	// REQUIRED; Starting time for peak period.
	PeakStartTime *Time

	// REQUIRED; Starting time for ramp down period.
	RampDownStartTime *Time

	// REQUIRED; Starting time for ramp up period.
	RampUpStartTime *Time
...
// ScalingPlanPooledSchedulePatchProperties - A ScalingPlanPooledSchedule.
type ScalingPlanPooledSchedulePatchProperties struct {
	...
	// READ-ONLY; Name of the ScalingPlanPooledSchedule.
	Name *string
}
...
type ScalingPlanPooledScheduleProperties struct {
	...
	// REQUIRED; Capacity threshold for ramp down period.
	RampDownCapacityThresholdPct *int32

	// REQUIRED; Starting time for ramp down period.
	RampDownStartTime *Time

	// REQUIRED; Capacity threshold for ramp up period.
	RampUpCapacityThresholdPct *int32
...
type SessionHostManagementProperties struct {
	...
	// Parameters that apply when session hosts are provisioned.
	Provisioning *SessionHostProvisioningConfigurationProperties
}

// SessionHostManagementProvisioningOperationProgress - SessionHostManagement Provisioning Operation Progress information.
type SessionHostManagementProvisioningOperationProgress struct {
	// Time that the sessionHostManagement operation was created.
	ExecutionStartTime *time.Time

	// The final instance count of the host pool for this provisioning operation.
	FinalSessionHostsCount *int32

	// The initial instance count of the host pool for this provisioning operation.
	InitialSessionHostsCount *int32
...
// SessionHostManagementProvisioningStatus - The current status of the session host provisioning async operation.
type SessionHostManagementProvisioningStatus struct {
	// REQUIRED; Operation status. Current defined values are < Scheduled | ValidatingSessionHostProvisioning | Cancelling | ProvisioningSessionHosts
	...
	Properties *SessionHostManagementProvisioningStatusProperties

	// The start time of the operation.
	StartTime *time.Time
}

// SessionHostManagementProvisioningStatusProperties - Properties bag to hold custom RP properties for sessionHostManagement
// Update Statuses.
type SessionHostManagementProvisioningStatusProperties struct {
	// The correlation Id for Log Analytics.
	CorrelationID *string

	// Progress information on the sessionHostManagement operation.
	Progress *SessionHostManagementProvisioningOperationProgress
...
// SessionHostProvisioningConfigurationPatchProperties - The configuration for session host provisioning operations.
type SessionHostProvisioningConfigurationPatchProperties struct {
...
// SessionHostProvisioningConfigurationProperties - The configuration for session host provisioning operations.
type SessionHostProvisioningConfigurationProperties struct {
	// Policy on whether a Canary VM should be provisioned during a session host provisioning operation.
	CanaryPolicy *CanaryPolicy
