...
// DataSetMappingsCreateFuture an abstraction for monitoring and retrieving the results of a long-running
// operation.
type DataSetMappingsCreateFuture struct {
	azure.FutureAPI
	// Result returns the result of the asynchronous operation.
	// If the operation has not completed it will return an error.
	Result func(DataSetMappingsClient) (DataSetMappingModel, error)
}

// UnmarshalJSON is the custom unmarshaller for CreateFuture.
func (future *DataSetMappingsCreateFuture) UnmarshalJSON(body []byte) error {
	var azFuture azure.Future
	if err := json.Unmarshal(body, &azFuture); err != nil {
		return err
	}
...
// result is the default implementation for DataSetMappingsCreateFuture.Result.
func (future *DataSetMappingsCreateFuture) result(client DataSetMappingsClient) (dsmm DataSetMappingModel, err error) {
	var done bool
	done, err = future.DoneWithContext(context.Background(), client)
	if err != nil {
		err = autorest.NewErrorWithError(err, "datashare.DataSetMappingsCreateFuture", "Result", future.Response(), "Polling failure")
		return
	}
	if !done {
		dsmm.Response.Response = future.Response()
		err = azure.NewAsyncOpIncompleteError("datashare.DataSetMappingsCreateFuture")
		return
	}
	sender := autorest.DecorateSender(client, autorest.DoRetryForStatusCodes(client.RetryAttempts, client.RetryDuration, autorest.StatusCodesForRetry...))
	if dsmm.Response.Response, err = future.GetResult(sender); err == nil && dsmm.Response.Response.StatusCode != http.StatusNoContent {
		dsmm, err = client.CreateResponder(dsmm.Response.Response)
		if err != nil {
			err = autorest.NewErrorWithError(err, "datashare.DataSetMappingsCreateFuture", "Result", dsmm.Response.Response, "Failure responding to request")
		}
	}
	return
}
...
type EmailRegistration struct {
	autorest.Response `json:"-"`
	// ActivationCode - Activation code for the registration
	ActivationCode *string `json:"activationCode,omitempty"`
	// ActivationExpirationDate - READ-ONLY; Date of the activation expiration
	ActivationExpirationDate *date.Time `json:"activationExpirationDate,omitempty"`
	// Email - READ-ONLY; The email to register
	Email *string `json:"email,omitempty"`
	// RegistrationStatus - READ-ONLY; Registration status. Possible values include: 'ActivationPending', 'Activated', 'ActivationAttemptsExhausted'
	RegistrationStatus RegistrationStatus `json:"registrationStatus,omitempty"`
	// TenantID - READ-ONLY; The tenant to register
	TenantID *string `json:"tenantId,omitempty"`
}

// MarshalJSON is the custom marshaler for EmailRegistration.
func (er EmailRegistration) MarshalJSON() ([]byte, error) {
	objectMap := make(map[string]interface{})
	if er.ActivationCode != nil {
		objectMap["activationCode"] = er.ActivationCode
	}
	return json.Marshal(objectMap)
}
