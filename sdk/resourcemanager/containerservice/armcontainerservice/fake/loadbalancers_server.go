...
type LoadBalancersServer struct {
	...
	// BeginRebalance is the fake for method LoadBalancersClient.BeginRebalance
	// HTTP status codes to indicate success: http.StatusAccepted
	BeginRebalance func(ctx context.Context, resourceGroupName string, resourceName string, parameters armcontainerservice.RebalanceLoadBalancersRequestBody, options *armcontainerservice.LoadBalancersClientBeginRebalanceOptions) (resp azfake.PollerResponder[armcontainerservice.LoadBalancersClientRebalanceResponse], errResp azfake.ErrorResponder)
}
...
func NewLoadBalancersServerTransport(srv *LoadBalancersServer) *LoadBalancersServerTransport {
	return &LoadBalancersServerTransport{
		...
		beginRebalance:               newTracker[azfake.PollerResponder[armcontainerservice.LoadBalancersClientRebalanceResponse]](),
	}
}
...
type LoadBalancersServerTransport struct {
	...
	beginRebalance               *tracker[azfake.PollerResponder[armcontainerservice.LoadBalancersClientRebalanceResponse]]
}
...
func (l *LoadBalancersServerTransport) Do(req *http.Request) (*http.Response, error) {
	...
	case "LoadBalancersClient.BeginRebalance":
		resp, err = l.dispatchBeginRebalance(req)
	default:
...
func (l *LoadBalancersServerTransport) dispatchBeginRebalance(req *http.Request) (*http.Response, error) {
	if l.srv.BeginRebalance == nil {
		return nil, &nonRetriableError{errors.New("fake for method BeginRebalance not implemented")}
	}
	beginRebalance := l.beginRebalance.get(req)
	if beginRebalance == nil {
		...
		respr, errRespr := l.srv.BeginRebalance(req.Context(), resourceGroupNameParam, resourceNameParam, body, nil)
		if respErr := server.GetError(errRespr, req); respErr != nil {
			return nil, respErr
		}
		beginRebalance = &respr
		l.beginRebalance.add(req, beginRebalance)
	}

	resp, err := server.PollerResponderNext(beginRebalance, req)
	if err != nil {
		return nil, err
	}

	if !contains([]int{http.StatusAccepted}, resp.StatusCode) {
		l.beginRebalance.remove(req)
		return nil, &nonRetriableError{fmt.Errorf("unexpected status code %d. acceptable values are http.StatusAccepted", resp.StatusCode)}
	}
	if !server.PollerResponderMore(beginRebalance) {
		l.beginRebalance.remove(req)
	}
