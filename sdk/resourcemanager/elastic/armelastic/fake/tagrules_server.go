...
// The returned TagRulesServerTransport instance is connected to an instance of armelastic.TagRulesClient via the
// azcore.ClientOptions.Transporter field in the client's constructor parameters.
func NewTagRulesServerTransport(srv *TagRulesServer) *TagRulesServerTransport {
	return &TagRulesServerTransport{
		srv:          srv,
		beginDelete:  newTracker[azfake.PollerResponder[armelastic.TagRulesClientDeleteResponse]](),
		newListPager: newTracker[azfake.PagerResponder[armelastic.TagRulesClientListResponse]](),
	}
}

// TagRulesServerTransport connects instances of armelastic.TagRulesClient to instances of TagRulesServer.
// Don't use this type directly, use NewTagRulesServerTransport instead.
type TagRulesServerTransport struct {
	srv          *TagRulesServer
	beginDelete  *tracker[azfake.PollerResponder[armelastic.TagRulesClientDeleteResponse]]
	newListPager *tracker[azfake.PagerResponder[armelastic.TagRulesClientListResponse]]
}
...
func (t *TagRulesServerTransport) dispatchCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.Elastic/monitors/(?P<monitorName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/tagRules/(?P<ruleSetName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)`
	regex := regexp.MustCompile(regexStr)
	matches := regex.FindStringSubmatch(req.URL.EscapedPath())
	if matches == nil || len(matches) < 4 {
		return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
	}
	body, err := server.UnmarshalRequestAsJSON[armelastic.MonitoringTagRules](req)
	if err != nil {
		return nil, err
	}
	...
	var options *armelastic.TagRulesClientCreateOrUpdateOptions
...
func (t *TagRulesServerTransport) dispatchNewListPager(req *http.Request) (*http.Response, error) {
	...
	if newListPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.Elastic/monitors/(?P<monitorName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/tagRules`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		...
		server.PagerResponderInjectNextLinks(newListPager, req, func(page *armelastic.TagRulesClientListResponse, createLink func() string) {
