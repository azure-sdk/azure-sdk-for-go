...
- New value `FormatTypeDelta`, `FormatTypeParquet` added to enum type `FormatType`
...
- New value `StatusTypeSystemSuspended` added to enum type `StatusType`
...
- New enum type `DestinationType` with values `DestinationTypeAzureBlob`, `DestinationTypeMicrosoftFabric`
...
- New struct `FabricLakehouse`
- New struct `FabricWorkspace`
- New struct `FilterItems`
- New struct `ForecastSpend`
- New struct `MCAPriceSheetProperties`
- New struct `MicrosoftFabricDestinationSettings`
...
- New field `MicrosoftFabricSettings`, `Type` in struct `ExportDeliveryDestination`
