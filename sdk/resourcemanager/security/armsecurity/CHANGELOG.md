...
- New value `AssessmentTypeBuiltInPolicy`, `AssessmentTypeManualBuiltIn`, `AssessmentTypeManualBuiltInPolicy`, `AssessmentTypeManualCustomPolicy` added to enum type `AssessmentType`
- New value `CategoriesAppServices`, `CategoriesContainer` added to enum type `Categories`
- New value `TechniquesCloudStorageObjectDiscovery`, `TechniquesExfiltrationOverC2Channel`, `TechniquesGatherVictimHostInformation`, `TechniquesNetworkServiceDiscovery`, `TechniquesStealApplicationAccessToken`, `TechniquesWeakenEncryption` added to enum type `Techniques`
- New enum type `AssessmentRiskLevel` with values `AssessmentRiskLevelCritical`, `AssessmentRiskLevelHigh`, `AssessmentRiskLevelLow`, `AssessmentRiskLevelMedium`
- New enum type `CloudProviders` with values `CloudProvidersAWS`, `CloudProvidersAzure`, `CloudProvidersAzureDevOps`, `CloudProvidersGCP`, `CloudProvidersGitHub`, `CloudProvidersGitLab`
- New enum type `ManagementProvider` with values `ManagementProviderAzurePolicy`, `ManagementProviderMDC`
- New struct `AssessmentRisk`
- New struct `AssessmentRiskPath`
- New struct `AssessmentRiskPathReferenceItem`
- New field `CloudProviders`, `ManagementProvider`, `RemediationPolicy` in struct `AssessmentMetadataPropertiesResponse`
- New field `Risk` in struct `AssessmentProperties`
- New field `Risk` in struct `AssessmentPropertiesBase`
