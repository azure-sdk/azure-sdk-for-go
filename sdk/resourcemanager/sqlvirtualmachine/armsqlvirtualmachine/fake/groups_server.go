...
type GroupsServer struct {
	...
	// NewListBySQLVMGroupPager is the fake for method GroupsClient.NewListBySQLVMGroupPager
	// HTTP status codes to indicate success: http.StatusOK
	NewListBySQLVMGroupPager func(resourceGroupName string, sqlVirtualMachineGroupName string, options *armsqlvirtualmachine.GroupsClientListBySQLVMGroupOptions) (resp azfake.PagerResponder[armsqlvirtualmachine.GroupsClientListBySQLVMGroupResponse])
...
func NewGroupsServerTransport(srv *GroupsServer) *GroupsServerTransport {
	return &GroupsServerTransport{
		...
		newListBySQLVMGroupPager:    newTracker[azfake.PagerResponder[armsqlvirtualmachine.GroupsClientListBySQLVMGroupResponse]](),
...
type GroupsServerTransport struct {
	...
	newListBySQLVMGroupPager    *tracker[azfake.PagerResponder[armsqlvirtualmachine.GroupsClientListBySQLVMGroupResponse]]
...
func (g *GroupsServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	...
	go func() {
		...
		if !intercepted {
			...
			case "GroupsClient.NewListBySQLVMGroupPager":
				res.resp, res.err = g.dispatchNewListBySQLVMGroupPager(req)
...
func (g *GroupsServerTransport) dispatchNewListBySQLVMGroupPager(req *http.Request) (*http.Response, error) {
	if g.srv.NewListBySQLVMGroupPager == nil {
		return nil, &nonRetriableError{errors.New("fake for method NewListBySQLVMGroupPager not implemented")}
	}
	newListBySQLVMGroupPager := g.newListBySQLVMGroupPager.get(req)
	if newListBySQLVMGroupPager == nil {
		...
		resp := g.srv.NewListBySQLVMGroupPager(resourceGroupNameParam, sqlVirtualMachineGroupNameParam, nil)
		newListBySQLVMGroupPager = &resp
		g.newListBySQLVMGroupPager.add(req, newListBySQLVMGroupPager)
		server.PagerResponderInjectNextLinks(newListBySQLVMGroupPager, req, func(page *armsqlvirtualmachine.GroupsClientListBySQLVMGroupResponse, createLink func() string) {
			page.NextLink = to.Ptr(createLink())
		})
	}
	...
	if !contains([]int{http.StatusOK}, resp.StatusCode) {
		g.newListBySQLVMGroupPager.remove(req)
		return nil, &nonRetriableError{fmt.Errorf("unexpected status code %d. acceptable values are http.StatusOK", resp.StatusCode)}
	}
