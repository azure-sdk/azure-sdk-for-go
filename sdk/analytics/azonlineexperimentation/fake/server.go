...
type Server struct {
	...
	// HTTP status codes to indicate success: http.StatusOK
	ValidateMetric func(ctx context.Context, experimentMetricID string, body azonlineexperimentation.ExperimentMetric, options *azonlineexperimentation.ValidateMetricOptions) (resp azfake.Responder[azonlineexperimentation.ValidateMetricResponse], errResp azfake.ErrorResponder)
}
...
func (s *ServerTransport) dispatchValidateMetric(req *http.Request) (*http.Response, error) {
	...
	respr, errRespr := s.srv.ValidateMetric(req.Context(), experimentMetricIDParam, body, options)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
	respContent := server.GetResponseContent(respr)
