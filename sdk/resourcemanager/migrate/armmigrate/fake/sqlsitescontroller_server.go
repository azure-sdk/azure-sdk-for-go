...
type SQLSitesControllerServer struct {
	...
	// BeginImportInventory is the fake for method SQLSitesControllerClient.BeginImportInventory
	// HTTP status codes to indicate success: http.StatusAccepted
	BeginImportInventory func(ctx context.Context, resourceGroupName string, siteName string, sqlSiteName string, body armmigrate.SQLInventoryImportBody, options *armmigrate.SQLSitesControllerClientBeginImportInventoryOptions) (resp azfake.PollerResponder[armmigrate.SQLSitesControllerClientImportInventoryResponse], errResp azfake.ErrorResponder)
...
func NewSQLSitesControllerServerTransport(srv *SQLSitesControllerServer) *SQLSitesControllerServerTransport {
	return &SQLSitesControllerServerTransport{
		...
		beginImportInventory:       newTracker[azfake.PollerResponder[armmigrate.SQLSitesControllerClientImportInventoryResponse]](),
...
type SQLSitesControllerServerTransport struct {
	...
	beginImportInventory       *tracker[azfake.PollerResponder[armmigrate.SQLSitesControllerClientImportInventoryResponse]]
...
func (s *SQLSitesControllerServerTransport) Do(req *http.Request) (*http.Response, error) {
	...
	case "SQLSitesControllerClient.ExportImportedSQLInventoryAsync":
		resp, err = s.dispatchExportImportedSQLInventoryAsync(req)
	...
	case "SQLSitesControllerClient.BeginImportInventory":
		resp, err = s.dispatchBeginImportInventory(req)
	...
	case "SQLSitesControllerClient.ValidateAndImportInventory":
		resp, err = s.dispatchValidateAndImportInventory(req)
	default:
...
func (s *SQLSitesControllerServerTransport) dispatchExportImportedSQLInventoryAsync(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.OffAzure/masterSites/(?P<siteName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/sqlSites/(?P<sqlSiteName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/exportImportedSqlInventory`
	regex := regexp.MustCompile(regexStr)
	matches := regex.FindStringSubmatch(req.URL.EscapedPath())
	if matches == nil || len(matches) < 4 {
		return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
func (s *SQLSitesControllerServerTransport) dispatchBeginImportInventory(req *http.Request) (*http.Response, error) {
	if s.srv.BeginImportInventory == nil {
		return nil, &nonRetriableError{errors.New("fake for method BeginImportInventory not implemented")}
	}
	beginImportInventory := s.beginImportInventory.get(req)
	if beginImportInventory == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.OffAzure/masterSites/(?P<siteName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/sqlSites/(?P<sqlSiteName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/importInventory`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if matches == nil || len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		...
		respr, errRespr := s.srv.BeginImportInventory(req.Context(), resourceGroupNameParam, siteNameParam, sqlSiteNameParam, body, nil)
		if respErr := server.GetError(errRespr, req); respErr != nil {
			return nil, respErr
		}
		beginImportInventory = &respr
		s.beginImportInventory.add(req, beginImportInventory)
	}

	resp, err := server.PollerResponderNext(beginImportInventory, req)
	if err != nil {
		return nil, err
	}

	if !contains([]int{http.StatusAccepted}, resp.StatusCode) {
		s.beginImportInventory.remove(req)
		return nil, &nonRetriableError{fmt.Errorf("unexpected status code %d. acceptable values are http.StatusAccepted", resp.StatusCode)}
	}
	if !server.PollerResponderMore(beginImportInventory) {
		s.beginImportInventory.remove(req)
	}
