...
// MarshalJSON implements the json.Marshaller interface for type ModelDiscoverySettingsUpdate.
func (m ModelDiscoverySettingsUpdate) MarshalJSON() ([]byte, error) {
	objectMap := make(map[string]any)
	populate(objectMap, "addRecommendedSignals", m.AddRecommendedSignals)
	populate(objectMap, "identity", m.Identity)
	populate(objectMap, "scope", m.Scope)
...
// UnmarshalJSON implements the json.Unmarshaller interface for type ModelDiscoverySettingsUpdate.
func (m *ModelDiscoverySettingsUpdate) UnmarshalJSON(data []byte) error {
	var rawMsg map[string]json.RawMessage
	if err := json.Unmarshal(data, &rawMsg); err != nil {
		return fmt.Errorf("unmarshalling type %T: %v", m, err)
	}
