...
// ValidateManifest - Validates the manifest has been applied in all regions.
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2024-09-01
//   - providerNamespace - The name of the resource provider hosted within ProviderHub.
//   - options - NewRegionFrontloadReleaseClientValidateManifestOptions contains the optional parameters for the NewRegionFrontloadReleaseClient.ValidateManifest
//     method.
func (client *NewRegionFrontloadReleaseClient) ValidateManifest(ctx context.Context, providerNamespace string, options *NewRegionFrontloadReleaseClientValidateManifestOptions) (NewRegionFrontloadReleaseClientValidateManifestResponse, error) {
	var err error
	const operationName = "NewRegionFrontloadReleaseClient.ValidateManifest"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.validateManifestCreateRequest(ctx, providerNamespace, options)
	if err != nil {
		return NewRegionFrontloadReleaseClientValidateManifestResponse{}, err
	}
	httpResp, err := client.internal.Pipeline().Do(req)
	if err != nil {
		return NewRegionFrontloadReleaseClientValidateManifestResponse{}, err
	...
	resp, err := client.validateManifestHandleResponse(httpResp)
	return resp, err
}

// validateManifestCreateRequest creates the ValidateManifest request.
func (client *NewRegionFrontloadReleaseClient) validateManifestCreateRequest(ctx context.Context, providerNamespace string, _ *NewRegionFrontloadReleaseClientValidateManifestOptions) (*policy.Request, error) {
	urlPath := "/providers/{providerNamespace}/validateManifestAppliedInAllRegions"
	if providerNamespace == "" {
		return nil, errors.New("parameter providerNamespace cannot be empty")
	}
	urlPath = strings.ReplaceAll(urlPath, "{providerNamespace}", url.PathEscape(providerNamespace))
...
// validateManifestHandleResponse handles the ValidateManifest response.
func (client *NewRegionFrontloadReleaseClient) validateManifestHandleResponse(resp *http.Response) (NewRegionFrontloadReleaseClientValidateManifestResponse, error) {
	result := NewRegionFrontloadReleaseClientValidateManifestResponse{}
