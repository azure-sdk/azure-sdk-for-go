...
func (client *SQLSitesControllerClient) exportImportedSQLInventoryAsyncCreateRequest(ctx context.Context, resourceGroupName string, siteName string, sqlSiteName string, body SQLInventoryImportBody, options *SQLSitesControllerClientExportImportedSQLInventoryAsyncOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.OffAzure/masterSites/{siteName}/sqlSites/{sqlSiteName}/exportImportedSqlInventory"
	if client.subscriptionID == "" {
		return nil, errors.New("parameter client.subscriptionID cannot be empty")
	}
	urlPath = strings.ReplaceAll(urlPath, "{subscriptionId}", url.PathEscape(client.subscriptionID))
...
// BeginImportInventory - Initiates SQL inventory import process. This is an asynchronous operation that returns a 202 Accepted
...
//   - options - SQLSitesControllerClientBeginImportInventoryOptions contains the optional parameters for the SQLSitesControllerClient.BeginImportInventory
//     method.
func (client *SQLSitesControllerClient) BeginImportInventory(ctx context.Context, resourceGroupName string, siteName string, sqlSiteName string, body SQLInventoryImportBody, options *SQLSitesControllerClientBeginImportInventoryOptions) (*runtime.Poller[SQLSitesControllerClientImportInventoryResponse], error) {
	if options == nil || options.ResumeToken == "" {
		resp, err := client.importInventory(ctx, resourceGroupName, siteName, sqlSiteName, body, options)
		if err != nil {
			return nil, err
		}
		poller, err := runtime.NewPoller(resp, client.internal.Pipeline(), &runtime.NewPollerOptions[SQLSitesControllerClientImportInventoryResponse]{
			FinalStateVia: runtime.FinalStateViaLocation,
			Tracer:        client.internal.Tracer(),
		})
		return poller, err
	} else {
		return runtime.NewPollerFromResumeToken(options.ResumeToken, client.internal.Pipeline(), &runtime.NewPollerFromResumeTokenOptions[SQLSitesControllerClientImportInventoryResponse]{
			Tracer: client.internal.Tracer(),
		})
	}
}

// ImportInventory - Initiates SQL inventory import process. This is an asynchronous operation that returns a 202 Accepted
// response and requires polling for completion status.
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2024-12-01-preview
func (client *SQLSitesControllerClient) importInventory(ctx context.Context, resourceGroupName string, siteName string, sqlSiteName string, body SQLInventoryImportBody, options *SQLSitesControllerClientBeginImportInventoryOptions) (*http.Response, error) {
	var err error
	const operationName = "SQLSitesControllerClient.BeginImportInventory"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.importInventoryCreateRequest(ctx, resourceGroupName, siteName, sqlSiteName, body, options)
	if err != nil {
		return nil, err
	}
	httpResp, err := client.internal.Pipeline().Do(req)
...
// importInventoryCreateRequest creates the ImportInventory request.
func (client *SQLSitesControllerClient) importInventoryCreateRequest(ctx context.Context, resourceGroupName string, siteName string, sqlSiteName string, body SQLInventoryImportBody, options *SQLSitesControllerClientBeginImportInventoryOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.OffAzure/masterSites/{siteName}/sqlSites/{sqlSiteName}/importInventory"
	if client.subscriptionID == "" {
		return nil, errors.New("parameter client.subscriptionID cannot be empty")
	}
	urlPath = strings.ReplaceAll(urlPath, "{subscriptionId}", url.PathEscape(client.subscriptionID))
...
func (client *SQLSitesControllerClient) validateAndImportInventoryCreateRequest(ctx context.Context, resourceGroupName string, siteName string, sqlSiteName string, body SQLInventoryImportBody, options *SQLSitesControllerClientValidateAndImportInventoryOptions) (*policy.Request, error) {
