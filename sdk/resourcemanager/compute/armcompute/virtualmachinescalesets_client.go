...
// BeginInvokeUpgrade - Invoke upgrade on deferred rolling upgrade for OS disks in the virtual machines in a VM scale set.
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2023-07-01
//   - resourceGroupName - The name of the resource group.
//   - vmScaleSetName - The name of the VM scale set.
//   - options - VirtualMachineScaleSetsClientBeginInvokeUpgradeOptions contains the optional parameters for the VirtualMachineScaleSetsClient.BeginInvokeUpgrade
//     method.
func (client *VirtualMachineScaleSetsClient) BeginInvokeUpgrade(ctx context.Context, resourceGroupName string, vmScaleSetName string, options *VirtualMachineScaleSetsClientBeginInvokeUpgradeOptions) (*runtime.Poller[VirtualMachineScaleSetsClientInvokeUpgradeResponse], error) {
	if options == nil || options.ResumeToken == "" {
		resp, err := client.invokeUpgrade(ctx, resourceGroupName, vmScaleSetName, options)
		if err != nil {
			return nil, err
		}
		return runtime.NewPoller[VirtualMachineScaleSetsClientInvokeUpgradeResponse](resp, client.internal.Pipeline(), nil)
	} else {
		return runtime.NewPollerFromResumeToken[VirtualMachineScaleSetsClientInvokeUpgradeResponse](options.ResumeToken, client.internal.Pipeline(), nil)
	}
}

// InvokeUpgrade - Invoke upgrade on deferred rolling upgrade for OS disks in the virtual machines in a VM scale set.
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2023-07-01
func (client *VirtualMachineScaleSetsClient) invokeUpgrade(ctx context.Context, resourceGroupName string, vmScaleSetName string, options *VirtualMachineScaleSetsClientBeginInvokeUpgradeOptions) (*http.Response, error) {
	req, err := client.invokeUpgradeCreateRequest(ctx, resourceGroupName, vmScaleSetName, options)
	if err != nil {
		return nil, err
	}
	resp, err := client.internal.Pipeline().Do(req)
...
func (client *VirtualMachineScaleSetsClient) invokeUpgradeCreateRequest(ctx context.Context, resourceGroupName string, vmScaleSetName string, options *VirtualMachineScaleSetsClientBeginInvokeUpgradeOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Compute/virtualMachineScaleSets/{vmScaleSetName}/invokeupgrade"
	if resourceGroupName == "" {
		return nil, errors.New("parameter resourceGroupName cannot be empty")
	}
	urlPath = strings.ReplaceAll(urlPath, "{resourceGroupName}", url.PathEscape(resourceGroupName))
