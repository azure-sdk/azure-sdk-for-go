...
## 1.3.0-beta.4 (2025-04-25)
### Breaking Changes

- Type of `ConnectedRegistryProperties.Activation` has been changed from `*ActivationProperties` to `*ConnectedRegistryPropertiesActivation`
- Type of `ConnectedRegistryUpdateParameters.Properties` has been changed from `*ConnectedRegistryUpdateProperties` to `*ConnectedRegistryUpdateParametersProperties`
- Type of `LoginServerProperties.TLS` has been changed from `*TLSProperties` to `*LoginServerPropertiesTLS`
...
- Enum `RoleAssignmentMode` has been removed
...
- Field `AutoGeneratedDomainNameLabelScope`, `MetadataSearch`, `RoleAssignmentMode` of struct `RegistryProperties` has been removed
- Field `MetadataSearch`, `RoleAssignmentMode` of struct `RegistryPropertiesUpdateParameters` has been removed
- Field `Identity` of struct `SourceRegistryCredentials` has been removed

### Features Added

- New struct `ConnectedRegistryPropertiesActivation`
- New struct `ConnectedRegistryUpdateParametersProperties`
- New struct `LoginServerPropertiesTLS`
- New struct `TLSPropertiesCertificate`
