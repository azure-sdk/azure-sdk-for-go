...
// MarshalJSON implements the json.Marshaller interface for type SourceScanIdentity.
func (s SourceScanIdentity) MarshalJSON() ([]byte, error) {
	objectMap := make(map[string]any)
	populate(objectMap, "useSystemAssignedIdentity", s.UseSystemAssignedIdentity)
	populate(objectMap, "userAssignedIdentity", s.UserAssignedIdentity)
	return json.Marshal(objectMap)
}

// UnmarshalJSON implements the json.Unmarshaller interface for type SourceScanIdentity.
func (s *SourceScanIdentity) UnmarshalJSON(data []byte) error {
	var rawMsg map[string]json.RawMessage
	if err := json.Unmarshal(data, &rawMsg); err != nil {
		return fmt.Errorf("unmarshalling type %T: %v", s, err)
	}
	for key, val := range rawMsg {
		...
		switch key {
		case "useSystemAssignedIdentity":
			err = unpopulate(val, "UseSystemAssignedIdentity", &s.UseSystemAssignedIdentity)
			delete(rawMsg, key)
		case "userAssignedIdentity":
