...
// ValidateMedtechMappings - Validates Medtech mapping files against sample device data.
// If the operation fails it returns an *azcore.ResponseError type.
// Generated from API version 2022-07-01-preview
// validationRequestInputs - The mapping files and device events which will be validated
// options - ServicesClientValidateMedtechMappingsOptions contains the optional parameters for the ServicesClient.ValidateMedtechMappings
// method.
func (client *ServicesClient) ValidateMedtechMappings(ctx context.Context, validationRequestInputs ValidateMedtechMappingsParameters, options *ServicesClientValidateMedtechMappingsOptions) (ServicesClientValidateMedtechMappingsResponse, error) {
	req, err := client.validateMedtechMappingsCreateRequest(ctx, validationRequestInputs, options)
	if err != nil {
		return ServicesClientValidateMedtechMappingsResponse{}, err
	}
	resp, err := client.pl.Do(req)
	if err != nil {
		return ServicesClientValidateMedtechMappingsResponse{}, err
	...
	if !runtime.HasStatusCode(resp, http.StatusOK) {
		return ServicesClientValidateMedtechMappingsResponse{}, runtime.NewResponseError(resp)
	}
	return client.validateMedtechMappingsHandleResponse(resp)
}

// validateMedtechMappingsCreateRequest creates the ValidateMedtechMappings request.
func (client *ServicesClient) validateMedtechMappingsCreateRequest(ctx context.Context, validationRequestInputs ValidateMedtechMappingsParameters, options *ServicesClientValidateMedtechMappingsOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/providers/Microsoft.HealthcareApis/validateMedtechMappings"
	if client.subscriptionID == "" {
		return nil, errors.New("parameter client.subscriptionID cannot be empty")
	}
	urlPath = strings.ReplaceAll(urlPath, "{subscriptionId}", url.PathEscape(client.subscriptionID))
	...
	req.Raw().URL.RawQuery = reqQP.Encode()
	req.Raw().Header["Accept"] = []string{"application/json"}
	return req, runtime.MarshalAsJSON(req, validationRequestInputs)
}

// validateMedtechMappingsHandleResponse handles the ValidateMedtechMappings response.
func (client *ServicesClient) validateMedtechMappingsHandleResponse(resp *http.Response) (ServicesClientValidateMedtechMappingsResponse, error) {
	result := ServicesClientValidateMedtechMappingsResponse{}
	if err := runtime.UnmarshalAsJSON(resp, &result.ValidateMedtechMappingsResult); err != nil {
		return ServicesClientValidateMedtechMappingsResponse{}, err
	}
	return result, nil
}
