...
func ExampleWorkloadImpactsClient_BeginCreate_reportingArmOperationFailure() {
	...
	clientFactory, err := armimpact.NewClientFactory("00000000-0000-0000-0000-000000000000", cred, nil)
	if err != nil {
		log.Fatalf("failed to create client: %v", err)
	}
	poller, err := clientFactory.NewWorkloadImpactsClient().BeginCreate(ctx, "impact-002", armimpact.WorkloadImpact{
		Properties: &armimpact.WorkloadImpactProperties{
			...
			Workload: &armimpact.Workload{
				Context: to.Ptr("webapp/scenario1"),
				Toolset: to.Ptr(armimpact.ToolsetOther),
			},
			ClientIncidentDetails: &armimpact.ClientIncidentDetails{
				ClientIncidentID:     to.Ptr("AA123"),
				ClientIncidentSource: to.Ptr(armimpact.IncidentSourceJira),
			},
		},
	}, nil)
	if err != nil {
	...
	// res = armimpact.WorkloadImpactsClientCreateResponse{
	// 	WorkloadImpact: &armimpact.WorkloadImpact{
...
func ExampleWorkloadImpactsClient_BeginCreate_reportingAConnectivityImpact() {
	...
	poller, err := clientFactory.NewWorkloadImpactsClient().BeginCreate(ctx, "impact-001", armimpact.WorkloadImpact{
		Properties: &armimpact.WorkloadImpactProperties{
			ImpactedResourceID: to.Ptr("/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/resource-rg/providers/Microsoft.Sql/sqlserver/dbservercontext"),
			StartDateTime:      to.Ptr(func() time.Time { t, _ := time.Parse(time.RFC3339Nano, "2022-06-15T05:59:46.6517821Z"); return t }()),
			ImpactDescription:  to.Ptr("conection failure"),
			ImpactCategory:     to.Ptr("Resource.Connectivity"),
			Connectivity: &armimpact.Connectivity{
				Protocol: to.Ptr(armimpact.ProtocolTCP),
				Port:     to.Ptr[int32](1443),
				Source: &armimpact.SourceOrTarget{
				...
				Target: &armimpact.SourceOrTarget{
...
func ExampleWorkloadImpactsClient_BeginCreate_reportingPerformanceRelatedImpact() {
	...
	poller, err := clientFactory.NewWorkloadImpactsClient().BeginCreate(ctx, "impact-002", armimpact.WorkloadImpact{
		Properties: &armimpact.WorkloadImpactProperties{
			...
			Performance: []*armimpact.Performance{
				{
...
func ExampleWorkloadImpactsClient_Delete() {
	...
	_ = res
	// If the HTTP response code is 200 as defined in example definition, your response structure would look as follows. Please pay attention that all the values in the output are fake values for just demo purposes.
	// res = armimpact.WorkloadImpactsClientDeleteResponse{
	// }
}
...
func ExampleWorkloadImpactsClient_Get() {
	...
	_ = res
	// If the HTTP response code is 200 as defined in example definition, your response structure would look as follows. Please pay attention that all the values in the output are fake values for just demo purposes.
	// res = armimpact.WorkloadImpactsClientGetResponse{
	// 	WorkloadImpact: &armimpact.WorkloadImpact{
	// 		ID: to.Ptr("/subscriptions/00000000-0000-0000-0000-000000000000/providers/Microsoft.Impact/workloadImpacts/impact-001"),
	// 		Name: to.Ptr("impact-001"),
	// 		Type: to.Ptr("Microsoft.Impact/workloadImpacts"),
...
func ExampleWorkloadImpactsClient_NewListBySubscriptionPager() {
	...
	for pager.More() {
		...
		// page = armimpact.WorkloadImpactsClientListBySubscriptionResponse{
		// 	WorkloadImpactListResult: armimpact.WorkloadImpactListResult{
		// 		Value: []*armimpact.WorkloadImpact{
		// 			{
		...
		// 					AdditionalProperties: &armimpact.WorkloadImpactPropertiesAdditionalProperties{
		// 					},
		// 					Workload: &armimpact.Workload{
		// 						Context: to.Ptr("webapp/scenario1"),
		// 						Toolset: to.Ptr(armimpact.ToolsetOther),
