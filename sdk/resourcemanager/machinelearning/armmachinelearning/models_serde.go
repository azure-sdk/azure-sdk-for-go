...
func (f FeatureStoreSettings) MarshalJSON() ([]byte, error) {
	objectMap := make(map[string]any)
	populate(objectMap, "allowRoleAssignmentsOnResourceGroupLevel", f.AllowRoleAssignmentsOnResourceGroupLevel)
...
func (f *FeatureStoreSettings) UnmarshalJSON(data []byte) error {
	...
	for key, val := range rawMsg {
		var err error
		switch key {
		case "allowRoleAssignmentsOnResourceGroupLevel":
			err = unpopulate(val, "AllowRoleAssignmentsOnResourceGroupLevel", &f.AllowRoleAssignmentsOnResourceGroupLevel)
			delete(rawMsg, key)
		case "computeRuntime":
