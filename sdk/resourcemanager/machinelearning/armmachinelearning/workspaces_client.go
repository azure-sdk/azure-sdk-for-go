...
// ListConnectionModels - List available models from all connections.
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2024-07-01-preview
//   - resourceGroupName - The name of the resource group. The name is case insensitive.
//   - workspaceName - Azure Machine Learning Workspace Name
//   - options - WorkspacesClientListConnectionModelsOptions contains the optional parameters for the WorkspacesClient.ListConnectionModels
//     method.
func (client *WorkspacesClient) ListConnectionModels(ctx context.Context, resourceGroupName string, workspaceName string, options *WorkspacesClientListConnectionModelsOptions) (WorkspacesClientListConnectionModelsResponse, error) {
	var err error
	const operationName = "WorkspacesClient.ListConnectionModels"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.listConnectionModelsCreateRequest(ctx, resourceGroupName, workspaceName, options)
	if err != nil {
		return WorkspacesClientListConnectionModelsResponse{}, err
	}
	httpResp, err := client.internal.Pipeline().Do(req)
	if err != nil {
		return WorkspacesClientListConnectionModelsResponse{}, err
	...
	resp, err := client.listConnectionModelsHandleResponse(httpResp)
	return resp, err
}

// listConnectionModelsCreateRequest creates the ListConnectionModels request.
func (client *WorkspacesClient) listConnectionModelsCreateRequest(ctx context.Context, resourceGroupName string, workspaceName string, options *WorkspacesClientListConnectionModelsOptions) (*policy.Request, error) {
...
// listConnectionModelsHandleResponse handles the ListConnectionModels response.
func (client *WorkspacesClient) listConnectionModelsHandleResponse(resp *http.Response) (WorkspacesClientListConnectionModelsResponse, error) {
	result := WorkspacesClientListConnectionModelsResponse{}
