...
type RegistryModelVersionsServer struct {
	...
	// BeginPackage is the fake for method RegistryModelVersionsClient.BeginPackage
...
func NewRegistryModelVersionsServerTransport(srv *RegistryModelVersionsServer) *RegistryModelVersionsServerTransport {
	return &RegistryModelVersionsServerTransport{
		...
		beginPackage:        newTracker[azfake.PollerResponder[armmachinelearning.RegistryModelVersionsClientPackageResponse]](),
	}
}
...
type RegistryModelVersionsServerTransport struct {
	...
	beginPackage        *tracker[azfake.PollerResponder[armmachinelearning.RegistryModelVersionsClientPackageResponse]]
}
...
func (r *RegistryModelVersionsServerTransport) Do(req *http.Request) (*http.Response, error) {
	...
	case "RegistryModelVersionsClient.BeginPackage":
		resp, err = r.dispatchBeginPackage(req)
	default:
...
func (r *RegistryModelVersionsServerTransport) dispatchBeginPackage(req *http.Request) (*http.Response, error) {
	if r.srv.BeginPackage == nil {
		return nil, &nonRetriableError{errors.New("fake for method BeginPackage not implemented")}
	}
	beginPackage := r.beginPackage.get(req)
	if beginPackage == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.MachineLearningServices/registries/(?P<registryName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/models/(?P<modelName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/versions/(?P<version>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/package`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if matches == nil || len(matches) < 5 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		...
		respr, errRespr := r.srv.BeginPackage(req.Context(), resourceGroupNameParam, registryNameParam, modelNameParam, versionParam, body, nil)
		if respErr := server.GetError(errRespr, req); respErr != nil {
			return nil, respErr
		}
		beginPackage = &respr
		r.beginPackage.add(req, beginPackage)
	}
