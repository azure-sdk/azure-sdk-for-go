...
//   - resourceGroupName - The name of the resource group.
//   - networkVirtualApplianceName - The name of Network Virtual Appliance.
//   - options - VirtualAppliancesClientRestartOptions contains the optional parameters for the VirtualAppliancesClient.Restart
//     method.
func (client *VirtualAppliancesClient) Restart(ctx context.Context, resourceGroupName string, networkVirtualApplianceName string, options *VirtualAppliancesClientRestartOptions) (VirtualAppliancesClientRestartResponse, error) {
	var err error
	const operationName = "VirtualAppliancesClient.Restart"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.restartCreateRequest(ctx, resourceGroupName, networkVirtualApplianceName, options)
	if err != nil {
		return VirtualAppliancesClientRestartResponse{}, err
	}
	httpResp, err := client.internal.Pipeline().Do(req)
	if err != nil {
		return VirtualAppliancesClientRestartResponse{}, err
	...
	return VirtualAppliancesClientRestartResponse{}, nil
}

// restartCreateRequest creates the Restart request.
func (client *VirtualAppliancesClient) restartCreateRequest(ctx context.Context, resourceGroupName string, networkVirtualApplianceName string, options *VirtualAppliancesClientRestartOptions) (*policy.Request, error) {
