...
func (client *RecommendationMetadataClient) listCreateRequest(ctx context.Context, options *RecommendationMetadataClientListOptions) (*policy.Request, error) {
	...
	if options != nil && options.Category != nil {
		reqQP.Set("[category](#category)", *options.Category)
	}
	if options != nil && options.Control != nil {
		reqQP.Set("[control](#control)", *options.Control)
	}
	reqQP.Set("api-version", "2025-01-01")
	if options != nil && options.RetirementsFrom != nil {
		reqQP.Set("retirementsFrom", *options.RetirementsFrom)
	}
	if options != nil && options.RetirementsTill != nil {
		reqQP.Set("retirementsTill", *options.RetirementsTill)
	}
	req.Raw().URL.RawQuery = reqQP.Encode()
	req.Raw().Header["Accept"] = []string{"application/json"}
	return req, nil
