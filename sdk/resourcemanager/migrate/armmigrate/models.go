...
// ArcScopeProperties - Properties for defining Arc scope.
type ArcScopeProperties struct {
	// REQUIRED; The type of scope, currently supporting SubscriptionOrResourceGroupIds only.
...
// ArcScopePropertiesUpdate - Properties for defining Arc scope.
type ArcScopePropertiesUpdate struct {
	// Locations to discover Arc resources from.
	Locations []*string
...
// ImportSQLInventoryJob - Import SQL inventory job REST Resource.
type ImportSQLInventoryJob struct {
	// Gets or sets the import job properties.
	Properties *ImportSQLInventoryJobProperties

	// READ-ONLY; Handled by resource provider.
	Type *string
}

// ImportSQLInventoryJobProperties - Import SQL inventory job properties web model
type ImportSQLInventoryJobProperties struct {
	...
	NumberOfAvailabilityGroupsImported *int32

	// Gets the Number of databases imported.
	NumberOfDatabasesImported *int32

	// Gets the Number of servers imported.
	NumberOfServersImported *int32
}
...
type MasterSiteProperties struct {
	...
	// READ-ONLY; Gets the last refresh time of arc status in UTC.
	LastRefreshArcStatusTime *time.Time
...
// PagedImportSQLInventoryJob - Paged collection of ImportSqlInventoryJob items
type PagedImportSQLInventoryJob struct {
	// READ-ONLY; The link to the next page of items
	NextLink *string

	// READ-ONLY; The ImportSqlInventoryJob items on this page
	Value []*ImportSQLInventoryJob
}
...
// SQLImportJobErrorSummary - Job Error Summary
type SQLImportJobErrorSummary struct {
	...
	// Gets or sets the Number of warnings while trying to import SQL Inventory from CSV. Warnings are included in the 'errors'
	// array.
	WarningCount *int32
}

// SQLInventoryImportBody - SQL Inventory Import URI response object model.
type SQLInventoryImportBody struct {
...
type ServerProperties struct {
	// READ-ONLY; Gets or sets the discovery source of the server.
	// Field has constant value "Arc", any specified value is ignored.
	...
	// READ-ONLY; Gets or sets the hypervisor type of the server.
	Hypervisor *string
...
type ServerSiteResource struct {
	...
	// The resource-specific properties for this resource.
	Properties *ServerSitesProperties
...
// ServerSitesProperties - The properties of ServerSiteResource.
type ServerSitesProperties struct {
	// Gets or sets the on-premises agent details.
	AgentDetails *SiteAgentProperties
