...
// BeginDelete - Delete a BgpPeer
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2023-10-01-preview
//   - resourceURI - The fully qualified Azure Resource manager identifier of the resource.
//   - bgpPeerName - The name of the BgpPeer
//   - options - BgpPeersClientBeginDeleteOptions contains the optional parameters for the BgpPeersClient.BeginDelete method.
func (client *BgpPeersClient) BeginDelete(ctx context.Context, resourceURI string, bgpPeerName string, options *BgpPeersClientBeginDeleteOptions) (*runtime.Poller[BgpPeersClientDeleteResponse], error) {
	if options == nil || options.ResumeToken == "" {
		resp, err := client.deleteOperation(ctx, resourceURI, bgpPeerName, options)
		if err != nil {
			return nil, err
		}
		poller, err := runtime.NewPoller(resp, client.internal.Pipeline(), &runtime.NewPollerOptions[BgpPeersClientDeleteResponse]{
			FinalStateVia: runtime.FinalStateViaLocation,
			Tracer:        client.internal.Tracer(),
		})
		return poller, err
	} else {
		return runtime.NewPollerFromResumeToken(options.ResumeToken, client.internal.Pipeline(), &runtime.NewPollerFromResumeTokenOptions[BgpPeersClientDeleteResponse]{
			Tracer: client.internal.Tracer(),
		})
	}
}
...
//
// Generated from API version 2023-10-01-preview
func (client *BgpPeersClient) deleteOperation(ctx context.Context, resourceURI string, bgpPeerName string, options *BgpPeersClientBeginDeleteOptions) (*http.Response, error) {
	var err error
	const operationName = "BgpPeersClient.BeginDelete"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
...
func (client *BgpPeersClient) deleteCreateRequest(ctx context.Context, resourceURI string, bgpPeerName string, options *BgpPeersClientBeginDeleteOptions) (*policy.Request, error) {
	urlPath := "/{resourceUri}/providers/Microsoft.KubernetesRuntime/bgpPeers/{bgpPeerName}"
	urlPath = strings.ReplaceAll(urlPath, "{resourceUri}", resourceURI)
	if bgpPeerName == "" {
		return nil, errors.New("parameter bgpPeerName cannot be empty")
