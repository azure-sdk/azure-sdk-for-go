...
- New enum type `LicenseStatus` with values `LicenseStatusExtendedGrace`, `LicenseStatusLicensed`, `LicenseStatusNonGenuineGrace`, `LicenseStatusNotification`, `LicenseStatusOOBGrace`, `LicenseStatusOOTGrace`, `LicenseStatusUnknown`, `LicenseStatusUnlicensed`
...
- New enum type `MachineType` with values `MachineTypeNative`, `MachineTypeUnknown`, `MachineTypeVM`
...
- New struct `IotEnterpriseProfileUpdateProperties`
- New struct `License`
...
- New struct `LicenseProfileArmIoTEnterpriseProperties`
- New struct `LicenseProfileArmWindowsServerProperties`
- New struct `LicenseProfileMachineInstanceView`
- New struct `LicenseProfileMachineInstanceViewEsuProperties`
- New struct `LicenseProfileMachineInstanceViewProducts`
- New struct `LicenseProfileMachineInstanceViewSoftwareAssurance`
- New struct `LicenseProfileProperties`
- New struct `LicenseProfilePropertiesProducts`
...
- New struct `LicenseProfileUpdatePropertiesProducts`
...
- New struct `WindowsServerProfileUpdateProperties`
...
- New field `AgentUpgrade`, `LicenseProfile`, `MachineType`, `NetworkProfile`, `OSEdition` in struct `MachineProperties`
