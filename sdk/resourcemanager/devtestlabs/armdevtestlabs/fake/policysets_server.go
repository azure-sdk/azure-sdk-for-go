...
func (p *PolicySetsServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	...
	go func() {
		...
		var res result
		if policySetsServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = policySetsServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
			case "PolicySetsClient.EvaluatePolicies":
				res.resp, res.err = p.dispatchEvaluatePolicies(req)
			default:
...
// set this to conditionally intercept incoming requests to PolicySetsServerTransport
var policySetsServerTransportInterceptor interface {
