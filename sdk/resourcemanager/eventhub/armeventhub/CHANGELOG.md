...
- New enum type `PreferredRing` with values `PreferredRingEarly`, `PreferredRingLate`, `PreferredRingMiddle`, `PreferredRingNone`
- New enum type `Preferredstartdayofweek` with values `PreferredstartdayofweekFriday`, `PreferredstartdayofweekMonday`, `PreferredstartdayofweekSaturday`, `PreferredstartdayofweekSunday`, `PreferredstartdayofweekThursday`, `PreferredstartdayofweekTuesday`, `PreferredstartdayofweekWednesday`
...
- New enum type `Status` with values `StatusFailed`, `StatusRunning`, `StatusScheduled`, `StatusSucceeded`, `StatusTerminated`
- New function `*ClientFactory.NewUpgradeWindowClient() *UpgradeWindowClient`
- New function `*ClientFactory.NewUpgradeWindowHistoryClient() *UpgradeWindowHistoryClient`
- New function `*ClientFactory.NewUpgradeWindowTriggerClient() *UpgradeWindowTriggerClient`
- New function `NewUpgradeWindowClient(string, azcore.TokenCredential, *arm.ClientOptions) (*UpgradeWindowClient, error)`
- New function `*UpgradeWindowClient.Get(context.Context, string, string, *UpgradeWindowClientGetOptions) (UpgradeWindowClientGetResponse, error)`
- New function `*UpgradeWindowClient.Patch(context.Context, string, string, ClusterUpgradeWindow, *UpgradeWindowClientPatchOptions) (UpgradeWindowClientPatchResponse, error)`
- New function `NewUpgradeWindowHistoryClient(string, azcore.TokenCredential, *arm.ClientOptions) (*UpgradeWindowHistoryClient, error)`
- New function `*UpgradeWindowHistoryClient.NewGetPager(string, string, *UpgradeWindowHistoryClientGetOptions) *runtime.Pager[UpgradeWindowHistoryClientGetResponse]`
- New function `NewUpgradeWindowTriggerClient(string, azcore.TokenCredential, *arm.ClientOptions) (*UpgradeWindowTriggerClient, error)`
- New function `*UpgradeWindowTriggerClient.Post(context.Context, string, string, *UpgradeWindowTriggerClientPostOptions) (UpgradeWindowTriggerClientPostResponse, error)`
...
- New struct `ClusterUpgradeHistoryList`
- New struct `ClusterUpgradeWindow`
- New struct `ClusterUpgradeWindowClusterupgradewindowproperties`
...
- New field `ClusterUpgradeList`, `ProvisioningState` in struct `ClusterProperties`
