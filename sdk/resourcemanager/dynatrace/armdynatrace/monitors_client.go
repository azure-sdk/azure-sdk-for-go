...
// UpdateAgentStatus - Create/Update the list of all resources that have Dynatrace agent installed through the Azure Dynatrace
// resource.
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2024-04-24
...
//   - options - MonitorsClientUpdateAgentStatusOptions contains the optional parameters for the MonitorsClient.UpdateAgentStatus
//     method.
func (client *MonitorsClient) UpdateAgentStatus(ctx context.Context, resourceGroupName string, monitorName string, request AgentStatusRequest, options *MonitorsClientUpdateAgentStatusOptions) (MonitorsClientUpdateAgentStatusResponse, error) {
	var err error
	const operationName = "MonitorsClient.UpdateAgentStatus"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.updateAgentStatusCreateRequest(ctx, resourceGroupName, monitorName, request, options)
	if err != nil {
		return MonitorsClientUpdateAgentStatusResponse{}, err
	}
	httpResp, err := client.internal.Pipeline().Do(req)
	if err != nil {
		return MonitorsClientUpdateAgentStatusResponse{}, err
	...
	return MonitorsClientUpdateAgentStatusResponse{}, nil
}

// updateAgentStatusCreateRequest creates the UpdateAgentStatus request.
func (client *MonitorsClient) updateAgentStatusCreateRequest(ctx context.Context, resourceGroupName string, monitorName string, request AgentStatusRequest, _ *MonitorsClientUpdateAgentStatusOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Dynatrace.Observability/monitors/{monitorName}/updateAgentStatus"
	if client.subscriptionID == "" {
		return nil, errors.New("parameter client.subscriptionID cannot be empty")
	}
	urlPath = strings.ReplaceAll(urlPath, "{subscriptionId}", url.PathEscape(client.subscriptionID))
