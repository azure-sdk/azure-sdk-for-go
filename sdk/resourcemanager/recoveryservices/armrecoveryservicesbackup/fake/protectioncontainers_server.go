...
type ProtectionContainersServer struct {
	...
	// BeginUnregister is the fake for method ProtectionContainersClient.BeginUnregister
	// HTTP status codes to indicate success: http.StatusOK, http.StatusAccepted, http.StatusNoContent
	BeginUnregister func(ctx context.Context, vaultName string, resourceGroupName string, fabricName string, containerName string, options *armrecoveryservicesbackup.ProtectionContainersClientBeginUnregisterOptions) (resp azfake.PollerResponder[armrecoveryservicesbackup.ProtectionContainersClientUnregisterResponse], errResp azfake.ErrorResponder)
}
...
func NewProtectionContainersServerTransport(srv *ProtectionContainersServer) *ProtectionContainersServerTransport {
	return &ProtectionContainersServerTransport{
		...
		beginUnregister: newTracker[azfake.PollerResponder[armrecoveryservicesbackup.ProtectionContainersClientUnregisterResponse]](),
	}
}
...
type ProtectionContainersServerTransport struct {
	...
	beginUnregister *tracker[azfake.PollerResponder[armrecoveryservicesbackup.ProtectionContainersClientUnregisterResponse]]
}
...
func (p *ProtectionContainersServerTransport) Do(req *http.Request) (*http.Response, error) {
	...
	case "ProtectionContainersClient.BeginUnregister":
		resp, err = p.dispatchBeginUnregister(req)
	default:
...
func (p *ProtectionContainersServerTransport) dispatchBeginUnregister(req *http.Request) (*http.Response, error) {
	if p.srv.BeginUnregister == nil {
		return nil, &nonRetriableError{errors.New("fake for method BeginUnregister not implemented")}
	}
	beginUnregister := p.beginUnregister.get(req)
	if beginUnregister == nil {
		...
		respr, errRespr := p.srv.BeginUnregister(req.Context(), vaultNameParam, resourceGroupNameParam, fabricNameParam, containerNameParam, nil)
		if respErr := server.GetError(errRespr, req); respErr != nil {
			return nil, respErr
		}
		beginUnregister = &respr
		p.beginUnregister.add(req, beginUnregister)
	}
