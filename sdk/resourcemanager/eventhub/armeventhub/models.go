...
type ClusterProperties struct {
	...
	// READ-ONLY; List of cluster upgrades performed onto this cluster
	ClusterUpgradeList []*ClusterUpgrade
...
// ClusterUpgrade - Details for a cluster upgrade
type ClusterUpgrade struct {
	// READ-ONLY; Name of the cluster upgrade
	Name *string

	// READ-ONLY; Status of the cluster upgrade
	Status *Status
}

// ClusterUpgradeHistory - Contains past history of all cluster upgrades cluster has gone through
type ClusterUpgradeHistory struct {
	// READ-ONLY; The UTC time when the upgrade ended
	End *string

	// READ-ONLY; Name of the upgrade procedure performed on the cluster
	Name *string

	// READ-ONLY; The UTC time when the upgrade started
	Start *string
}

// ClusterUpgradeHistoryList - The response of the List Cluster Upgrade History operation
type ClusterUpgradeHistoryList struct {
	// Link to the next set of results. Empty unless the value parameter contains an incomplete list of Event Hubs Clusters.
	NextLink *string
...
// ClusterUpgradeWindow - Contains all settings for the cluster upgrade window.
type ClusterUpgradeWindow struct {
	Clusterupgradewindowproperties *ClusterUpgradeWindowClusterupgradewindowproperties
}

type ClusterUpgradeWindowClusterupgradewindowproperties struct {
	// Preferred ring to perform upgrade
	Preferredring *PreferredRing
...
type FailOverProperties struct {
	...
	// Unplanned Geo-Failover, which switches the role between primary and secondary
	// immediately. The data that is not being replicated yet will be discarded. Use maximumGracePeriodInMins > 0: For Planned
	// Geo-Failover/DR Drill, continue replicating data until grace period expires. Any
	// data that is not replicated during the grace period will be discarded. During the replication the namespace stops accepting
	// any new publishing requests
	MaximumGracePeriodInMins *int32
...
type GeoDataReplicationProperties struct {
	...
	// When the lag exceeds the configured amount, operations on the primary replica
	// will be failed. The allowed values are 0 and 5 minutes to 1 day.
