...
// The returned ServerBasedPerformanceTierServerTransport instance is connected to an instance of armmariadb.ServerBasedPerformanceTierClient via the
...
func NewServerBasedPerformanceTierServerTransport(srv *ServerBasedPerformanceTierServer) *ServerBasedPerformanceTierServerTransport {
	return &ServerBasedPerformanceTierServerTransport{
		srv:          srv,
		newListPager: newTracker[azfake.PagerResponder[armmariadb.ServerBasedPerformanceTierClientListResponse]](),
	}
}

// ServerBasedPerformanceTierServerTransport connects instances of armmariadb.ServerBasedPerformanceTierClient to instances of ServerBasedPerformanceTierServer.
...
type ServerBasedPerformanceTierServerTransport struct {
	...
	newListPager *tracker[azfake.PagerResponder[armmariadb.ServerBasedPerformanceTierClientListResponse]]
}
...
func (s *ServerBasedPerformanceTierServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	...
	go func() {
		...
		var res result
		if serverBasedPerformanceTierServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = serverBasedPerformanceTierServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
...
func (s *ServerBasedPerformanceTierServerTransport) dispatchNewListPager(req *http.Request) (*http.Response, error) {
	...
	if newListPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.DBforMariaDB/servers/(?P<serverName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/performanceTiers`
...
// set this to conditionally intercept incoming requests to ServerBasedPerformanceTierServerTransport
var serverBasedPerformanceTierServerTransportInterceptor interface {
