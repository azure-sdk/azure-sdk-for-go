...
// GetBlobReferenceSAS - Get a SAS for a blob reference associated with the data asset
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2023-10-01
//   - resourceGroupName - The name of the resource group. The name is case insensitive.
...
//   - options - RegistryDataVersionsClientGetBlobReferenceSASOptions contains the optional parameters for the RegistryDataVersionsClient.GetBlobReferenceSAS
//     method.
func (client *RegistryDataVersionsClient) GetBlobReferenceSAS(ctx context.Context, resourceGroupName string, registryName string, name string, version string, body BlobReferenceSASRequestDto, options *RegistryDataVersionsClientGetBlobReferenceSASOptions) (RegistryDataVersionsClientGetBlobReferenceSASResponse, error) {
	var err error
	req, err := client.getBlobReferenceSASCreateRequest(ctx, resourceGroupName, registryName, name, version, body, options)
	if err != nil {
		return RegistryDataVersionsClientGetBlobReferenceSASResponse{}, err
	}
	httpResp, err := client.internal.Pipeline().Do(req)
	if err != nil {
		return RegistryDataVersionsClientGetBlobReferenceSASResponse{}, err
...
func (client *RegistryDataVersionsClient) getBlobReferenceSASCreateRequest(ctx context.Context, resourceGroupName string, registryName string, name string, version string, body BlobReferenceSASRequestDto, options *RegistryDataVersionsClientGetBlobReferenceSASOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.MachineLearningServices/registries/{registryName}/data/{name}/versions/{version}/getBlobReferenceSAS"
	if client.subscriptionID == "" {
		return nil, errors.New("parameter client.subscriptionID cannot be empty")
	}
	urlPath = strings.ReplaceAll(urlPath, "{subscriptionId}", url.PathEscape(client.subscriptionID))
...
func (client *RegistryDataVersionsClient) getBlobReferenceSASHandleResponse(resp *http.Response) (RegistryDataVersionsClientGetBlobReferenceSASResponse, error) {
	result := RegistryDataVersionsClientGetBlobReferenceSASResponse{}
