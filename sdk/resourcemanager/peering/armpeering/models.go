...
type ConnectivityProbe struct {
	...
	// READ-ONLY; The Azure region where traffic will originate from.
	AzureRegion *string

	// READ-ONLY; The endpoint IP address where traffic will be sent to from Azure.
	Endpoint *string

	// READ-ONLY; Set to contain the prefixes that agents in Azure will send traffic from. For peers to allow into their network
	// the connectivity probe traffic can reach their endpoint.
	PrefixesToAccesslist []*string

	// READ-ONLY; The protocol of the traffic that will be sent.
	Protocol *Protocol
}
...
type ExchangeConnection struct {
	...
	// The old V4 BGP session associated with the connection during migration on the same device. Will be used for Work-Window
	// validation
	MigrationWorkWindowBgpSessionSameDevice map[string]*Enum13
