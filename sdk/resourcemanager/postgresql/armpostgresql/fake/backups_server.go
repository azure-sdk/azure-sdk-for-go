...
// BackupsServer is a fake server for instances of the armpostgresql.BackupsClient type.
type BackupsServer struct {
	// Get is the fake for method BackupsClient.Get
	// HTTP status codes to indicate success: http.StatusOK
	Get func(ctx context.Context, resourceGroupName string, serverName string, backupName string, options *armpostgresql.BackupsClientGetOptions) (resp azfake.Responder[armpostgresql.BackupsClientGetResponse], errResp azfake.ErrorResponder)

	// NewListByServerPager is the fake for method BackupsClient.NewListByServerPager
	// HTTP status codes to indicate success: http.StatusOK
	NewListByServerPager func(resourceGroupName string, serverName string, options *armpostgresql.BackupsClientListByServerOptions) (resp azfake.PagerResponder[armpostgresql.BackupsClientListByServerResponse])
}

// NewBackupsServerTransport creates a new instance of BackupsServerTransport with the provided implementation.
// The returned BackupsServerTransport instance is connected to an instance of armpostgresql.BackupsClient via the
// azcore.ClientOptions.Transporter field in the client's constructor parameters.
func NewBackupsServerTransport(srv *BackupsServer) *BackupsServerTransport {
	return &BackupsServerTransport{
		srv:                  srv,
		newListByServerPager: newTracker[azfake.PagerResponder[armpostgresql.BackupsClientListByServerResponse]](),
	}
}

// BackupsServerTransport connects instances of armpostgresql.BackupsClient to instances of BackupsServer.
// Don't use this type directly, use NewBackupsServerTransport instead.
type BackupsServerTransport struct {
	srv                  *BackupsServer
	newListByServerPager *tracker[azfake.PagerResponder[armpostgresql.BackupsClientListByServerResponse]]
}
...
func (b *BackupsServerTransport) dispatchNewListByServerPager(req *http.Request) (*http.Response, error) {
	...
	if newListByServerPager == nil {
		...
		server.PagerResponderInjectNextLinks(newListByServerPager, req, func(page *armpostgresql.BackupsClientListByServerResponse, createLink func() string) {
