...
type SQLSitesControllerServer struct {
	...
	// ErrorDetailedSummary is the fake for method SQLSitesControllerClient.ErrorDetailedSummary
	// HTTP status codes to indicate success: http.StatusOK
	ErrorDetailedSummary func(ctx context.Context, resourceGroupName string, siteName string, sqlSiteName string, body armmigrate.ErrorSummaryRequest, options *armmigrate.SQLSitesControllerClientErrorDetailedSummaryOptions) (resp azfake.Responder[armmigrate.SQLSitesControllerClientErrorDetailedSummaryResponse], errResp azfake.ErrorResponder)
...
func (s *SQLSitesControllerServerTransport) Do(req *http.Request) (*http.Response, error) {
	...
	case "SQLSitesControllerClient.ErrorDetailedSummary":
		resp, err = s.dispatchErrorDetailedSummary(req)
...
func (s *SQLSitesControllerServerTransport) dispatchErrorDetailedSummary(req *http.Request) (*http.Response, error) {
	if s.srv.ErrorDetailedSummary == nil {
		return nil, &nonRetriableError{errors.New("fake for method ErrorDetailedSummary not implemented")}
	}
	const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.OffAzure/masterSites/(?P<siteName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/sqlSites/(?P<sqlSiteName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/errorDetailedSummary`
	regex := regexp.MustCompile(regexStr)
	matches := regex.FindStringSubmatch(req.URL.EscapedPath())
	if matches == nil || len(matches) < 4 {
		return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
	...
	respr, errRespr := s.srv.ErrorDetailedSummary(req.Context(), resourceGroupNameParam, siteNameParam, sqlSiteNameParam, body, nil)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
	respContent := server.GetResponseContent(respr)
	...
	resp, err := server.MarshalResponseAsJSON(respContent, server.GetResponse(respr).SQLSiteErrorDetails, req)
	if err != nil {
		return nil, err
	}
	return resp, nil
