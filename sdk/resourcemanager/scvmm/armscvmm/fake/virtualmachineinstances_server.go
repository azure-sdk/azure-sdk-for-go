...
type VirtualMachineInstancesServer struct {
	...
	// NewListByArmPager is the fake for method VirtualMachineInstancesClient.NewListByArmPager
	// HTTP status codes to indicate success: http.StatusOK
	NewListByArmPager func(resourceURI string, options *armscvmm.VirtualMachineInstancesClientListByArmOptions) (resp azfake.PagerResponder[armscvmm.VirtualMachineInstancesClientListByArmResponse])
...
func NewVirtualMachineInstancesServerTransport(srv *VirtualMachineInstancesServer) *VirtualMachineInstancesServerTransport {
	return &VirtualMachineInstancesServerTransport{
		...
		newListByArmPager:      newTracker[azfake.PagerResponder[armscvmm.VirtualMachineInstancesClientListByArmResponse]](),
...
type VirtualMachineInstancesServerTransport struct {
	...
	newListByArmPager      *tracker[azfake.PagerResponder[armscvmm.VirtualMachineInstancesClientListByArmResponse]]
...
func (v *VirtualMachineInstancesServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	...
	case "VirtualMachineInstancesClient.NewListByArmPager":
		resp, err = v.dispatchNewListByArmPager(req)
...
func (v *VirtualMachineInstancesServerTransport) dispatchNewListByArmPager(req *http.Request) (*http.Response, error) {
	if v.srv.NewListByArmPager == nil {
		return nil, &nonRetriableError{errors.New("fake for method NewListByArmPager not implemented")}
	}
	newListByArmPager := v.newListByArmPager.get(req)
	if newListByArmPager == nil {
		...
		resp := v.srv.NewListByArmPager(resourceURIParam, nil)
		newListByArmPager = &resp
		v.newListByArmPager.add(req, newListByArmPager)
		server.PagerResponderInjectNextLinks(newListByArmPager, req, func(page *armscvmm.VirtualMachineInstancesClientListByArmResponse, createLink func() string) {
			page.NextLink = to.Ptr(createLink())
		})
	}
	resp, err := server.PagerResponderNext(newListByArmPager, req)
	if err != nil {
		return nil, err
	}
	if !contains([]int{http.StatusOK}, resp.StatusCode) {
		v.newListByArmPager.remove(req)
		return nil, &nonRetriableError{fmt.Errorf("unexpected status code %d. acceptable values are http.StatusOK", resp.StatusCode)}
	}
	if !server.PagerResponderMore(newListByArmPager) {
		v.newListByArmPager.remove(req)
	}
	return resp, nil
}
