...
const fqdn = "github.com/Azure/azure-sdk-for-go/services/preview/mysql/mgmt/2021-12-01-preview/mysqlflexibleservers"
...
type Backup struct {
	...
	// GeoRedundantBackup - Whether or not geo redundant backup is enabled. Possible values include: 'Enabled', 'Disabled'
...
type ConfigurationListForBatchUpdate struct {
	...
	ResetAllToDefault *bool `json:"resetAllToDefault,omitempty"`
}
...
type ConfigurationProperties struct {
	...
	// Source - Source of the configuration. Possible values include: 'SystemDefault', 'UserOverride'
...
type DataEncryption struct {
	...
	// Type - The key type, AzureKeyVault for enable cmk, SystemManaged for disable cmk. Possible values include: 'AzureKeyVault', 'SystemManaged'
...
type HighAvailability struct {
	...
	// State - READ-ONLY; The state of server high availability. Possible values include: 'NotEnabled', 'CreatingStandby', 'Healthy', 'FailingOver', 'RemovingStandby'
...
type Identity struct {
	...
	// Type - Type of managed service identity. Possible values include: 'UserAssigned'
...
type LogFileListResult struct {
	autorest.Response `json:"-"`
	// Value - The list of logFiles in a server
	Value *[]LogFile `json:"value,omitempty"`
	// NextLink - The link used to get the next page of operations.
	NextLink *string `json:"nextLink,omitempty"`
}

// LogFileListResultIterator provides access to a complete listing of LogFile values.
type LogFileListResultIterator struct {
	i    int
	page LogFileListResultPage
}

// NextWithContext advances to the next value.  If there was an error making
// the request the iterator does not advance and the error is returned.
func (iter *LogFileListResultIterator) NextWithContext(ctx context.Context) (err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/LogFileListResultIterator.NextWithContext")
		defer func() {
			sc := -1
			if iter.Response().Response.Response != nil {
				sc = iter.Response().Response.Response.StatusCode
...
// the request the iterator does not advance and the error is returned.
// Deprecated: Use NextWithContext() instead.
func (iter *LogFileListResultIterator) Next() error {
	return iter.NextWithContext(context.Background())
}

// NotDone returns true if the enumeration should be started or is not yet complete.
func (iter LogFileListResultIterator) NotDone() bool {
	return iter.page.NotDone() && iter.i < len(iter.page.Values())
}

// Response returns the raw server response from the last page request.
func (iter LogFileListResultIterator) Response() LogFileListResult {
	return iter.page.Response()
}

// Value returns the current value or a zero-initialized value if the
// iterator has advanced beyond the end of the collection.
func (iter LogFileListResultIterator) Value() LogFile {
	if !iter.page.NotDone() {
		return LogFile{}
	}
	return iter.page.Values()[iter.i]
...
// Creates a new instance of the LogFileListResultIterator type.
func NewLogFileListResultIterator(page LogFileListResultPage) LogFileListResultIterator {
	return LogFileListResultIterator{page: page}
}

// IsEmpty returns true if the ListResult contains no values.
func (lflr LogFileListResult) IsEmpty() bool {
	return lflr.Value == nil || len(*lflr.Value) == 0
}

// hasNextLink returns true if the NextLink is not empty.
func (lflr LogFileListResult) hasNextLink() bool {
	return lflr.NextLink != nil && len(*lflr.NextLink) != 0
}

// logFileListResultPreparer prepares a request to retrieve the next set of results.
// It returns nil if no more results exist.
func (lflr LogFileListResult) logFileListResultPreparer(ctx context.Context) (*http.Request, error) {
	if !lflr.hasNextLink() {
		return nil, nil
	}
	return autorest.Prepare((&http.Request{}).WithContext(ctx),
		autorest.AsJSON(),
		autorest.AsGet(),
		autorest.WithBaseURL(to.String(lflr.NextLink)))
}

// LogFileListResultPage contains a page of LogFile values.
type LogFileListResultPage struct {
	fn   func(context.Context, LogFileListResult) (LogFileListResult, error)
	lflr LogFileListResult
}

// NextWithContext advances to the next page of values.  If there was an error making
// the request the page does not advance and the error is returned.
func (page *LogFileListResultPage) NextWithContext(ctx context.Context) (err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/LogFileListResultPage.NextWithContext")
		defer func() {
			sc := -1
			if page.Response().Response.Response != nil {
				sc = page.Response().Response.Response.StatusCode
	...
	for {
		next, err := page.fn(ctx, page.lflr)
		if err != nil {
			return err
		}
		page.lflr = next
...
// the request the page does not advance and the error is returned.
// Deprecated: Use NextWithContext() instead.
func (page *LogFileListResultPage) Next() error {
	return page.NextWithContext(context.Background())
}

// NotDone returns true if the page enumeration should be started or is not yet complete.
func (page LogFileListResultPage) NotDone() bool {
	return !page.lflr.IsEmpty()
}

// Response returns the raw server response from the last page request.
func (page LogFileListResultPage) Response() LogFileListResult {
	return page.lflr
}

// Values returns the slice of values for the current page or nil if there are no values.
func (page LogFileListResultPage) Values() []LogFile {
	if page.lflr.IsEmpty() {
		return nil
	}
	return *page.lflr.Value
}

// Creates a new instance of the LogFileListResultPage type.
func NewLogFileListResultPage(cur LogFileListResult, getNextPage func(context.Context, LogFileListResult) (LogFileListResult, error)) LogFileListResultPage {
	return LogFileListResultPage{
		fn:   getNextPage,
		lflr: cur,
	}
}
...
type LogFileProperties struct {
	// SizeInKB - The size in kb of the logFile.
...
type Network struct {
	// PublicNetworkAccess - READ-ONLY; Whether or not public network access is allowed for this server. Value is 'Disabled' when server has VNet integration. Possible values include: 'Enabled', 'Disabled'
...
type ServerProperties struct {
	...
	// Version - Server version. Possible values include: 'FiveFullStopSeven', 'EightFullStopZeroFullStopTwoOne'
	Version ServerVersion `json:"version,omitempty"`
	// AvailabilityZone - availability Zone information of the server.
	AvailabilityZone *string `json:"availabilityZone,omitempty"`
	// CreateMode - The mode to create a new MySQL server. Possible values include: 'Default', 'PointInTimeRestore', 'Replica', 'GeoRestore'
...
type ServerRestartParameter struct {
	// RestartWithFailover - Whether or not failover to standby server when restarting a server with high availability enabled. Possible values include: 'Enabled', 'Disabled'
...
type Storage struct {
	...
	// AutoGrow - Enable Storage Auto Grow or not. Possible values include: 'Enabled', 'Disabled'
