...
// The returned ServerSecurityAlertPoliciesServerTransport instance is connected to an instance of armmariadb.ServerSecurityAlertPoliciesClient via the
...
func NewServerSecurityAlertPoliciesServerTransport(srv *ServerSecurityAlertPoliciesServer) *ServerSecurityAlertPoliciesServerTransport {
	return &ServerSecurityAlertPoliciesServerTransport{
		srv:                  srv,
		beginCreateOrUpdate:  newTracker[azfake.PollerResponder[armmariadb.ServerSecurityAlertPoliciesClientCreateOrUpdateResponse]](),
		newListByServerPager: newTracker[azfake.PagerResponder[armmariadb.ServerSecurityAlertPoliciesClientListByServerResponse]](),
	}
}

// ServerSecurityAlertPoliciesServerTransport connects instances of armmariadb.ServerSecurityAlertPoliciesClient to instances of ServerSecurityAlertPoliciesServer.
...
type ServerSecurityAlertPoliciesServerTransport struct {
	...
	beginCreateOrUpdate  *tracker[azfake.PollerResponder[armmariadb.ServerSecurityAlertPoliciesClientCreateOrUpdateResponse]]
	newListByServerPager *tracker[azfake.PagerResponder[armmariadb.ServerSecurityAlertPoliciesClientListByServerResponse]]
}
...
func (s *ServerSecurityAlertPoliciesServerTransport) dispatchBeginCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginCreateOrUpdate == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.DBforMariaDB/servers/(?P<serverName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/securityAlertPolicies/(?P<securityAlertPolicyName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if matches == nil || len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		}
		body, err := server.UnmarshalRequestAsJSON[armmariadb.ServerSecurityAlertPolicy](req)
		if err != nil {
			return nil, err
		}
		...
		securityAlertPolicyNameParam, err := parseWithCast(matches[regex.SubexpIndex("securityAlertPolicyName")], func(v string) (armmariadb.SecurityAlertPolicyName, error) {
			p, unescapeErr := url.PathUnescape(v)
			if unescapeErr != nil {
				return "", unescapeErr
			}
			return armmariadb.SecurityAlertPolicyName(p), nil
		})
		if err != nil {
			return nil, err
		}
...
func (s *ServerSecurityAlertPoliciesServerTransport) dispatchNewListByServerPager(req *http.Request) (*http.Response, error) {
	...
	if newListByServerPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.DBforMariaDB/servers/(?P<serverName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/securityAlertPolicies`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		...
		server.PagerResponderInjectNextLinks(newListByServerPager, req, func(page *armmariadb.ServerSecurityAlertPoliciesClientListByServerResponse, createLink func() string) {
