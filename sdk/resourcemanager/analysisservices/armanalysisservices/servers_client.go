...
// GetDetais - Gets details about the specified Analysis Services server.
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2017-08-01
//   - resourceGroupName - The name of the Azure Resource group of which a given Analysis Services server is part. This name must
//     be at least 1 character in length, and no more than 90.
//   - serverName - The name of the Analysis Services server. It must be a minimum of 3 characters, and a maximum of 63.
//   - options - ServersClientGetDetaisOptions contains the optional parameters for the ServersClient.GetDetais method.
func (client *ServersClient) GetDetais(ctx context.Context, resourceGroupName string, serverName string, options *ServersClientGetDetaisOptions) (ServersClientGetDetaisResponse, error) {
	req, err := client.getDetaisCreateRequest(ctx, resourceGroupName, serverName, options)
	if err != nil {
		return ServersClientGetDetaisResponse{}, err
	}
	resp, err := client.pl.Do(req)
	if err != nil {
		return ServersClientGetDetaisResponse{}, err
	...
	if !runtime.HasStatusCode(resp, http.StatusOK) {
		return ServersClientGetDetaisResponse{}, runtime.NewResponseError(resp)
	}
	return client.getDetaisHandleResponse(resp)
}

// getDetaisCreateRequest creates the GetDetais request.
func (client *ServersClient) getDetaisCreateRequest(ctx context.Context, resourceGroupName string, serverName string, options *ServersClientGetDetaisOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.AnalysisServices/servers/{serverName}"
	if resourceGroupName == "" {
		return nil, errors.New("parameter resourceGroupName cannot be empty")
	}
...
// getDetaisHandleResponse handles the GetDetais response.
func (client *ServersClient) getDetaisHandleResponse(resp *http.Response) (ServersClientGetDetaisResponse, error) {
	result := ServersClientGetDetaisResponse{}
