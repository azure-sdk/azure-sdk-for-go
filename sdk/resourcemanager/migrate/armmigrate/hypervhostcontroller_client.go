...
//   - siteName - Site name
//   - hostName - Host name
//   - options - HypervHostControllerClientDeleteOptions contains the optional parameters for the HypervHostControllerClient.Delete
//     method.
func (client *HypervHostControllerClient) Delete(ctx context.Context, resourceGroupName string, siteName string, hostName string, options *HypervHostControllerClientDeleteOptions) (HypervHostControllerClientDeleteResponse, error) {
	var err error
	const operationName = "HypervHostControllerClient.Delete"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.deleteCreateRequest(ctx, resourceGroupName, siteName, hostName, options)
	if err != nil {
		return HypervHostControllerClientDeleteResponse{}, err
	}
	httpResp, err := client.internal.Pipeline().Do(req)
	if err != nil {
		return HypervHostControllerClientDeleteResponse{}, err
	...
	return HypervHostControllerClientDeleteResponse{}, nil
}

// deleteCreateRequest creates the Delete request.
func (client *HypervHostControllerClient) deleteCreateRequest(ctx context.Context, resourceGroupName string, siteName string, hostName string, options *HypervHostControllerClientDeleteOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.OffAzure/hypervSites/{siteName}/hosts/{hostName}"
	if client.subscriptionID == "" {
		return nil, errors.New("parameter client.subscriptionID cannot be empty")
	}
