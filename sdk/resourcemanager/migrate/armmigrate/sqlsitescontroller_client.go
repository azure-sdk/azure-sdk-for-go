...
// ErrorDetailedSummary - Method to get error detailed summary from SQL site.
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2024-05-01-preview
//   - resourceGroupName - The name of the resource group. The name is case insensitive.
...
//   - options - SQLSitesControllerClientErrorDetailedSummaryOptions contains the optional parameters for the SQLSitesControllerClient.ErrorDetailedSummary
//     method.
func (client *SQLSitesControllerClient) ErrorDetailedSummary(ctx context.Context, resourceGroupName string, siteName string, sqlSiteName string, body ErrorSummaryRequest, options *SQLSitesControllerClientErrorDetailedSummaryOptions) (SQLSitesControllerClientErrorDetailedSummaryResponse, error) {
	var err error
	const operationName = "SQLSitesControllerClient.ErrorDetailedSummary"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.errorDetailedSummaryCreateRequest(ctx, resourceGroupName, siteName, sqlSiteName, body, options)
	if err != nil {
		return SQLSitesControllerClientErrorDetailedSummaryResponse{}, err
	}
	httpResp, err := client.internal.Pipeline().Do(req)
	if err != nil {
		return SQLSitesControllerClientErrorDetailedSummaryResponse{}, err
	...
	resp, err := client.errorDetailedSummaryHandleResponse(httpResp)
	return resp, err
}

// errorDetailedSummaryCreateRequest creates the ErrorDetailedSummary request.
func (client *SQLSitesControllerClient) errorDetailedSummaryCreateRequest(ctx context.Context, resourceGroupName string, siteName string, sqlSiteName string, body ErrorSummaryRequest, options *SQLSitesControllerClientErrorDetailedSummaryOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.OffAzure/masterSites/{siteName}/sqlSites/{sqlSiteName}/errorDetailedSummary"
	if client.subscriptionID == "" {
		return nil, errors.New("parameter client.subscriptionID cannot be empty")
	}
	urlPath = strings.ReplaceAll(urlPath, "{subscriptionId}", url.PathEscape(client.subscriptionID))
...
// errorDetailedSummaryHandleResponse handles the ErrorDetailedSummary response.
func (client *SQLSitesControllerClient) errorDetailedSummaryHandleResponse(resp *http.Response) (SQLSitesControllerClientErrorDetailedSummaryResponse, error) {
	result := SQLSitesControllerClientErrorDetailedSummaryResponse{}
	if err := runtime.UnmarshalAsJSON(resp, &result.SQLSiteErrorDetails); err != nil {
		return SQLSitesControllerClientErrorDetailedSummaryResponse{}, err
	}
	return result, nil
}
