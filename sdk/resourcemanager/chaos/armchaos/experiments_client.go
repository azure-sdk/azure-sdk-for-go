...
func (client *ExperimentsClient) BeginStart(ctx context.Context, resourceGroupName string, experimentName string, options *ExperimentsClientBeginStartOptions) (*runtime.Poller[ExperimentsClientStartResponse], error) {
	if options == nil || options.ResumeToken == "" {
		...
		return runtime.NewPoller(resp, client.pl, &runtime.NewPollerOptions[ExperimentsClientStartResponse]{
			FinalStateVia: runtime.FinalStateViaLocation,
		})
	} else {
		return runtime.NewPollerFromResumeToken[ExperimentsClientStartResponse](options.ResumeToken, client.pl, nil)
	}
}
