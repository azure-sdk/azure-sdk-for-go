...
type RPServiceServer struct {
	// PrivateStoreCollectionOfferPlansList is the fake for method RPServiceClient.PrivateStoreCollectionOfferPlansList
	// HTTP status codes to indicate success: http.StatusOK
	PrivateStoreCollectionOfferPlansList func(ctx context.Context, privateStoreID string, collectionID string, offerID string, options *armmarketplace.RPServiceClientPrivateStoreCollectionOfferPlansListOptions) (resp azfake.Responder[armmarketplace.RPServiceClientPrivateStoreCollectionOfferPlansListResponse], errResp azfake.ErrorResponder)
...
func (r *RPServiceServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	...
	go func() {
		...
		if !intercepted {
			switch method {
			case "RPServiceClient.PrivateStoreCollectionOfferPlansList":
				res.resp, res.err = r.dispatchPrivateStoreCollectionOfferPlansList(req)
...
func (r *RPServiceServerTransport) dispatchPrivateStoreCollectionOfferPlansList(req *http.Request) (*http.Response, error) {
	if r.srv.PrivateStoreCollectionOfferPlansList == nil {
		return nil, &nonRetriableError{errors.New("fake for method PrivateStoreCollectionOfferPlansList not implemented")}
	}
	const regexStr = `/providers/Microsoft\.Marketplace/privateStores/(?P<privateStoreId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/collections/(?P<collectionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/offers/(?P<offerId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/plans`
	...
	var options *armmarketplace.RPServiceClientPrivateStoreCollectionOfferPlansListOptions
	if !reflect.ValueOf(body).IsZero() {
		options = &armmarketplace.RPServiceClientPrivateStoreCollectionOfferPlansListOptions{
			Payload: &body,
		}
	}
	respr, errRespr := r.srv.PrivateStoreCollectionOfferPlansList(req.Context(), privateStoreIDParam, collectionIDParam, offerIDParam, options)
	...
	resp, err := server.MarshalResponseAsJSON(respContent, server.GetResponse(respr).PlanListResponse, req)
	if err != nil {
		return nil, err
	}
	return resp, nil
