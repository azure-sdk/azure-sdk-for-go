...
// policy, Unlocked state allows increase and decrease of immutability retention time and also allows toggling allowProtectedAppendWrites
// property, Locked state only allows the increase of the immutability retention time. A policy can only be created in a Disabled
// or Unlocked state and can be toggled between the two states. Only a policy in an Unlocked state can transition to a Locked
...
// is set to AD (AD DS authentication), this property is optional. If provided, samAccountName should also be provided. For
...
// restore is ongoing. - Complete: Indicates that blob restore has been completed successfully. - Failed: Indicates that blob
// restore is failed.
...
// in a single subscription, which creates accounts in an Azure DNS Zone and the endpoint URL will have an alphanumeric DNS
// Zone identifier.
...
// The 'Log' action can be used for audit purposes and the 'Block' action can be used to block and deny the usage of SAS tokens
// that do not adhere to the sas policy expiration period.
...
// location is active and operational. - Bootstrap: Indicates initial synchronization from the primary location to the secondary
// location is in progress.This typically occurs when replication is first enabled. - Unavailable: Indicates that the secondary
...
// is set when the SKU has requiredQuotas parameter as the subscription does not belong to that quota. The "NotAvailableForSubscription"
...
// APIs; Container (c): Access to container-level APIs; Object (o): Access to object-level APIs for blobs, queue messages,
...
// PossibleStorageTaskAssignmentProvisioningStateValues returns the possible values for the StorageTaskAssignmentProvisioningState const type.
func PossibleStorageTaskAssignmentProvisioningStateValues() []StorageTaskAssignmentProvisioningState {
	return []StorageTaskAssignmentProvisioningState{
		StorageTaskAssignmentProvisioningStateAccepted,
		StorageTaskAssignmentProvisioningStateCanceled,
		StorageTaskAssignmentProvisioningStateCreating,
		StorageTaskAssignmentProvisioningStateDeleting,
