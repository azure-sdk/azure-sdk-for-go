...
- Type of `NameAvailability.NameAvailable` has been changed from `*bool` to `*NameAvailableEnum`
- Type of `Operation.IsDataAction` has been changed from `*bool` to `*IsDataActionEnum`
- Type of `Operation.Properties` has been changed from `map[string]interface{}` to `map[string]any`
- Type of `ServerProperties.EnablePublicIP` has been changed from `*bool` to `*EnablePublicIPEnum`
- Type of `ServerProperties.EnableHa` has been changed from `*bool` to `*EnableHaEnum`
...
- New type alias `CoordinatorEnablePublicIPEnum` with values `CoordinatorEnablePublicIPEnumFalse`, `CoordinatorEnablePublicIPEnumTrue`
- New type alias `EnableHaEnum` with values `EnableHaEnumFalse`, `EnableHaEnumTrue`
- New type alias `EnablePublicIPEnum` with values `EnablePublicIPEnumFalse`, `EnablePublicIPEnumTrue`
- New type alias `EnableShardsOnCoordinatorEnum` with values `EnableShardsOnCoordinatorEnumFalse`, `EnableShardsOnCoordinatorEnumTrue`
- New type alias `IsDataActionEnum` with values `IsDataActionEnumFalse`, `IsDataActionEnumTrue`
- New type alias `NameAvailableEnum` with values `NameAvailableEnumFalse`, `NameAvailableEnumTrue`
- New type alias `NodeEnablePublicIPEnum` with values `NodeEnablePublicIPEnumFalse`, `NodeEnablePublicIPEnumTrue`
...
- New type alias `RequiresRestartEnum` with values `RequiresRestartEnumFalse`, `RequiresRestartEnumTrue`
