...
func (d DynamicPoolConfiguration) MarshalJSON() ([]byte, error) {
	objectMap := make(map[string]any)
	populate(objectMap, "coolDownPeriodInSecond", d.CoolDownPeriodInSecond)
...
func (d *DynamicPoolConfiguration) UnmarshalJSON(data []byte) error {
	...
	for key, val := range rawMsg {
		var err error
		switch key {
		case "coolDownPeriodInSecond":
			err = unpopulate(val, "CoolDownPeriodInSecond", &d.CoolDownPeriodInSecond)
			delete(rawMsg, key)
		case "executionType":
...
func (j JavaComponentProperties) MarshalJSON() ([]byte, error) {
	objectMap := make(map[string]any)
	populate(objectMap, "componentType", j.ComponentType)
