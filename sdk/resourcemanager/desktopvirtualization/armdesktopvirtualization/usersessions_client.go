...
//     method.
func (client *UserSessionsClient) SendMessage(ctx context.Context, resourceGroupName string, hostPoolName string, sessionHostName string, userSessionID string, sendMessage SendMessage, options *UserSessionsClientSendMessageOptions) (UserSessionsClientSendMessageResponse, error) {
	var err error
	const operationName = "UserSessionsClient.SendMessage"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.sendMessageCreateRequest(ctx, resourceGroupName, hostPoolName, sessionHostName, userSessionID, sendMessage, options)
	if err != nil {
...
func (client *UserSessionsClient) sendMessageCreateRequest(ctx context.Context, resourceGroupName string, hostPoolName string, sessionHostName string, userSessionID string, sendMessage SendMessage, options *UserSessionsClientSendMessageOptions) (*policy.Request, error) {
	...
	req.Raw().URL.RawQuery = reqQP.Encode()
	req.Raw().Header["Accept"] = []string{"application/json"}
	if err := runtime.MarshalAsJSON(req, sendMessage); err != nil {
		return nil, err
	}
	return req, nil
}
