...
func (client *SessionHostsClient) listCreateRequest(ctx context.Context, resourceGroupName string, hostPoolName string, options *SessionHostsClientListOptions) (*policy.Request, error) {
	...
	if options != nil && options.VMPath != nil {
		reqQP.Set("vmPath", *options.VMPath)
	}
	req.Raw().URL.RawQuery = reqQP.Encode()
	req.Raw().Header["Accept"] = []string{"application/json"}
	return req, nil
...
// RetryProvisioning - A synchronous resource action that returns no content.
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2024-08-08-preview
//   - resourceGroupName - The name of the resource group. The name is case insensitive.
//   - hostPoolName - The name of the host pool within the specified resource group
//   - sessionHostName - The name of the session host within the specified host pool
//   - options - SessionHostsClientRetryProvisioningOptions contains the optional parameters for the SessionHostsClient.RetryProvisioning
//     method.
func (client *SessionHostsClient) RetryProvisioning(ctx context.Context, resourceGroupName string, hostPoolName string, sessionHostName string, options *SessionHostsClientRetryProvisioningOptions) (SessionHostsClientRetryProvisioningResponse, error) {
	var err error
	const operationName = "SessionHostsClient.RetryProvisioning"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.retryProvisioningCreateRequest(ctx, resourceGroupName, hostPoolName, sessionHostName, options)
	if err != nil {
		return SessionHostsClientRetryProvisioningResponse{}, err
	}
	httpResp, err := client.internal.Pipeline().Do(req)
	if err != nil {
		return SessionHostsClientRetryProvisioningResponse{}, err
	...
	return SessionHostsClientRetryProvisioningResponse{}, nil
}

// retryProvisioningCreateRequest creates the RetryProvisioning request.
func (client *SessionHostsClient) retryProvisioningCreateRequest(ctx context.Context, resourceGroupName string, hostPoolName string, sessionHostName string, options *SessionHostsClientRetryProvisioningOptions) (*policy.Request, error) {
...
func (client *SessionHostsClient) Update(ctx context.Context, resourceGroupName string, hostPoolName string, sessionHostName string, sessionHost SessionHostPatch, options *SessionHostsClientUpdateOptions) (SessionHostsClientUpdateResponse, error) {
	var err error
	const operationName = "SessionHostsClient.Update"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.updateCreateRequest(ctx, resourceGroupName, hostPoolName, sessionHostName, sessionHost, options)
	if err != nil {
...
func (client *SessionHostsClient) updateCreateRequest(ctx context.Context, resourceGroupName string, hostPoolName string, sessionHostName string, sessionHost SessionHostPatch, options *SessionHostsClientUpdateOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.DesktopVirtualization/hostPools/{hostPoolName}/sessionHosts/{sessionHostName}"
	if client.subscriptionID == "" {
		return nil, errors.New("parameter client.subscriptionID cannot be empty")
	}
	...
	req.Raw().URL.RawQuery = reqQP.Encode()
	req.Raw().Header["Accept"] = []string{"application/json"}
	if err := runtime.MarshalAsJSON(req, sessionHost); err != nil {
		return nil, err
	}
	return req, nil
}
