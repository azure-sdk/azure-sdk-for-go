...
// The returned ServerParametersServerTransport instance is connected to an instance of armmariadb.ServerParametersClient via the
...
func NewServerParametersServerTransport(srv *ServerParametersServer) *ServerParametersServerTransport {
	return &ServerParametersServerTransport{
		srv:                           srv,
		beginListUpdateConfigurations: newTracker[azfake.PollerResponder[armmariadb.ServerParametersClientListUpdateConfigurationsResponse]](),
	}
}

// ServerParametersServerTransport connects instances of armmariadb.ServerParametersClient to instances of ServerParametersServer.
...
type ServerParametersServerTransport struct {
	...
	beginListUpdateConfigurations *tracker[azfake.PollerResponder[armmariadb.ServerParametersClientListUpdateConfigurationsResponse]]
}
...
func (s *ServerParametersServerTransport) dispatchBeginListUpdateConfigurations(req *http.Request) (*http.Response, error) {
	...
	if beginListUpdateConfigurations == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.DBforMariaDB/servers/(?P<serverName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/updateConfigurations`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if matches == nil || len(matches) < 3 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		}
		body, err := server.UnmarshalRequestAsJSON[armmariadb.ConfigurationListResult](req)
		if err != nil {
			return nil, err
		}
