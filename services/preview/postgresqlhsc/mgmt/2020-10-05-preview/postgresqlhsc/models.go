...
type ServerGroupProperties struct {
	...
	// State - A state of a server group that is visible to user. Possible values include: 'ServerStateReady', 'ServerStateDropping', 'ServerStateDisabled', 'ServerStateStarting', 'ServerStateStopping', 'ServerStateStopped', 'ServerStateUpdating', 'ServerStateProvisioning', 'ServerStateNeedsAttention'
...
type ServerGroupServerProperties struct {
	...
	// State - A state of a server that is visible to user. Possible values include: 'ServerStateReady', 'ServerStateDropping', 'ServerStateDisabled', 'ServerStateStarting', 'ServerStateStopping', 'ServerStateStopped', 'ServerStateUpdating', 'ServerStateProvisioning', 'ServerStateNeedsAttention'
	State ServerState `json:"state,omitempty"`
	// HaState - A state of a server group that is visible to user for HA feature. Possible values include: 'NotEnabled', 'CreatingStandby', 'ReplicatingData', 'FailingOver', 'Healthy', 'RemovingStandby', 'NotSync', 'NeedsAttention'
...
// ServerGroupsPromoteFuture an abstraction for monitoring and retrieving the results of a long-running
// operation.
type ServerGroupsPromoteFuture struct {
	azure.FutureAPI
	// Result returns the result of the asynchronous operation.
	// If the operation has not completed it will return an error.
	Result func(ServerGroupsClient) (autorest.Response, error)
...
// UnmarshalJSON is the custom unmarshaller for CreateFuture.
func (future *ServerGroupsPromoteFuture) UnmarshalJSON(body []byte) error {
	var azFuture azure.Future
	if err := json.Unmarshal(body, &azFuture); err != nil {
		return err
	}
...
// result is the default implementation for ServerGroupsPromoteFuture.Result.
func (future *ServerGroupsPromoteFuture) result(client ServerGroupsClient) (ar autorest.Response, err error) {
	var done bool
	done, err = future.DoneWithContext(context.Background(), client)
	if err != nil {
		err = autorest.NewErrorWithError(err, "postgresqlhsc.ServerGroupsPromoteFuture", "Result", future.Response(), "Polling failure")
		return
	}
	if !done {
		ar.Response = future.Response()
		err = azure.NewAsyncOpIncompleteError("postgresqlhsc.ServerGroupsPromoteFuture")
		return
	}
	ar.Response = future.Response()
	return
