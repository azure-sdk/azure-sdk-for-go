...
// GrantAccessOnVMSSVMInstance - Grants access to a VMSS VM Instance OS disk for DiskInspection scenario.
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2023-01-02
//   - resourceGroupName - The name of the resource group.
//   - vmssName - The name of the VirtualmachineScaleSet managing the managed disk. The name can't be changed after the disk is
//     created. Supported characters for the name are a-z, A-Z, 0-9, _ and -. The maximum name
//     length is 80 characters.
//   - diskName - The name of the managed disk that is being created. The name can't be changed after the disk is created. Supported
//     characters for the name are a-z, A-Z, 0-9, _ and -. The maximum name length is 80
//     characters.
//   - grantAccessData - Access data object supplied in the body of the get disk access operation. The supported accessLevel for
//     this action is 'ReadForDiskInspection' only.
//   - options - DisksClientGrantAccessOnVMSSVMInstanceOptions contains the optional parameters for the DisksClient.GrantAccessOnVMSSVMInstance
//     method.
func (client *DisksClient) GrantAccessOnVMSSVMInstance(ctx context.Context, resourceGroupName string, vmssName string, diskName string, grantAccessData GrantAccessData, options *DisksClientGrantAccessOnVMSSVMInstanceOptions) (DisksClientGrantAccessOnVMSSVMInstanceResponse, error) {
	var err error
	const operationName = "DisksClient.GrantAccessOnVMSSVMInstance"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.grantAccessOnVMSSVMInstanceCreateRequest(ctx, resourceGroupName, vmssName, diskName, grantAccessData, options)
	if err != nil {
		return DisksClientGrantAccessOnVMSSVMInstanceResponse{}, err
	}
	httpResp, err := client.internal.Pipeline().Do(req)
	if err != nil {
		return DisksClientGrantAccessOnVMSSVMInstanceResponse{}, err
	...
	resp, err := client.grantAccessOnVMSSVMInstanceHandleResponse(httpResp)
	return resp, err
}

// grantAccessOnVMSSVMInstanceCreateRequest creates the GrantAccessOnVMSSVMInstance request.
func (client *DisksClient) grantAccessOnVMSSVMInstanceCreateRequest(ctx context.Context, resourceGroupName string, vmssName string, diskName string, grantAccessData GrantAccessData, options *DisksClientGrantAccessOnVMSSVMInstanceOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Compute/virtualMachineScaleSets/{vmssName}/disks/{diskName}/beginGetAccess"
	if client.subscriptionID == "" {
		return nil, errors.New("parameter client.subscriptionID cannot be empty")
	}
	urlPath = strings.ReplaceAll(urlPath, "{subscriptionId}", url.PathEscape(client.subscriptionID))
...
// grantAccessOnVMSSVMInstanceHandleResponse handles the GrantAccessOnVMSSVMInstance response.
func (client *DisksClient) grantAccessOnVMSSVMInstanceHandleResponse(resp *http.Response) (DisksClientGrantAccessOnVMSSVMInstanceResponse, error) {
	result := DisksClientGrantAccessOnVMSSVMInstanceResponse{}
	if err := runtime.UnmarshalAsJSON(resp, &result.AccessURI); err != nil {
		return DisksClientGrantAccessOnVMSSVMInstanceResponse{}, err
	}
	return result, nil
}
