...
// MarshalJSON implements the json.Marshaller interface for type PrometheusRuleGroupActions.
func (p PrometheusRuleGroupActions) MarshalJSON() ([]byte, error) {
	objectMap := make(map[string]any)
	populate(objectMap, "actionGroups", p.ActionGroups)
	populate(objectMap, "actionProperties", p.ActionProperties)
	populate(objectMap, "customProperties", p.CustomProperties)
	return json.Marshal(objectMap)
}

// UnmarshalJSON implements the json.Unmarshaller interface for type PrometheusRuleGroupActions.
func (p *PrometheusRuleGroupActions) UnmarshalJSON(data []byte) error {
	var rawMsg map[string]json.RawMessage
	if err := json.Unmarshal(data, &rawMsg); err != nil {
		return fmt.Errorf("unmarshalling type %T: %v", p, err)
	}
	for key, val := range rawMsg {
		...
		switch key {
		case "actionGroups":
			err = unpopulate(val, "ActionGroups", &p.ActionGroups)
			delete(rawMsg, key)
