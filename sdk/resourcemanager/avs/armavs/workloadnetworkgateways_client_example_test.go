...
func ExampleWorkloadNetworkGatewaysClient_Get() {
	...
	res, err := clientFactory.NewWorkloadNetworkGatewaysClient().Get(ctx, "group1", "cloud1", "gateway1", nil)
	if err != nil {
	...
	// res = armavs.WorkloadNetworkGatewaysClientGetResponse{
...
func ExampleWorkloadNetworkGatewaysClient_NewListPager() {
	cred, err := azidentity.NewDefaultAzureCredential(nil)
	if err != nil {
		log.Fatalf("failed to obtain a credential: %v", err)
	}
	...
	pager := clientFactory.NewWorkloadNetworkGatewaysClient().NewListPager("group1", "cloud1", nil)
	for pager.More() {
		...
		// page = armavs.WorkloadNetworkGatewaysClientListResponse{
