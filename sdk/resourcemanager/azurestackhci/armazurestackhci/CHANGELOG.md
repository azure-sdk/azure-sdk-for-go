...
- Type of `ExtensionParameters.ProtectedSettings` has been changed from `any` to `*ExtensionParametersProtectedSettings`
- Type of `ExtensionParameters.Settings` has been changed from `any` to `*ExtensionParametersSettings`
...
- New struct `ExtensionParametersProtectedSettings`
- New struct `ExtensionParametersSettings`
...
- New field `EhcResourceID`, `LastLicensingTimestamp`, `NodeType`, `OSDisplayVersion` in struct `ClusterNode`
- New field `Identity` in struct `ClusterPatch`
- New field `ResourceProviderObjectID`, `SoftwareAssuranceProperties` in struct `ClusterProperties`
- New field `ClusterType`, `Manufacturer`, `SupportedCapabilities` in struct `ClusterReportedProperties`
