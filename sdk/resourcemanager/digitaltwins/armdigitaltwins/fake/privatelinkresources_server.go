...
type PrivateLinkResourcesServer struct {
	...
	Get func(ctx context.Context, resourceGroupName string, resourceName string, resourceID string, options *armdigitaltwins.PrivateLinkResourcesClientGetOptions) (resp azfake.Responder[armdigitaltwins.PrivateLinkResourcesClientGetResponse], errResp azfake.ErrorResponder)

	// List is the fake for method PrivateLinkResourcesClient.List
	// HTTP status codes to indicate success: http.StatusOK
	List func(ctx context.Context, resourceGroupName string, resourceName string, options *armdigitaltwins.PrivateLinkResourcesClientListOptions) (resp azfake.Responder[armdigitaltwins.PrivateLinkResourcesClientListResponse], errResp azfake.ErrorResponder)
}
...
// The returned PrivateLinkResourcesServerTransport instance is connected to an instance of armdigitaltwins.PrivateLinkResourcesClient via the
// azcore.ClientOptions.Transporter field in the client's constructor parameters.
func NewPrivateLinkResourcesServerTransport(srv *PrivateLinkResourcesServer) *PrivateLinkResourcesServerTransport {
	return &PrivateLinkResourcesServerTransport{srv: srv}
}

// PrivateLinkResourcesServerTransport connects instances of armdigitaltwins.PrivateLinkResourcesClient to instances of PrivateLinkResourcesServer.
...
func (p *PrivateLinkResourcesServerTransport) dispatchGet(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.DigitalTwins/digitalTwinsInstances/(?P<resourceName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/privateLinkResources/(?P<resourceId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)`
	...
	respr, errRespr := p.srv.Get(req.Context(), resourceGroupNameParam, resourceNameParam, resourceIDParam, nil)
...
func (p *PrivateLinkResourcesServerTransport) dispatchList(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.DigitalTwins/digitalTwinsInstances/(?P<resourceName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/privateLinkResources`
	regex := regexp.MustCompile(regexStr)
	matches := regex.FindStringSubmatch(req.URL.EscapedPath())
	...
	resp, err := server.MarshalResponseAsJSON(respContent, server.GetResponse(respr).GroupIDInformationResponse, req)
	if err != nil {
		return nil, err
	}
	return resp, nil
