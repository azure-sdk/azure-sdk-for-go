...
func (s *ServerDNSAliasesServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	...
	go func() {
		...
		var res result
		if serverDnsAliasesServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = serverDnsAliasesServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
			case "ServerDNSAliasesClient.BeginAcquire":
				res.resp, res.err = s.dispatchBeginAcquire(req)
...
// set this to conditionally intercept incoming requests to ServerDNSAliasesServerTransport
var serverDnsAliasesServerTransportInterceptor interface {
