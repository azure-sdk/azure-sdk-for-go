...
- Type of `ImportNewDatabaseDefinition.NetworkIsolation` has been changed from `*NetworkIsolationSettings` to `*NetworkIsolationSettingsAutoGenerated2`
...
- New struct `CreateOrUpdateProperties`
...
- New struct `DistributedAvailabilityGroupsCreateOrUpdateRequestBody`
...
- New struct `NetworkIsolationSettingsAutoGenerated2`
...
- New field `Databases`, `DistributedAvailabilityGroupName`, `InstanceAvailabilityGroupName`, `InstanceLinkRole`, `PartnerAvailabilityGroupName`, `PartnerEndpoint`, `PartnerLinkRole` in struct `DistributedAvailabilityGroupProperties`
...
- New field `IsBackupImmutable` in struct `LongTermRetentionBackupProperties`
...
- New field `CreateTime`, `ExternalGovernanceStatus`, `HybridSecondaryUsage`, `HybridSecondaryUsageDetected`, `IsGeneralPurposeV2`, `PricingModel`, `StorageIOps`, `StorageThroughputMBps`, `VirtualClusterID` in struct `ManagedInstanceProperties`
- New field `IncludedStorageIOps`, `IncludedStorageThroughputMBps`, `IopsIncludedValueOverrideFactorPerSelectedStorageGB`, `IopsMinValueOverrideFactorPerSelectedStorageGB`, `SupportedStorageIOps`, `SupportedStorageThroughputMBps`, `ThroughputMBpsIncludedValueOverrideFactorPerSelectedStorageGB`, `ThroughputMBpsMinValueOverrideFactorPerSelectedStorageGB` in struct `ManagedInstanceVcoresCapability`
...
- New field `ExternalGovernanceStatus`, `IsIPv6Enabled` in struct `ServerProperties`
