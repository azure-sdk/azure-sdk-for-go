...
type SecurityPerimeterAssociationsServer struct {
	...
	// Post is the fake for method SecurityPerimeterAssociationsClient.Post
	// HTTP status codes to indicate success: http.StatusOK
	Post func(ctx context.Context, resourceGroupName string, networkSecurityPerimeterName string, associationName string, parameters any, options *armnetwork.SecurityPerimeterAssociationsClientPostOptions) (resp azfake.Responder[armnetwork.SecurityPerimeterAssociationsClientPostResponse], errResp azfake.ErrorResponder)
}
...
func (s *SecurityPerimeterAssociationsServerTransport) Do(req *http.Request) (*http.Response, error) {
	...
	case "SecurityPerimeterAssociationsClient.Post":
		resp, err = s.dispatchPost(req)
	default:
...
func (s *SecurityPerimeterAssociationsServerTransport) dispatchPost(req *http.Request) (*http.Response, error) {
	if s.srv.Post == nil {
	...
	respr, errRespr := s.srv.Post(req.Context(), resourceGroupNameParam, networkSecurityPerimeterNameParam, associationNameParam, body, nil)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
	respContent := server.GetResponseContent(respr)
