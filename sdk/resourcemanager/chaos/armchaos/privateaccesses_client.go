...
// BeginPatch - Patch a private access tags
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2024-03-22-preview
//   - resourceGroupName - String that represents an Azure resource group.
...
//   - options - PrivateAccessesClientBeginPatchOptions contains the optional parameters for the PrivateAccessesClient.BeginPatch
//     method.
func (client *PrivateAccessesClient) BeginPatch(ctx context.Context, resourceGroupName string, privateAccessName string, privateAccessPatch PrivateAccessPatch, options *PrivateAccessesClientBeginPatchOptions) (*runtime.Poller[PrivateAccessesClientPatchResponse], error) {
	if options == nil || options.ResumeToken == "" {
		resp, err := client.patch(ctx, resourceGroupName, privateAccessName, privateAccessPatch, options)
		if err != nil {
			return nil, err
		}
		poller, err := runtime.NewPoller(resp, client.internal.Pipeline(), &runtime.NewPollerOptions[PrivateAccessesClientPatchResponse]{
			FinalStateVia: runtime.FinalStateViaLocation,
			Tracer:        client.internal.Tracer(),
		})
		return poller, err
	} else {
		return runtime.NewPollerFromResumeToken(options.ResumeToken, client.internal.Pipeline(), &runtime.NewPollerFromResumeTokenOptions[PrivateAccessesClientPatchResponse]{
			Tracer: client.internal.Tracer(),
		})
	}
}

// Patch - Patch a private access tags
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2024-03-22-preview
func (client *PrivateAccessesClient) patch(ctx context.Context, resourceGroupName string, privateAccessName string, privateAccessPatch PrivateAccessPatch, options *PrivateAccessesClientBeginPatchOptions) (*http.Response, error) {
	var err error
	const operationName = "PrivateAccessesClient.BeginPatch"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.patchCreateRequest(ctx, resourceGroupName, privateAccessName, privateAccessPatch, options)
	if err != nil {
		return nil, err
	}
	httpResp, err := client.internal.Pipeline().Do(req)
...
func (client *PrivateAccessesClient) patchCreateRequest(ctx context.Context, resourceGroupName string, privateAccessName string, privateAccessPatch PrivateAccessPatch, options *PrivateAccessesClientBeginPatchOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Chaos/privateAccesses/{privateAccessName}"
	if client.subscriptionID == "" {
		return nil, errors.New("parameter client.subscriptionID cannot be empty")
	}
