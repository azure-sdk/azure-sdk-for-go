...
func NewGetUsagesInLocationClient(subscriptionID string, credential azcore.TokenCredential, options *arm.ClientOptions) (*GetUsagesInLocationClient, error) {
	cl, err := arm.NewClient(moduleName+".GetUsagesInLocationClient", moduleVersion, credential, options)
	if err != nil {
		return nil, err
	}
...
//   - quotaType - The type of quota to get usages for
...
//     method.
func (client *GetUsagesInLocationClient) NewListPager(location string, quotaType string, options *GetUsagesInLocationClientListOptions) *runtime.Pager[GetUsagesInLocationClientListResponse] {
	return runtime.NewPager(runtime.PagingHandler[GetUsagesInLocationClientListResponse]{
		...
		Fetcher: func(ctx context.Context, page *GetUsagesInLocationClientListResponse) (GetUsagesInLocationClientListResponse, error) {
			...
			var err error
			if page == nil {
				req, err = client.listCreateRequest(ctx, location, quotaType, options)
			} else {
...
func (client *GetUsagesInLocationClient) listCreateRequest(ctx context.Context, location string, quotaType string, options *GetUsagesInLocationClientListOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/providers/Microsoft.Web/locations/{location}/quotaType/{quotaType}/usages"
	if location == "" {
		return nil, errors.New("parameter location cannot be empty")
	}
	urlPath = strings.ReplaceAll(urlPath, "{location}", url.PathEscape(location))
	if quotaType == "" {
		return nil, errors.New("parameter quotaType cannot be empty")
	}
	urlPath = strings.ReplaceAll(urlPath, "{quotaType}", url.PathEscape(quotaType))
