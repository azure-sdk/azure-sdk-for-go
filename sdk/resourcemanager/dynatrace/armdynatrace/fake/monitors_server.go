...
type MonitorsServer struct {
	...
	// UpdateAgentStatus is the fake for method MonitorsClient.UpdateAgentStatus
	// HTTP status codes to indicate success: http.StatusNoContent
	UpdateAgentStatus func(ctx context.Context, resourceGroupName string, monitorName string, request armdynatrace.AgentStatusRequest, options *armdynatrace.MonitorsClientUpdateAgentStatusOptions) (resp azfake.Responder[armdynatrace.MonitorsClientUpdateAgentStatusResponse], errResp azfake.ErrorResponder)
...
func (m *MonitorsServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	...
	go func() {
		...
		if !intercepted {
			...
			case "MonitorsClient.UpdateAgentStatus":
				res.resp, res.err = m.dispatchUpdateAgentStatus(req)
...
func (m *MonitorsServerTransport) dispatchUpdateAgentStatus(req *http.Request) (*http.Response, error) {
	if m.srv.UpdateAgentStatus == nil {
		return nil, &nonRetriableError{errors.New("fake for method UpdateAgentStatus not implemented")}
	}
	const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Dynatrace\.Observability/monitors/(?P<monitorName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/updateAgentStatus`
	regex := regexp.MustCompile(regexStr)
	matches := regex.FindStringSubmatch(req.URL.EscapedPath())
	if matches == nil || len(matches) < 3 {
		return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
	}
	body, err := server.UnmarshalRequestAsJSON[armdynatrace.AgentStatusRequest](req)
	if err != nil {
		return nil, err
	}
	resourceGroupNameParam, err := url.PathUnescape(matches[regex.SubexpIndex("resourceGroupName")])
	...
	respr, errRespr := m.srv.UpdateAgentStatus(req.Context(), resourceGroupNameParam, monitorNameParam, body, nil)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
	respContent := server.GetResponseContent(respr)
