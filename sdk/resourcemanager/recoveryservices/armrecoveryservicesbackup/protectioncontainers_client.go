...
// BeginUnregister - Unregisters the given container from your Recovery Services Vault. This is an asynchronous operation.
// To determine whether the backend service has finished processing the request, call Get Container
...
//   - options - ProtectionContainersClientBeginUnregisterOptions contains the optional parameters for the ProtectionContainersClient.BeginUnregister
//     method.
func (client *ProtectionContainersClient) BeginUnregister(ctx context.Context, vaultName string, resourceGroupName string, fabricName string, containerName string, options *ProtectionContainersClientBeginUnregisterOptions) (*runtime.Poller[ProtectionContainersClientUnregisterResponse], error) {
	if options == nil || options.ResumeToken == "" {
		resp, err := client.unregister(ctx, vaultName, resourceGroupName, fabricName, containerName, options)
		if err != nil {
			return nil, err
		}
		poller, err := runtime.NewPoller(resp, client.internal.Pipeline(), &runtime.NewPollerOptions[ProtectionContainersClientUnregisterResponse]{
			FinalStateVia: runtime.FinalStateViaAzureAsyncOp,
			Tracer:        client.internal.Tracer(),
		})
		return poller, err
	} else {
		return runtime.NewPollerFromResumeToken(options.ResumeToken, client.internal.Pipeline(), &runtime.NewPollerFromResumeTokenOptions[ProtectionContainersClientUnregisterResponse]{
			Tracer: client.internal.Tracer(),
		})
	}
}
...
//
// Generated from API version 2023-06-01
func (client *ProtectionContainersClient) unregister(ctx context.Context, vaultName string, resourceGroupName string, fabricName string, containerName string, options *ProtectionContainersClientBeginUnregisterOptions) (*http.Response, error) {
	var err error
	const operationName = "ProtectionContainersClient.BeginUnregister"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
...
func (client *ProtectionContainersClient) unregisterCreateRequest(ctx context.Context, vaultName string, resourceGroupName string, fabricName string, containerName string, options *ProtectionContainersClientBeginUnregisterOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.RecoveryServices/vaults/{vaultName}/backupFabrics/{fabricName}/protectionContainers/{containerName}"
	if vaultName == "" {
		return nil, errors.New("parameter vaultName cannot be empty")
	}
