...
func (m *MsixImagesServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	...
	go func() {
		...
		var res result
		if msixImagesServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = msixImagesServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
			case "MsixImagesClient.NewExpandPager":
				res.resp, res.err = m.dispatchNewExpandPager(req)
			default:
...
// set this to conditionally intercept incoming requests to MsixImagesServerTransport
var msixImagesServerTransportInterceptor interface {
