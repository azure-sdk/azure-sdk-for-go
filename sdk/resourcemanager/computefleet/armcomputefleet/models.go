...
type BaseVirtualMachineProfile struct {
	...
	// creating/updating the VMSS VM Profile with minimum api-version 2023-09-01. Examples: "2024-07-01T00:00:01.1234567+00:00"
...
type ComputeProfile struct {
	// REQUIRED; Base Virtual Machine Profile Properties to be specified according to
	...
	// The default value will be the latest supported computeApiVersion by Compute
	// Fleet.
	...
	// within an Azure datacenter. VMs in the same fault domain share a common
	// power source and network switch. If not specified, defaults to 1, which represents "Max Spreading" (using as many fault
	// domains as possible). This property cannot be updated.
...
type DiffDiskSettings struct {
	...
	// ResourceDiskis used. Refer to the VM size documentation for Windows VM at https://docs.microsoft.com/azure/virtual-machines/windows/sizes
	// and Linux VM at
...
type OSImageNotificationProfile struct {
	...
	// is specified in ISO 8601 format, and the value must not exceed 15 minutes (PT15M)
...
type SpotPriorityProfile struct {
	...
	// is enabled, AzureFleetRP will use all VM sizes in vmSizesProfile to create new
	// VMs (if VMs are evicted deleted) or update existing VMs with new VM sizes (if VMs are evicted deallocated or failed to
	// allocate due to capacity constraint) in order to achieve the desired capacity.
...
type VMSizeProfile struct {
	// REQUIRED; The Sku name (e.g. 'StandardDS1v2')
	Name *string
...
type VMSizeProperties struct {
	...
	// List all available virtual machine sizes in a region [https://docs.microsoft.com/en-us/rest/api/compute/resource-skus/list].Setting
...
type VirtualMachineScaleSet struct {
	// READ-ONLY; The compute RP resource id of the virtualMachineScaleSet "subscriptions/{subId}/resourceGroups/{rgName}/providers/Microsoft.Compute/virtualMachineScaleSets/{vmssName}"
	ID *string
...
type VirtualMachineScaleSetOSProfile struct {
	...
	// Disallowed values:"administrator", "admin", "user", "user1", "test", "user2", "test1", "user3", "admin1", "1", "123", "a",
...
type WindowsConfiguration struct {
	...
	// Specifies the time zone of the virtual machine. e.g. "Pacific Standard Time". Possible values can beTimeZoneInfo.Id
	// [https://docs.microsoft.com/dotnet/api/system.timezoneinfo.id?#System_TimeZoneInfo_Id]value from time zones returned byTimeZoneInfo.GetSystemTimeZones
