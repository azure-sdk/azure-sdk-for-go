...
// MarshalJSON implements the json.Marshaller interface for type MachineRunCommandInstanceViewStatus.
func (m MachineRunCommandInstanceViewStatus) MarshalJSON() ([]byte, error) {
	objectMap := make(map[string]any)
	populate(objectMap, "code", m.Code)
	populate(objectMap, "displayStatus", m.DisplayStatus)
	populate(objectMap, "level", m.Level)
...
// UnmarshalJSON implements the json.Unmarshaller interface for type MachineRunCommandInstanceViewStatus.
func (m *MachineRunCommandInstanceViewStatus) UnmarshalJSON(data []byte) error {
	var rawMsg map[string]json.RawMessage
	if err := json.Unmarshal(data, &rawMsg); err != nil {
		return fmt.Errorf("unmarshalling type %T: %v", m, err)
	}
