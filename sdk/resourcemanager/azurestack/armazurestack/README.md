...
Azure Azurestack module consists of one or more clients. We provide a client factory which could be used to create any client in this module.

```go
clientFactory, err := armazurestack.NewClientFactory(<subscription ID>, cred, nil)
```
...
clientFactory, err := armazurestack.NewClientFactory(<subscription ID>, cred, &options)
```
...
```go
client := clientFactory.NewCustomerSubscriptionsClient()
```
