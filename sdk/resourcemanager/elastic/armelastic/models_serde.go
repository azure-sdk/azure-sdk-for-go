...
func (m MonitorResourceUpdateParameters) MarshalJSON() ([]byte, error) {
	objectMap := make(map[string]any)
	populate(objectMap, "resubscribeProperties", m.ResubscribeProperties)
	populate(objectMap, "tags", m.Tags)
	return json.Marshal(objectMap)
}
...
func (m *MonitorResourceUpdateParameters) UnmarshalJSON(data []byte) error {
	...
	for key, val := range rawMsg {
		var err error
		switch key {
		case "resubscribeProperties":
			err = unpopulate(val, "ResubscribeProperties", &m.ResubscribeProperties)
			delete(rawMsg, key)
		case "tags":
			err = unpopulate(val, "Tags", &m.Tags)
			delete(rawMsg, key)
