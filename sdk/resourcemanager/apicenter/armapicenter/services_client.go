...
//   - resourceGroupName - The name of the resource group. The name is case insensitive.
//   - serviceName - Service name
//   - assignedTo - An entity the metadata schema is requested for.
...
//     method.
func (client *ServicesClient) ExportMetadataSchema(ctx context.Context, resourceGroupName string, serviceName string, assignedTo MetadataAssignmentEntity, options *ServicesClientExportMetadataSchemaOptions) (ServicesClientExportMetadataSchemaResponse, error) {
	var err error
	req, err := client.exportMetadataSchemaCreateRequest(ctx, resourceGroupName, serviceName, assignedTo, options)
	if err != nil {
	...
	resp, err := client.exportMetadataSchemaHandleResponse(httpResp)
	return resp, err
}

// exportMetadataSchemaCreateRequest creates the ExportMetadataSchema request.
func (client *ServicesClient) exportMetadataSchemaCreateRequest(ctx context.Context, resourceGroupName string, serviceName string, assignedTo MetadataAssignmentEntity, options *ServicesClientExportMetadataSchemaOptions) (*policy.Request, error) {
	...
	reqQP := req.Raw().URL.Query()
	reqQP.Set("api-version", "2024-03-01")
	reqQP.Set("assignedTo", string(assignedTo))
	req.Raw().URL.RawQuery = reqQP.Encode()
	req.Raw().Header["Accept"] = []string{"application/json"}
	return req, nil
}
