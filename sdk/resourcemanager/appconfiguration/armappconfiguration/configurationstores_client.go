...
// ResetSasTokens - Reset all SAS tokens of a given kind (primary/secondary).
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2024-09-01-preview
//   - resourceGroupName - The name of the resource group to which the container registry belongs.
//   - configStoreName - The name of the configuration store.
//   - resetSasTokensParameters - The object containing information for the SAS token reset request.
//   - options - ConfigurationStoresClientResetSasTokensOptions contains the optional parameters for the ConfigurationStoresClient.ResetSasTokens
//     method.
func (client *ConfigurationStoresClient) ResetSasTokens(ctx context.Context, resourceGroupName string, configStoreName string, resetSasTokensParameters ResetSasTokensParameters, options *ConfigurationStoresClientResetSasTokensOptions) (ConfigurationStoresClientResetSasTokensResponse, error) {
	var err error
	const operationName = "ConfigurationStoresClient.ResetSasTokens"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.resetSasTokensCreateRequest(ctx, resourceGroupName, configStoreName, resetSasTokensParameters, options)
	if err != nil {
		return ConfigurationStoresClientResetSasTokensResponse{}, err
	}
	httpResp, err := client.internal.Pipeline().Do(req)
	if err != nil {
		return ConfigurationStoresClientResetSasTokensResponse{}, err
	...
	return ConfigurationStoresClientResetSasTokensResponse{}, nil
}

// resetSasTokensCreateRequest creates the ResetSasTokens request.
func (client *ConfigurationStoresClient) resetSasTokensCreateRequest(ctx context.Context, resourceGroupName string, configStoreName string, resetSasTokensParameters ResetSasTokensParameters, options *ConfigurationStoresClientResetSasTokensOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.AppConfiguration/configurationStores/{configStoreName}/resetSasTokens"
	if client.subscriptionID == "" {
		return nil, errors.New("parameter client.subscriptionID cannot be empty")
	}
	urlPath = strings.ReplaceAll(urlPath, "{subscriptionId}", url.PathEscape(client.subscriptionID))
	...
	req.Raw().URL.RawQuery = reqQP.Encode()
	req.Raw().Header["Accept"] = []string{"application/json"}
	if err := runtime.MarshalAsJSON(req, resetSasTokensParameters); err != nil {
		return nil, err
	}
	return req, nil
}
