...
//   - volumeGroupName - The name of the VolumeGroup.
//   - volumeName - The name of the Volume.
//   - options - VolumesClientBeginRestoreVolumeOptions contains the optional parameters for the VolumesClient.BeginRestoreVolume
//     method.
func (client *VolumesClient) BeginRestoreVolume(ctx context.Context, resourceGroupName string, elasticSanName string, volumeGroupName string, volumeName string, options *VolumesClientBeginRestoreVolumeOptions) (*runtime.Poller[VolumesClientRestoreVolumeResponse], error) {
	if options == nil || options.ResumeToken == "" {
		resp, err := client.restoreVolume(ctx, resourceGroupName, elasticSanName, volumeGroupName, volumeName, options)
		if err != nil {
			return nil, err
		}
		poller, err := runtime.NewPoller(resp, client.internal.Pipeline(), &runtime.NewPollerOptions[VolumesClientRestoreVolumeResponse]{
			Tracer: client.internal.Tracer(),
		})
		return poller, err
	} else {
		return runtime.NewPollerFromResumeToken(options.ResumeToken, client.internal.Pipeline(), &runtime.NewPollerFromResumeTokenOptions[VolumesClientRestoreVolumeResponse]{
			Tracer: client.internal.Tracer(),
		})
	}
}
...
//
// Generated from API version 2024-07-01-preview
func (client *VolumesClient) restoreVolume(ctx context.Context, resourceGroupName string, elasticSanName string, volumeGroupName string, volumeName string, options *VolumesClientBeginRestoreVolumeOptions) (*http.Response, error) {
	var err error
	const operationName = "VolumesClient.BeginRestoreVolume"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
...
func (client *VolumesClient) restoreVolumeCreateRequest(ctx context.Context, resourceGroupName string, elasticSanName string, volumeGroupName string, volumeName string, _ *VolumesClientBeginRestoreVolumeOptions) (*policy.Request, error) {
