...
type ServersServer struct {
	...
	// EstimateRestoreSnapshot is the fake for method ServersClient.EstimateRestoreSnapshot
	// HTTP status codes to indicate success: http.StatusOK
	EstimateRestoreSnapshot func(ctx context.Context, resourceGroupName string, serverName string, parameters armmysqlflexibleservers.ServerForEstimateRestoreSnapshotParameter, options *armmysqlflexibleservers.ServersClientEstimateRestoreSnapshotOptions) (resp azfake.Responder[armmysqlflexibleservers.ServersClientEstimateRestoreSnapshotResponse], errResp azfake.ErrorResponder)
...
func (s *ServersServerTransport) Do(req *http.Request) (*http.Response, error) {
	...
	case "ServersClient.EstimateRestoreSnapshot":
		resp, err = s.dispatchEstimateRestoreSnapshot(req)
...
func (s *ServersServerTransport) dispatchEstimateRestoreSnapshot(req *http.Request) (*http.Response, error) {
	if s.srv.EstimateRestoreSnapshot == nil {
		return nil, &nonRetriableError{errors.New("fake for method EstimateRestoreSnapshot not implemented")}
	}
	const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.DBforMySQL/flexibleServers/(?P<serverName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/estimateRestoreSnapshot`
	regex := regexp.MustCompile(regexStr)
	matches := regex.FindStringSubmatch(req.URL.EscapedPath())
	if matches == nil || len(matches) < 3 {
		return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
	}
	body, err := server.UnmarshalRequestAsJSON[armmysqlflexibleservers.ServerForEstimateRestoreSnapshotParameter](req)
	if err != nil {
		return nil, err
	}
	resourceGroupNameParam, err := url.PathUnescape(matches[regex.SubexpIndex("resourceGroupName")])
	...
	respr, errRespr := s.srv.EstimateRestoreSnapshot(req.Context(), resourceGroupNameParam, serverNameParam, body, nil)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
	...
	resp, err := server.MarshalResponseAsJSON(respContent, server.GetResponse(respr).ServerForEstimateRestoreSnapshotResponse, req)
	if err != nil {
		return nil, err
	}
	return resp, nil
