...
// AnalyticsSection - Analytics overview section data
type AnalyticsSection struct {
	// User alert rules aggregated by rule status: 'Enabled', 'Disabled' or 'Auto-disabled'
	Data *AnalyticsSectionData `json:"data,omitempty"`

	// The section kind
	SectionKind *OverviewSectionKind `json:"sectionKind,omitempty"`

	// READ-ONLY; The last time this data was calculated
	DataQueryTime *string `json:"dataQueryTime,omitempty" azure:"ro"`

	// READ-ONLY; Indicates if the data came from cache or not
	IsFromCache *bool `json:"isFromCache,omitempty" azure:"ro"`
}

// AnalyticsSectionData - User alert rules aggregated by rule status: 'Enabled', 'Disabled' or 'Auto-disabled'
type AnalyticsSectionData struct {
	// Dictionary of
	AlertRulesByStatus map[string]*string `json:"alertRulesByStatus,omitempty"`
}
...
// AutomationSection - Automation overview section data
type AutomationSection struct {
	Data *AutomationSectionData `json:"data,omitempty"`
...
type AutomationSectionData struct {
	// Aggregation of all actions performed by the user automation rules
	ActionsPerformedByAutomation map[string]*string `json:"actionsPerformedByAutomation,omitempty"`

	// The amount of active automation rules the user have
	ActiveAutomationRules *string `json:"activeAutomationRules,omitempty"`

	// The total amount of incidents that were closed by automation rule
	ClosedIncidentsByAutomation *string `json:"closedIncidentsByAutomation,omitempty"`

	// The mean time it took to close an incident via automation rule vs the mean time without
	TimeSavedByAutomation map[string]*string `json:"timeSavedByAutomation,omitempty"`
}
...
// DataConnectorsSection - Data connectors overview section data
type DataConnectorsSection struct {
	Data *DataConnectorsSectionData `json:"data,omitempty"`
...
type DataConnectorsSectionData struct {
	// A dictionary of all active connectors
	ActiveConnectors map[string]*string `json:"activeConnectors,omitempty"`

	// A series of anomaly ingested events
	AnomaliesVolume []*string `json:"anomaliesVolume,omitempty"`

	// Aggregation of all threat intelligence ingested events by type: 'URL', 'IP', 'Email', 'Other', …
	TiByType map[string]*string `json:"tiByType,omitempty"`

	// A series of all ingested events
	TotalVolume []*string `json:"totalVolume,omitempty"`

	// A dictionary of all unhealthy connectors
	UnhealthyConnectors map[string]*string `json:"unhealthyConnectors,omitempty"`
}
...
// IncidentsSection - Incidents overview section data
type IncidentsSection struct {
	Data *IncidentsSectionData `json:"data,omitempty"`
...
type IncidentsSectionData struct {
	// Aggregation of all closed incidents by closed classification reason: 'TruePositive', 'FalsePositive', 'BenignPositive',
	// 'Undetermined', …
	ClosedIncidentsClassification map[string]*string `json:"closedIncidentsClassification,omitempty"`

	// Aggregation of all incidents by severity: 'High', 'Medium', 'Low', 'Informational', …
	IncidentsBySeverity map[string]*string `json:"incidentsBySeverity,omitempty"`

	// Aggregation of all incidents by status: 'New', 'Active', 'Closed'
	IncidentsByStatus map[string]*string `json:"incidentsByStatus,omitempty"`

	// A series of incidents by status
	IncidentsStatusByCreationTime []map[string]*string `json:"incidentsStatusByCreationTime,omitempty"`

	// The mean time it took to acknowledge an incident
	MeanTimeToAck *MeanTimeToAction `json:"meanTimeToAck,omitempty"`

	// The mean time it took to close an incident
	MeanTimeToClose *MeanTimeToAction `json:"meanTimeToClose,omitempty"`
}
...
type MeanTimeToAction struct {
	// The diff between the mean time in current period of time to the previous one
...
// OverviewClientListOptions contains the optional parameters for the OverviewClient.List method.
type OverviewClientListOptions struct {
	// The section name. Supports - Analytics, Incidents, DataConnectors, Automation
	Sections *string
}

// OverviewSection - A specific overview section.
