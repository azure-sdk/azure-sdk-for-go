...
type AFDEndpointProperties struct {
	...
	// A reference to the origin group. The dedicated vip is created as a separate resource. The dedicated vip will be linked
	// to an endpoint for L4 profiles
...
type AFDOriginGroupProperties struct {
	...
	// The type of L7 or L4 route to use for this origin group. Valid options are 'L7Route' or 'L4Route'. This will help validate
	// L4 origin groups with L4 routes
	RouteType *RouteType
...
type AFDOriginProperties struct {
	...
	// Preserves the client side source IP allowing the back-end to see the IP address of the client. This can then be used by
	// applications for applying any rules. Valid options are 'Enabled' or 'Disabled'
	...
	// The ports used for the TCP/UDP connection to the customer's backend. Must be between 1024 and 65535 excluding reserved
	// ports. The ports can be a comma separated list or a valid range like 5050-5090
	OriginPorts *string
...
type HealthProbeParameters struct {
	...
	// The valid range of ports for health probes for TCP/UDP would typically be from 1024-65535. This range excludes the reserved
	// ports and allows for a wide range of available ports to use for health
	// probes
	ProbePort *int32
...
// L4Route - Friendly Routes name mapping to the any Routes or secret related information.
type L4Route struct {
	// The JSON object that contains the properties of the Routes to create.
	Properties *L4RouteProperties
...
// L4RouteBaseSettings - Base class for all L4 route settings.
type L4RouteBaseSettings struct {
	...
	// The ports used for the TCP/UDP connection. Must be between 1024 and 65535 excluding reserved ports. The ports can be a
	// comma separated list or a valid range like 5050-5090
	Ports *string
}

// L4RouteListResult - Result of the request to list L4 routes. It contains a list of L4 route objects and a URL link to get
// the next set of results.
type L4RouteListResult struct {
	// URL to get the next set of route objects if there are any.
	NextLink *string

	// READ-ONLY; List of AzureFrontDoor L4 routes within a profile.
	Value []*L4Route
}

// L4RouteProperties - The JSON object that contains the properties of the Routes to create.
type L4RouteProperties struct {
	...
	// Routing method for TCP/UDP routes based on SNI or Ports division, if it is SNI based then the we can read SNI header to
	// get the origin name or if its based on the ports then we can have port division
	// to connect to specific origin for example port range 2000-2050 belongs to origin group 1 and port range 3000-3050 belongs
	// to origin group 2.
...
// L4RouteUpdatePropertiesParameters - L4 Route.
type L4RouteUpdatePropertiesParameters struct {
	// Domains referenced by this endpoint.
	CustomDomains []*ActivatedResourceReference
...
// L4RoutesClientBeginCreateOptions contains the optional parameters for the L4RoutesClient.BeginCreate method.
type L4RoutesClientBeginCreateOptions struct {
	// Resumes the LRO from the provided token.
	ResumeToken string
}

// L4RoutesClientBeginDeleteOptions contains the optional parameters for the L4RoutesClient.BeginDelete method.
type L4RoutesClientBeginDeleteOptions struct {
	// Resumes the LRO from the provided token.
	ResumeToken string
}

// L4RoutesClientBeginUpdateOptions contains the optional parameters for the L4RoutesClient.BeginUpdate method.
type L4RoutesClientBeginUpdateOptions struct {
	// Resumes the LRO from the provided token.
	ResumeToken string
}

// L4RoutesClientGetOptions contains the optional parameters for the L4RoutesClient.Get method.
type L4RoutesClientGetOptions struct {
	// placeholder for future optional parameters
}

// L4RoutesClientListByEndpointOptions contains the optional parameters for the L4RoutesClient.NewListByEndpointPager method.
type L4RoutesClientListByEndpointOptions struct {
	// placeholder for future optional parameters
}
