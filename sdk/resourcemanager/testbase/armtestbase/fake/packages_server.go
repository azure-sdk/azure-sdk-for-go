...
type PackagesServer struct {
	...
	// RunTest is the fake for method PackagesClient.RunTest
	// HTTP status codes to indicate success: http.StatusOK, http.StatusAccepted, http.StatusNoContent
	RunTest func(ctx context.Context, resourceGroupName string, testBaseAccountName string, packageName string, options *armtestbase.PackagesClientRunTestOptions) (resp azfake.Responder[armtestbase.PackagesClientRunTestResponse], errResp azfake.ErrorResponder)
...
func (p *PackagesServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	...
	go func() {
		...
		var res result
		if packagesServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = packagesServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
			...
			case "PackagesClient.GetDownloadURL":
				res.resp, res.err = p.dispatchGetDownloadURL(req)
			case "PackagesClient.BeginHardDelete":
				res.resp, res.err = p.dispatchBeginHardDelete(req)
			case "PackagesClient.NewListByTestBaseAccountPager":
				res.resp, res.err = p.dispatchNewListByTestBaseAccountPager(req)
			case "PackagesClient.RunTest":
				res.resp, res.err = p.dispatchRunTest(req)
...
func (p *PackagesServerTransport) dispatchRunTest(req *http.Request) (*http.Response, error) {
	if p.srv.RunTest == nil {
		return nil, &nonRetriableError{errors.New("fake for method RunTest not implemented")}
	}
	const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.TestBase/testBaseAccounts/(?P<testBaseAccountName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/packages/(?P<packageName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/runTest`
	regex := regexp.MustCompile(regexStr)
	matches := regex.FindStringSubmatch(req.URL.EscapedPath())
	if matches == nil || len(matches) < 4 {
		return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
	}
	body, err := server.UnmarshalRequestAsJSON[armtestbase.PackageRunTestParameters](req)
	if err != nil {
		return nil, err
	}
	resourceGroupNameParam, err := url.PathUnescape(matches[regex.SubexpIndex("resourceGroupName")])
	...
	var options *armtestbase.PackagesClientRunTestOptions
	if !reflect.ValueOf(body).IsZero() {
		options = &armtestbase.PackagesClientRunTestOptions{
			Parameters: &body,
		}
	}
	respr, errRespr := p.srv.RunTest(req.Context(), resourceGroupNameParam, testBaseAccountNameParam, packageNameParam, options)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
	respContent := server.GetResponseContent(respr)
...
// set this to conditionally intercept incoming requests to PackagesServerTransport
var packagesServerTransportInterceptor interface {
