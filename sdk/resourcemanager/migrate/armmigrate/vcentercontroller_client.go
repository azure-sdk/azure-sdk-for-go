...
//   - siteName - Site name
//   - vcenterName - VCenters name
//   - options - VcenterControllerClientBeginDeleteOptions contains the optional parameters for the VcenterControllerClient.BeginDelete
//     method.
func (client *VcenterControllerClient) BeginDelete(ctx context.Context, resourceGroupName string, siteName string, vcenterName string, options *VcenterControllerClientBeginDeleteOptions) (*runtime.Poller[VcenterControllerClientDeleteResponse], error) {
	if options == nil || options.ResumeToken == "" {
		resp, err := client.deleteOperation(ctx, resourceGroupName, siteName, vcenterName, options)
		if err != nil {
			return nil, err
		}
		poller, err := runtime.NewPoller(resp, client.internal.Pipeline(), &runtime.NewPollerOptions[VcenterControllerClientDeleteResponse]{
			FinalStateVia: runtime.FinalStateViaLocation,
			Tracer:        client.internal.Tracer(),
		})
		return poller, err
	} else {
		return runtime.NewPollerFromResumeToken(options.ResumeToken, client.internal.Pipeline(), &runtime.NewPollerFromResumeTokenOptions[VcenterControllerClientDeleteResponse]{
			Tracer: client.internal.Tracer(),
		})
	}
}
...
//
// Generated from API version 2024-05-01-preview
func (client *VcenterControllerClient) deleteOperation(ctx context.Context, resourceGroupName string, siteName string, vcenterName string, options *VcenterControllerClientBeginDeleteOptions) (*http.Response, error) {
	var err error
	const operationName = "VcenterControllerClient.BeginDelete"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
...
func (client *VcenterControllerClient) deleteCreateRequest(ctx context.Context, resourceGroupName string, siteName string, vcenterName string, options *VcenterControllerClientBeginDeleteOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.OffAzure/vmwareSites/{siteName}/vcenters/{vcenterName}"
	if client.subscriptionID == "" {
		return nil, errors.New("parameter client.subscriptionID cannot be empty")
	}
