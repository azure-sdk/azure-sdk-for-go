//go:build go1.18
// +build go1.18

// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License. See License.txt in the project root for license information.
// Code generated by Microsoft (R) AutoRest Code Generator. DO NOT EDIT.
// Changes may cause incorrect behavior and will be lost if the code is regenerated.

package armdevopsinfrastructure

import "encoding/json"

func unmarshalAgentProfileClassification(rawMsg json.RawMessage) (AgentProfileClassification, error) {
	if rawMsg == nil {
		return nil, nil
	}
	var m map[string]any
	if err := json.Unmarshal(rawMsg, &m); err != nil {
		return nil, err
	}
	var b AgentProfileClassification
	switch m["kind"] {
	case "Stateful":
		b = &Stateful{}
	case "Stateless":
		b = &StatelessAgentProfile{}
	default:
		b = &AgentProfile{}
	}
	if err := json.Unmarshal(rawMsg, b); err != nil {
		return nil, err
	}
	return b, nil
}

func unmarshalAzureDevOpsPermissionProfileClassification(rawMsg json.RawMessage) (AzureDevOpsPermissionProfileClassification, error) {
	if rawMsg == nil {
		return nil, nil
	}
	var m map[string]any
	if err := json.Unmarshal(rawMsg, &m); err != nil {
		return nil, err
	}
	var b AzureDevOpsPermissionProfileClassification
	switch m["kind"] {
	case "CreatorOnly":
		b = &CreatorOnly{}
	case "Inherit":
		b = &InheritPermissions{}
	case "SpecificAccounts":
		b = &SpecificAccounts{}
	default:
		b = &AzureDevOpsPermissionProfile{}
	}
	if err := json.Unmarshal(rawMsg, b); err != nil {
		return nil, err
	}
	return b, nil
}

func unmarshalFabricProfileClassification(rawMsg json.RawMessage) (FabricProfileClassification, error) {
	if rawMsg == nil {
		return nil, nil
	}
	var m map[string]any
	if err := json.Unmarshal(rawMsg, &m); err != nil {
		return nil, err
	}
	var b FabricProfileClassification
	switch m["kind"] {
	case "Vmss":
		b = &VmssFabricProfile{}
	default:
		b = &FabricProfile{}
	}
	if err := json.Unmarshal(rawMsg, b); err != nil {
		return nil, err
	}
	return b, nil
}

func unmarshalLogonTypeClassification(rawMsg json.RawMessage) (LogonTypeClassification, error) {
	if rawMsg == nil {
		return nil, nil
	}
	var m map[string]any
	if err := json.Unmarshal(rawMsg, &m); err != nil {
		return nil, err
	}
	var b LogonTypeClassification
	switch m["kind"] {
	case "Interactive":
		b = &Interactive{}
	case "Service":
		b = &Service{}
	default:
		b = &LogonType{}
	}
	if err := json.Unmarshal(rawMsg, b); err != nil {
		return nil, err
	}
	return b, nil
}

func unmarshalOrganizationProfileClassification(rawMsg json.RawMessage) (OrganizationProfileClassification, error) {
	if rawMsg == nil {
		return nil, nil
	}
	var m map[string]any
	if err := json.Unmarshal(rawMsg, &m); err != nil {
		return nil, err
	}
	var b OrganizationProfileClassification
	switch m["kind"] {
	case "AzureDevOps":
		b = &AzureDevOpsOrganizationProfile{}
	case "GitHub":
		b = &GitHubOrganizationProfile{}
	default:
		b = &OrganizationProfile{}
	}
	if err := json.Unmarshal(rawMsg, b); err != nil {
		return nil, err
	}
	return b, nil
}
