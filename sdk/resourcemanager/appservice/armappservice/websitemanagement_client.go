...
// ListAseRegions - Description for get a list of available ASE regions and its supported Skus.
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2023-12-01
...
//     method.
func (client *WebSiteManagementClient) ListAseRegions(ctx context.Context, options *WebSiteManagementClientListAseRegionsOptions) (WebSiteManagementClientListAseRegionsResponse, error) {
	var err error
	const operationName = "WebSiteManagementClient.ListAseRegions"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	...
	resp, err := client.listAseRegionsHandleResponse(httpResp)
	return resp, err
}
