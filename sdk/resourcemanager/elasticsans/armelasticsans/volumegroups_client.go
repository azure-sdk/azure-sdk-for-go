...
func (client *VolumeGroupsClient) deleteCreateRequest(ctx context.Context, resourceGroupName string, elasticSanName string, volumeGroupName string, options *VolumeGroupsClientBeginDeleteOptions) (*policy.Request, error) {
	...
	reqQP.Set("api-version", "2022-06-15-preview")
	req.Raw().URL.RawQuery = reqQP.Encode()
	if options != nil && options.ForceDeleteVolumesAndSnapshots != nil {
		req.Raw().Header["x-ms-delete-volumes-snapshots"] = []string{strconv.FormatBool(*options.ForceDeleteVolumesAndSnapshots)}
	}
	req.Raw().Header["Accept"] = []string{"application/json"}
	return req, nil
}
