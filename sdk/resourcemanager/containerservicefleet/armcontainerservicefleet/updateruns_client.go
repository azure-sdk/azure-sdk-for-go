...
//   - options - UpdateRunsClientSkipOptions contains the optional parameters for the UpdateRunsClient.Skip method.
func (client *UpdateRunsClient) Skip(ctx context.Context, resourceGroupName string, fleetName string, updateRunName string, body SkipProperties, options *UpdateRunsClientSkipOptions) (UpdateRunsClientSkipResponse, error) {
	var err error
	const operationName = "UpdateRunsClient.Skip"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.skipCreateRequest(ctx, resourceGroupName, fleetName, updateRunName, body, options)
	if err != nil {
		return UpdateRunsClientSkipResponse{}, err
	}
	httpResp, err := client.internal.Pipeline().Do(req)
	if err != nil {
		return UpdateRunsClientSkipResponse{}, err
	...
	resp, err := client.skipHandleResponse(httpResp)
	return resp, err
}

// skipCreateRequest creates the Skip request.
func (client *UpdateRunsClient) skipCreateRequest(ctx context.Context, resourceGroupName string, fleetName string, updateRunName string, body SkipProperties, options *UpdateRunsClientSkipOptions) (*policy.Request, error) {
...
// skipHandleResponse handles the Skip response.
func (client *UpdateRunsClient) skipHandleResponse(resp *http.Response) (UpdateRunsClientSkipResponse, error) {
	result := UpdateRunsClientSkipResponse{}
	if err := runtime.UnmarshalAsJSON(resp, &result.UpdateRun); err != nil {
		return UpdateRunsClientSkipResponse{}, err
	}
	return result, nil
