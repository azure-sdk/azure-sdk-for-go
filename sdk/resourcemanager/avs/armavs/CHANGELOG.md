...
- New function `*ProvisionedNetworksClient.NewListByPrivateCloudPager(string, string, *ProvisionedNetworksClientListByPrivateCloudOptions) *runtime.Pager[ProvisionedNetworksClientListByPrivateCloudResponse]`
...
- New field `DisplayName`, `VsanDatastoreName` in struct `ClusterProperties`
- New field `SKU` in struct `ClusterUpdate`
- New field `DisplayName`, `VsanDatastoreName` in struct `ClusterUpdateProperties`
...
- New field `DisplayName`, `VsanDatastoreName` in struct `ManagementCluster`
...
- New field `SystemData`, `Zones` in struct `PrivateCloud`
- New field `DNSZoneType`, `DisplayName`, `VirtualNetworkID` in struct `PrivateCloudProperties`
- New field `SKU` in struct `PrivateCloudUpdate`
- New field `DNSZoneType`, `DisplayName` in struct `PrivateCloudUpdateProperties`
