...
func (j *JoinRequestsServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	...
	go func() {
		...
		var res result
		if joinRequestsServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = joinRequestsServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
			case "JoinRequestsClient.Approve":
				res.resp, res.err = j.dispatchApprove(req)
			case "JoinRequestsClient.Deny":
				res.resp, res.err = j.dispatchDeny(req)
...
// set this to conditionally intercept incoming requests to JoinRequestsServerTransport
var joinRequestsServerTransportInterceptor interface {
