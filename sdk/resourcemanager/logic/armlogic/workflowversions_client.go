...
func (client *WorkflowVersionsClient) getCreateRequest(ctx context.Context, resourceGroupName string, workflowName string, versionID string, _ *WorkflowVersionsClientGetOptions) (*policy.Request, error) {
...
//   - options - WorkflowVersionsClientListCallbackURLOptions contains the optional parameters for the WorkflowVersionsClient.ListCallbackURL
//     method.
func (client *WorkflowVersionsClient) ListCallbackURL(ctx context.Context, resourceGroupName string, workflowName string, versionID string, triggerName string, options *WorkflowVersionsClientListCallbackURLOptions) (WorkflowVersionsClientListCallbackURLResponse, error) {
	var err error
	const operationName = "WorkflowVersionsClient.ListCallbackURL"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.listCallbackURLCreateRequest(ctx, resourceGroupName, workflowName, versionID, triggerName, options)
	if err != nil {
		return WorkflowVersionsClientListCallbackURLResponse{}, err
	}
	httpResp, err := client.internal.Pipeline().Do(req)
	if err != nil {
		return WorkflowVersionsClientListCallbackURLResponse{}, err
...
func (client *WorkflowVersionsClient) listCallbackURLCreateRequest(ctx context.Context, resourceGroupName string, workflowName string, versionID string, triggerName string, options *WorkflowVersionsClientListCallbackURLOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Logic/workflows/{workflowName}/versions/{versionId}/{triggerName}/listCallbackUrl"
	if client.subscriptionID == "" {
		return nil, errors.New("parameter client.subscriptionID cannot be empty")
	}
	urlPath = strings.ReplaceAll(urlPath, "{subscriptionId}", url.PathEscape(client.subscriptionID))
...
func (client *WorkflowVersionsClient) listCallbackURLHandleResponse(resp *http.Response) (WorkflowVersionsClientListCallbackURLResponse, error) {
	result := WorkflowVersionsClientListCallbackURLResponse{}
