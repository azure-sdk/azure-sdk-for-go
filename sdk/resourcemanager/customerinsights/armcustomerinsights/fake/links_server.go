...
func (l *LinksServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	...
	go func() {
		...
		var res result
		if linksServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = linksServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
			...
			case "LinksClient.NewListByHubPager":
				res.resp, res.err = l.dispatchNewListByHubPager(req)
			default:
...
// set this to conditionally intercept incoming requests to LinksServerTransport
var linksServerTransportInterceptor interface {
