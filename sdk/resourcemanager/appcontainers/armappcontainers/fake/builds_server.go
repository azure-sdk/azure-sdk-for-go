...
type BuildsServer struct {
	...
	// ListAuthToken is the fake for method BuildsClient.ListAuthToken
	// HTTP status codes to indicate success: http.StatusOK
	ListAuthToken func(ctx context.Context, resourceGroupName string, builderName string, buildName string, options *armappcontainers.BuildsClientListAuthTokenOptions) (resp azfake.Responder[armappcontainers.BuildsClientListAuthTokenResponse], errResp azfake.ErrorResponder)

	// NewListByBuilderResourcePager is the fake for method BuildsClient.NewListByBuilderResourcePager
	// HTTP status codes to indicate success: http.StatusOK
	NewListByBuilderResourcePager func(resourceGroupName string, builderName string, options *armappcontainers.BuildsClientListByBuilderResourceOptions) (resp azfake.PagerResponder[armappcontainers.BuildsClientListByBuilderResourceResponse])
}
...
func NewBuildsServerTransport(srv *BuildsServer) *BuildsServerTransport {
	return &BuildsServerTransport{
		...
		newListByBuilderResourcePager: newTracker[azfake.PagerResponder[armappcontainers.BuildsClientListByBuilderResourceResponse]](),
	}
}
...
type BuildsServerTransport struct {
	...
	newListByBuilderResourcePager *tracker[azfake.PagerResponder[armappcontainers.BuildsClientListByBuilderResourceResponse]]
}
...
func (b *BuildsServerTransport) Do(req *http.Request) (*http.Response, error) {
	...
	case "BuildsClient.ListAuthToken":
		resp, err = b.dispatchListAuthToken(req)
	case "BuildsClient.NewListByBuilderResourcePager":
		resp, err = b.dispatchNewListByBuilderResourcePager(req)
	default:
...
func (b *BuildsServerTransport) dispatchListAuthToken(req *http.Request) (*http.Response, error) {
	if b.srv.ListAuthToken == nil {
		return nil, &nonRetriableError{errors.New("fake for method ListAuthToken not implemented")}
	}
	...
	respr, errRespr := b.srv.ListAuthToken(req.Context(), resourceGroupNameParam, builderNameParam, buildNameParam, nil)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
	respContent := server.GetResponseContent(respr)
...
func (b *BuildsServerTransport) dispatchNewListByBuilderResourcePager(req *http.Request) (*http.Response, error) {
	if b.srv.NewListByBuilderResourcePager == nil {
		return nil, &nonRetriableError{errors.New("fake for method NewListByBuilderResourcePager not implemented")}
	}
	newListByBuilderResourcePager := b.newListByBuilderResourcePager.get(req)
	if newListByBuilderResourcePager == nil {
		...
		resp := b.srv.NewListByBuilderResourcePager(resourceGroupNameParam, builderNameParam, nil)
		newListByBuilderResourcePager = &resp
		b.newListByBuilderResourcePager.add(req, newListByBuilderResourcePager)
		server.PagerResponderInjectNextLinks(newListByBuilderResourcePager, req, func(page *armappcontainers.BuildsClientListByBuilderResourceResponse, createLink func() string) {
			page.NextLink = to.Ptr(createLink())
		})
	}
	resp, err := server.PagerResponderNext(newListByBuilderResourcePager, req)
	if err != nil {
		return nil, err
	}
	if !contains([]int{http.StatusOK}, resp.StatusCode) {
		b.newListByBuilderResourcePager.remove(req)
		return nil, &nonRetriableError{fmt.Errorf("unexpected status code %d. acceptable values are http.StatusOK", resp.StatusCode)}
	}
	if !server.PagerResponderMore(newListByBuilderResourcePager) {
		b.newListByBuilderResourcePager.remove(req)
	}
	return resp, nil
}
