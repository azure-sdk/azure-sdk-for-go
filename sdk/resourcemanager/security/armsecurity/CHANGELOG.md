...
- New field `AccountName`, `Regions` in struct `AwsEnvironmentData`
- New field `DataSensitivityDiscovery`, `DatabasesDspm` in struct `DefenderCspmAwsOffering`
...
- New field `ExcludedScopes`, `IncludeMemberScopes`, `Metadata`, `TenantID` in struct `GovernanceRuleProperties`
- New field `EnablementTime`, `Extensions` in struct `PricingProperties`
