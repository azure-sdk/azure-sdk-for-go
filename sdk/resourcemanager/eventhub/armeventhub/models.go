...
type MessageTimestampDescription struct {
	...
	// refers the time in which message got appended inside broker log. CreateTime
...
type RetentionDescription struct {
	...
	// The minimum time a message will remain ineligible for compaction in the log. This value is used when cleanupPolicy is Compact
	// or DeleteOrCompact.
	...
	// special case where retention time is infinite, but the size of an entity is
	// restricted and its size depends on namespace SKU type.
	...
	// or DeleteOrCompact. Consumer must complete reading the tombstone marker
	// within this specified amount of time if consumer begins from starting offset to ensure they get a valid snapshot for the
