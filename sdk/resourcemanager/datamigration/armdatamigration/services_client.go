...
// reply when successful with 200 OK or 201 Created. Long-running operations use the provisioningState property. Database
// Migration Service (classic) - SQL scenarios are on a deprecation path and will be
// retired on 15 March 2026 for all customers. Please migrate to Azure SQL database services by using the latest Azure Database
// Migration Service version which is available as an extension in Azure Data
...
func (client *ServicesClient) BeginStart(ctx context.Context, groupName string, serviceName string, options *ServicesClientBeginStartOptions) (*runtime.Poller[ServicesClientStartResponse], error) {
	if options == nil || options.ResumeToken == "" {
		...
		return runtime.NewPoller[ServicesClientStartResponse](resp, client.internal.Pipeline(), nil)
	} else {
...
func (client *ServicesClient) BeginStop(ctx context.Context, groupName string, serviceName string, options *ServicesClientBeginStopOptions) (*runtime.Poller[ServicesClientStopResponse], error) {
	if options == nil || options.ResumeToken == "" {
		...
		return runtime.NewPoller[ServicesClientStopResponse](resp, client.internal.Pipeline(), nil)
	} else {
...
// ("ServiceIsBusy"). Database Migration Service (classic) - SQL scenarios are
// on a deprecation path and will be retired on 15 March 2026 for all customers. Please migrate to Azure SQL database services
// by using the latest Azure Database Migration Service version which is
