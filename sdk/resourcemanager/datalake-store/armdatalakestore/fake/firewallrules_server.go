...
// The returned FirewallRulesServerTransport instance is connected to an instance of armdatalakestore.FirewallRulesClient via the
...
func NewFirewallRulesServerTransport(srv *FirewallRulesServer) *FirewallRulesServerTransport {
	return &FirewallRulesServerTransport{
		srv:                   srv,
		newListByAccountPager: newTracker[azfake.PagerResponder[armdatalakestore.FirewallRulesClientListByAccountResponse]](),
	}
}

// FirewallRulesServerTransport connects instances of armdatalakestore.FirewallRulesClient to instances of FirewallRulesServer.
...
type FirewallRulesServerTransport struct {
	...
	newListByAccountPager *tracker[azfake.PagerResponder[armdatalakestore.FirewallRulesClientListByAccountResponse]]
}
	...
	go func() {
		...
		if !intercepted {
			...
			case "FirewallRulesClient.NewListByAccountPager":
				res.resp, res.err = f.dispatchNewListByAccountPager(req)
...
func (f *FirewallRulesServerTransport) dispatchCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.DataLakeStore/accounts/(?P<accountName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/firewallRules/(?P<firewallRuleName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)`
	regex := regexp.MustCompile(regexStr)
	matches := regex.FindStringSubmatch(req.URL.EscapedPath())
	if matches == nil || len(matches) < 4 {
		return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
	}
	body, err := server.UnmarshalRequestAsJSON[armdatalakestore.CreateOrUpdateFirewallRuleParameters](req)
	if err != nil {
		return nil, err
	}
...
func (f *FirewallRulesServerTransport) dispatchNewListByAccountPager(req *http.Request) (*http.Response, error) {
	...
	if newListByAccountPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.DataLakeStore/accounts/(?P<accountName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/firewallRules`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		...
		server.PagerResponderInjectNextLinks(newListByAccountPager, req, func(page *armdatalakestore.FirewallRulesClientListByAccountResponse, createLink func() string) {
...
func (f *FirewallRulesServerTransport) dispatchUpdate(req *http.Request) (*http.Response, error) {
	...
	body, err := server.UnmarshalRequestAsJSON[armdatalakestore.UpdateFirewallRuleParameters](req)
	if err != nil {
		return nil, err
	}
	resourceGroupNameParam, err := url.PathUnescape(matches[regex.SubexpIndex("resourceGroupName")])
	...
	var options *armdatalakestore.FirewallRulesClientUpdateOptions
	if !reflect.ValueOf(body).IsZero() {
		options = &armdatalakestore.FirewallRulesClientUpdateOptions{
			Parameters: &body,
		}
	}
