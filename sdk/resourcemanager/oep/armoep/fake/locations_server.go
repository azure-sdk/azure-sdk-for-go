...
// The returned LocationsServerTransport instance is connected to an instance of armoep.LocationsClient via the
...
// LocationsServerTransport connects instances of armoep.LocationsClient to instances of LocationsServer.
...
func (l *LocationsServerTransport) dispatchCheckNameAvailability(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.OpenEnergyPlatform/checkNameAvailability`
	regex := regexp.MustCompile(regexStr)
	matches := regex.FindStringSubmatch(req.URL.EscapedPath())
	if matches == nil || len(matches) < 1 {
		return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
	}
	body, err := server.UnmarshalRequestAsJSON[armoep.CheckNameAvailabilityRequest](req)
	if err != nil {
		return nil, err
	}
