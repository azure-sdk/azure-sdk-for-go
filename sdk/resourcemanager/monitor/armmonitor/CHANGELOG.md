...
- Type of `ActionList.ActionGroups` has been changed from `[]*ActionGroupAutoGenerated` to `[]*ActionGroupForActivityLogAlerts`
...
- New const `OperatorForConditionEquals`
...
- New const `OperatorForConditionGreaterThan`
- New const `PrivateEndpointConnectionProvisioningStateFailed`
- New const `OperatorForConditionGreaterThanOrEqual`
- New const `PrivateEndpointServiceConnectionStatusPending`
- New const `OperatorForConditionLessThan`
...
- New const `OperatorForConditionLessThanOrEqual`
- New const `TimeAggregationAverage`
- New type alias `OperatorForCondition`
...
- New function `*PredictiveMetricClient.Get(context.Context, string, string, string, *PredictiveMetricClientGetOptions) (PredictiveMetricClientGetResponse, error)`
- New function `PossibleOperatorForConditionValues() []OperatorForCondition`
...
- New struct `ActionGroupForActivityLogAlerts`
- New struct `Actions`
...
- New struct `ResourceForAutoscale`
