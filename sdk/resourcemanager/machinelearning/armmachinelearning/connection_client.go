...
// ListModels - Get models under the Azure ML workspace for all Azure OpenAI connections that the user can deploy.
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2024-10-01-preview
//   - resourceGroupName - The name of the resource group. The name is case insensitive.
//   - workspaceName - Azure Machine Learning Workspace Name
//   - options - ConnectionClientListModelsOptions contains the optional parameters for the ConnectionClient.ListModels method.
func (client *ConnectionClient) ListModels(ctx context.Context, resourceGroupName string, workspaceName string, options *ConnectionClientListModelsOptions) (ConnectionClientListModelsResponse, error) {
	var err error
	const operationName = "ConnectionClient.ListModels"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.listModelsCreateRequest(ctx, resourceGroupName, workspaceName, options)
	if err != nil {
		return ConnectionClientListModelsResponse{}, err
	}
	httpResp, err := client.internal.Pipeline().Do(req)
	if err != nil {
		return ConnectionClientListModelsResponse{}, err
...
func (client *ConnectionClient) listModelsCreateRequest(ctx context.Context, resourceGroupName string, workspaceName string, options *ConnectionClientListModelsOptions) (*policy.Request, error) {
...
func (client *ConnectionClient) listModelsHandleResponse(resp *http.Response) (ConnectionClientListModelsResponse, error) {
	result := ConnectionClientListModelsResponse{}
	if err := runtime.UnmarshalAsJSON(resp, &result.EndpointModels); err != nil {
		return ConnectionClientListModelsResponse{}, err
	}
	return result, nil
