...
// notificationName - Notification Name Identifier.
// email - Email identifier.
// options - NotificationRecipientEmailClientGetOptions contains the optional parameters for the NotificationRecipientEmailClient.Get
// method.
func (client *NotificationRecipientEmailClient) Get(ctx context.Context, resourceGroupName string, serviceName string, notificationName NotificationName, email string, options *NotificationRecipientEmailClientGetOptions) (NotificationRecipientEmailClientGetResponse, error) {
	req, err := client.getCreateRequest(ctx, resourceGroupName, serviceName, notificationName, email, options)
	if err != nil {
		return NotificationRecipientEmailClientGetResponse{}, err
	}
	resp, err := client.pl.Do(req)
	if err != nil {
		return NotificationRecipientEmailClientGetResponse{}, err
	...
	if !runtime.HasStatusCode(resp, http.StatusNoContent) {
		return NotificationRecipientEmailClientGetResponse{}, runtime.NewResponseError(resp)
	}
	return NotificationRecipientEmailClientGetResponse{Success: resp.StatusCode >= 200 && resp.StatusCode < 300}, nil
}

// getCreateRequest creates the Get request.
func (client *NotificationRecipientEmailClient) getCreateRequest(ctx context.Context, resourceGroupName string, serviceName string, notificationName NotificationName, email string, options *NotificationRecipientEmailClientGetOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.ApiManagement/service/{serviceName}/notifications/{notificationName}/recipientEmails/{email}"
	if resourceGroupName == "" {
		return nil, errors.New("parameter resourceGroupName cannot be empty")
	}
