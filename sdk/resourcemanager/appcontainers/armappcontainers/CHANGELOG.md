...
## 4.0.0-beta.1 (2024-09-12)
### Breaking Changes

- Function `*JobsClient.ProxyGet` parameter(s) have been changed from `(context.Context, string, string, string, *JobsClientProxyGetOptions)` to `(context.Context, string, string, *JobsClientProxyGetOptions)`
- Field `SystemData` of struct `BillingMeter` has been removed
- Field `DaprAIConnectionString` of struct `ManagedEnvironmentProperties` has been removed
...
- New enum type `JobRunningState` with values `JobRunningStateProgressing`, `JobRunningStateRunning`, `JobRunningStateSuspended`
