...
type CommonExportProperties struct {
	...
	// Allow customers to select compress data(gzip) for exports. This setting will enable destination file compression scheme
	// at runtime. By default set to None.
	...
	// READ-ONLY; The export suspension reason if export is in SystemSuspended state.
...
// EAPriceSheetProperties - The properties of the EA price sheet. Properties supported with version 2024-10-01-preview are
// as below
...
// ExportDatasetConfiguration - The export dataset configuration.
...
type ExportDeliveryDestination struct {
	...
	MicrosoftFabricSettings *MicrosoftFabricDestinationSettings
	...
	// The export delivery destination type. Currently only 'AzureBlob' and 'MicrosoftFabric' is supported. If no value is provided
	// it will default to 'AzureBlob'.
...
type ExportSchedule struct {
	...
	// The status of the export's schedule. If 'Inactive', the export's schedule is paused. 'SystemSuspended' can only be set
	// by export service.
	Status *StatusType
}

// ExportSuspensionContext - The properties of the export run.
...
// FabricLakehouse - This represents the fabric lakehouse identifier.
...
// FabricWorkspace - This represents the fabric workspace identifier.
...
// MicrosoftFabricDestinationSettings - This represents the fabric specific settings for export destination when delivery
// info type is 'MicrosoftFabric'.
...
type SavingsPlanUtilizationSummaryProperties struct {
	...
	// Suppose the API call is for usageDate > 2024-10-01-preview and usageDate < 2022-10-31 at a
	// daily granularity. There will be one record per benefit id for each day. For a single day, the avgUtilizationPercentage
	// value will be equal to the average of the set of values where the set contains
	// 24 utilization percentage entries one for each hour in a specific day.
	...
	// Suppose the API call is for usageDate > 2024-10-01-preview and usageDate < 2022-10-31 at a
	// daily granularity. There will be one record per benefit id for each day. For a single day, the maxUtilizationPercentage
	// value will be equal to the largest in the set of values where the set contains
	// 24 utilization percentage entries one for each hour in a specific day. If on the day 2022-10-18, the largest utilization
	// percentage was 90% at hour 5, then the value for the maxUtilizationPercentage
	// in the response will be 90%.
	...
	// Suppose the API call is for usageDate > 2024-10-01-preview and usageDate < 2022-10-31 at a
	// daily granularity. There will be one record per benefit id for each day. For a single day, the minUtilizationPercentage
	// value will be equal to the smallest in the set of values where the set contains
	// 24 utilization percentage entries one for each hour in a specific day. If on the day 2022-10-18, the lowest utilization
	// percentage was 10% at hour 4, then the value for the minUtilizationPercentage in
