...
## 3.0.0-beta.1 (2024-09-06)
### Breaking Changes

- Type of `AssociationContractProperties.ProvisioningState` has been changed from `*string` to `*ProvisioningState`
- Type of `ErrorResponse.Error` has been changed from `*ErrorResponseBody` to `*ErrorDetail`
- `SKUTypeBasic`, `SKUTypeConsumption`, `SKUTypeDeveloper`, `SKUTypeIsolated`, `SKUTypePremium` from enum `SKUType` has been removed
...
- New value `SKUTypeWorkspaceGatewayPremium`, `SKUTypeWorkspaceGatewayStandard` added to enum type `SKUType`
...
- New enum type `ProvisioningState` with values `ProvisioningStateActivating`, `ProvisioningStateCanceled`, `ProvisioningStateCreated`, `ProvisioningStateFailed`, `ProvisioningStateSucceeded`, `ProvisioningStateTerminating`, `ProvisioningStateUpdating`, `ProvisioningStateUpgrading`
...
- New field `ConfigurationAPI`, `DeveloperPortalStatus`, `LegacyPortalStatus`, `ReleaseChannel` in struct `ServiceBaseProperties`
- New field `Parameters` in struct `ServiceClientBeginMigrateToStv2Options`
- New field `ConfigurationAPI`, `DeveloperPortalStatus`, `LegacyPortalStatus`, `ReleaseChannel` in struct `ServiceProperties`
- New field `ConfigurationAPI`, `DeveloperPortalStatus`, `LegacyPortalStatus`, `ReleaseChannel` in struct `ServiceUpdateProperties`
