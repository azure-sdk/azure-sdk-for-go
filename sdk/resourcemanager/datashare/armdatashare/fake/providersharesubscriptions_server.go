...
func (p *ProviderShareSubscriptionsServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	...
	go func() {
		...
		var res result
		if providerShareSubscriptionsServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = providerShareSubscriptionsServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
			case "ProviderShareSubscriptionsClient.Adjust":
				res.resp, res.err = p.dispatchAdjust(req)
			case "ProviderShareSubscriptionsClient.GetByShare":
				res.resp, res.err = p.dispatchGetByShare(req)
			case "ProviderShareSubscriptionsClient.NewListBySharePager":
				res.resp, res.err = p.dispatchNewListBySharePager(req)
			case "ProviderShareSubscriptionsClient.Reinstate":
				res.resp, res.err = p.dispatchReinstate(req)
			case "ProviderShareSubscriptionsClient.BeginRevoke":
				res.resp, res.err = p.dispatchBeginRevoke(req)
			default:
...
// set this to conditionally intercept incoming requests to ProviderShareSubscriptionsServerTransport
var providerShareSubscriptionsServerTransportInterceptor interface {
