...
// The returned OperationResultsServerTransport instance is connected to an instance of armmysqlflexibleservers.OperationResultsClient via the
...
// OperationResultsServerTransport connects instances of armmysqlflexibleservers.OperationResultsClient to instances of OperationResultsServer.
...
func (o *OperationResultsServerTransport) dispatchGet(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.DBforMySQL/locations/(?P<locationName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/operationResults/(?P<operationId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)`
	...
	resp, err := server.MarshalResponseAsJSON(respContent, server.GetResponse(respr).OperationStatusExtendedResult, req)
	if err != nil {
		return nil, err
	}
	return resp, nil
