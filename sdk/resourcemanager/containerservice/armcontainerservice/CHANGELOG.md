...
- `ManagedClusterSKUNameBasic` from enum `ManagedClusterSKUName` has been removed
- `ManagedClusterSKUTierPaid` from enum `ManagedClusterSKUTier` has been removed
...
- New field `ImageCleaner`, `WorkloadIdentity` in struct `ManagedClusterSecurityProfile`
- New field `NetworkDataplane`, `NetworkPluginMode` in struct `NetworkProfile`
