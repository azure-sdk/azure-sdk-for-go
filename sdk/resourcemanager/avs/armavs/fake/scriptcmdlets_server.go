...
type ScriptCmdletsServer struct {
	...
	// HTTP status codes to indicate success: http.StatusOK
	NewListByScriptPackagePager func(resourceGroupName string, privateCloudName string, scriptPackageName string, options *armavs.ScriptCmdletsClientListByScriptPackageOptions) (resp azfake.PagerResponder[armavs.ScriptCmdletsClientListByScriptPackageResponse])
}
...
func (s *ScriptCmdletsServerTransport) dispatchNewListByScriptPackagePager(req *http.Request) (*http.Response, error) {
	...
	if newListByScriptPackagePager == nil {
		...
		resp := s.srv.NewListByScriptPackagePager(resourceGroupNameParam, privateCloudNameParam, scriptPackageNameParam, nil)
