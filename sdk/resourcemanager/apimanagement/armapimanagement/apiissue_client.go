...
// apiID - API identifier. Must be unique in the current API Management service instance.
// issueID - Issue identifier. Must be unique in the current API Management service instance.
// options - APIIssueClientHeadOptions contains the optional parameters for the APIIssueClient.Head method.
func (client *APIIssueClient) Head(ctx context.Context, resourceGroupName string, serviceName string, apiID string, issueID string, options *APIIssueClientHeadOptions) (APIIssueClientHeadResponse, error) {
	req, err := client.headCreateRequest(ctx, resourceGroupName, serviceName, apiID, issueID, options)
	if err != nil {
		return APIIssueClientHeadResponse{}, err
	}
	resp, err := client.pl.Do(req)
	if err != nil {
		return APIIssueClientHeadResponse{}, err
	...
	if !runtime.HasStatusCode(resp, http.StatusOK) {
		return APIIssueClientHeadResponse{}, runtime.NewResponseError(resp)
	}
	return client.headHandleResponse(resp)
}

// headCreateRequest creates the Head request.
func (client *APIIssueClient) headCreateRequest(ctx context.Context, resourceGroupName string, serviceName string, apiID string, issueID string, options *APIIssueClientHeadOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.ApiManagement/service/{serviceName}/apis/{apiId}/issues/{issueId}"
	if resourceGroupName == "" {
		return nil, errors.New("parameter resourceGroupName cannot be empty")
	}
...
func (client *APIIssueClient) headHandleResponse(resp *http.Response) (APIIssueClientHeadResponse, error) {
	result := APIIssueClientHeadResponse{}
	if val := resp.Header.Get("ETag"); val != "" {
		result.ETag = &val
	}
