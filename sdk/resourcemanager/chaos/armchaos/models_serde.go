...
// MarshalJSON implements the json.Marshaller interface for type PostActionResult.
func (p PostActionResult) MarshalJSON() ([]byte, error) {
	objectMap := make(map[string]any)
	populate(objectMap, "name", p.Name)
	populate(objectMap, "statusUrl", p.StatusURL)
	return json.Marshal(objectMap)
}

// UnmarshalJSON implements the json.Unmarshaller interface for type PostActionResult.
func (p *PostActionResult) UnmarshalJSON(data []byte) error {
	var rawMsg map[string]json.RawMessage
	if err := json.Unmarshal(data, &rawMsg); err != nil {
	...
	for key, val := range rawMsg {
		...
		case "name":
			...
			delete(rawMsg, key)
		case "statusUrl":
			err = unpopulate(val, "StatusURL", &p.StatusURL)
			delete(rawMsg, key)
		}
		if err != nil {
			return fmt.Errorf("unmarshalling type %T: %v", p, err)
