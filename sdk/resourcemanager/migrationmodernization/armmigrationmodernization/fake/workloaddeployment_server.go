...
func (w *WorkloadDeploymentServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	...
	go func() {
		...
		var res result
		if workloadDeploymentServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = workloadDeploymentServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
			case "WorkloadDeploymentClient.BeginBuildContainerImage":
				res.resp, res.err = w.dispatchBeginBuildContainerImage(req)
			...
			case "WorkloadDeploymentClient.GetSecretConfigurations":
				res.resp, res.err = w.dispatchGetSecretConfigurations(req)
			...
			case "WorkloadDeploymentClient.BeginMigrate":
				res.resp, res.err = w.dispatchBeginMigrate(req)
			case "WorkloadDeploymentClient.BeginTestMigrate":
				res.resp, res.err = w.dispatchBeginTestMigrate(req)
			case "WorkloadDeploymentClient.BeginTestMigrateCleanup":
				res.resp, res.err = w.dispatchBeginTestMigrateCleanup(req)
			default:
...
// set this to conditionally intercept incoming requests to WorkloadDeploymentServerTransport
var workloadDeploymentServerTransportInterceptor interface {
