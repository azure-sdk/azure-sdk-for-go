...
// BeginInvokeUpgrade - Invoke upgrade on deferred rolling upgrade for OS disk on a VM scale set instance.
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2023-07-01
//   - resourceGroupName - The name of the resource group.
//   - vmScaleSetName - The name of the VM scale set.
//   - instanceID - The instance ID of the virtual machine.
//   - options - VirtualMachineScaleSetVMsClientBeginInvokeUpgradeOptions contains the optional parameters for the VirtualMachineScaleSetVMsClient.BeginInvokeUpgrade
//     method.
func (client *VirtualMachineScaleSetVMsClient) BeginInvokeUpgrade(ctx context.Context, resourceGroupName string, vmScaleSetName string, instanceID string, options *VirtualMachineScaleSetVMsClientBeginInvokeUpgradeOptions) (*runtime.Poller[VirtualMachineScaleSetVMsClientInvokeUpgradeResponse], error) {
	if options == nil || options.ResumeToken == "" {
		resp, err := client.invokeUpgrade(ctx, resourceGroupName, vmScaleSetName, instanceID, options)
		if err != nil {
			return nil, err
		}
		return runtime.NewPoller[VirtualMachineScaleSetVMsClientInvokeUpgradeResponse](resp, client.internal.Pipeline(), nil)
	} else {
		return runtime.NewPollerFromResumeToken[VirtualMachineScaleSetVMsClientInvokeUpgradeResponse](options.ResumeToken, client.internal.Pipeline(), nil)
	}
}

// InvokeUpgrade - Invoke upgrade on deferred rolling upgrade for OS disk on a VM scale set instance.
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2023-07-01
func (client *VirtualMachineScaleSetVMsClient) invokeUpgrade(ctx context.Context, resourceGroupName string, vmScaleSetName string, instanceID string, options *VirtualMachineScaleSetVMsClientBeginInvokeUpgradeOptions) (*http.Response, error) {
	req, err := client.invokeUpgradeCreateRequest(ctx, resourceGroupName, vmScaleSetName, instanceID, options)
	if err != nil {
		return nil, err
	}
	resp, err := client.internal.Pipeline().Do(req)
...
// invokeUpgradeCreateRequest creates the InvokeUpgrade request.
func (client *VirtualMachineScaleSetVMsClient) invokeUpgradeCreateRequest(ctx context.Context, resourceGroupName string, vmScaleSetName string, instanceID string, options *VirtualMachineScaleSetVMsClientBeginInvokeUpgradeOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Compute/virtualMachineScaleSets/{vmScaleSetName}/virtualMachines/{instanceId}/invokeupgrade"
	if resourceGroupName == "" {
		return nil, errors.New("parameter resourceGroupName cannot be empty")
	}
	urlPath = strings.ReplaceAll(urlPath, "{resourceGroupName}", url.PathEscape(resourceGroupName))
