...
// workspaceName - Name of Azure Machine Learning workspace.
// computeName - Name of the Azure Machine Learning compute.
// options - ComputeClientUpdateSchedulesOptions contains the optional parameters for the ComputeClient.UpdateSchedules method.
func (client *ComputeClient) UpdateSchedules(ctx context.Context, resourceGroupName string, workspaceName string, computeName string, options *ComputeClientUpdateSchedulesOptions) (ComputeClientUpdateSchedulesResponse, error) {
	req, err := client.updateSchedulesCreateRequest(ctx, resourceGroupName, workspaceName, computeName, options)
	if err != nil {
		return ComputeClientUpdateSchedulesResponse{}, err
	}
	resp, err := client.pl.Do(req)
	if err != nil {
		return ComputeClientUpdateSchedulesResponse{}, err
	...
	if !runtime.HasStatusCode(resp, http.StatusOK) {
		return ComputeClientUpdateSchedulesResponse{}, runtime.NewResponseError(resp)
	}
	return ComputeClientUpdateSchedulesResponse{}, nil
}

// updateSchedulesCreateRequest creates the UpdateSchedules request.
func (client *ComputeClient) updateSchedulesCreateRequest(ctx context.Context, resourceGroupName string, workspaceName string, computeName string, options *ComputeClientUpdateSchedulesOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.MachineLearningServices/workspaces/{workspaceName}/computes/{computeName}/updateSchedules"
	if client.subscriptionID == "" {
		return nil, errors.New("parameter client.subscriptionID cannot be empty")
	}
	urlPath = strings.ReplaceAll(urlPath, "{subscriptionId}", url.PathEscape(client.subscriptionID))
