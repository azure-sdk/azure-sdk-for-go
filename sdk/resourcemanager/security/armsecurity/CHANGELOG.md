...
- Type of `MdeOnboardingDataProperties.OnboardingPackageLinux` has been changed from `[]byte` to `*string`
- Type of `MdeOnboardingDataProperties.OnboardingPackageWindows` has been changed from `[]byte` to `*string`
- `ProvisioningStateUpdating` from enum `ProvisioningState` has been removed
- Operation `*AlertsClient.BeginSimulate` has been changed to non-LRO, use `*AlertsClient.Simulate` instead.

### Features Added

- New value `ProvisioningStateCanceled`, `ProvisioningStateInProgress` added to enum type `ProvisioningState`
- New value `RecommendationTypeBinarySignature`, `RecommendationTypeFile`, `RecommendationTypeFileHash`, `RecommendationTypeProductSignature`, `RecommendationTypePublisherSignature`, `RecommendationTypeVersionAndAboveSignature` added to enum type `RecommendationType`
- New struct `BenchmarkReferenceAutoGenerated`
- New struct `DevOpsCapability`
- New field `Capabilities` in struct `DevOpsConfigurationProperties`
- New field `AllowedSourceAddressPrefixes`, `Duration` in struct `JitNetworkAccessPolicyInitiatePort`
- New field `Benchmarks`, `LastReportedScanTime` in struct `SQLServerVulnerabilityProperties`
