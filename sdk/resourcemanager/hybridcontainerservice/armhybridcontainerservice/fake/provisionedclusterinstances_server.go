...
type ProvisionedClusterInstancesServer struct {
	...
	// ListUpgradeProfile is the fake for method ProvisionedClusterInstancesClient.ListUpgradeProfile
	// HTTP status codes to indicate success: http.StatusOK
	ListUpgradeProfile func(ctx context.Context, connectedClusterResourceURI string, options *armhybridcontainerservice.ProvisionedClusterInstancesClientListUpgradeProfileOptions) (resp azfake.Responder[armhybridcontainerservice.ProvisionedClusterInstancesClientListUpgradeProfileResponse], errResp azfake.ErrorResponder)
...
func (p *ProvisionedClusterInstancesServerTransport) Do(req *http.Request) (*http.Response, error) {
	...
	case "ProvisionedClusterInstancesClient.ListUpgradeProfile":
		resp, err = p.dispatchListUpgradeProfile(req)
...
func (p *ProvisionedClusterInstancesServerTransport) dispatchListUpgradeProfile(req *http.Request) (*http.Response, error) {
	if p.srv.ListUpgradeProfile == nil {
		return nil, &nonRetriableError{errors.New("fake for method ListUpgradeProfile not implemented")}
	}
	const regexStr = `/(?P<connectedClusterResourceUri>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.HybridContainerService/provisionedClusterInstances/default/upgradeProfiles`
	regex := regexp.MustCompile(regexStr)
	matches := regex.FindStringSubmatch(req.URL.EscapedPath())
	if matches == nil || len(matches) < 1 {
		return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
	...
	respr, errRespr := p.srv.ListUpgradeProfile(req.Context(), connectedClusterResourceURIParam, nil)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
	respContent := server.GetResponseContent(respr)
	...
	resp, err := server.MarshalResponseAsJSON(respContent, server.GetResponse(respr).ProvisionedClusterUpgradeProfileList, req)
	if err != nil {
		return nil, err
	}
	return resp, nil
