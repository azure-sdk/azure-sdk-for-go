...
// Generated from API version 2022-06-13
...
func (client *AssignmentsClient) createOrUpdateCreateRequest(ctx context.Context, guestConfigurationAssignmentName string, resourceGroupName string, vmName string, parameters Assignment, options *AssignmentsClientCreateOrUpdateOptions) (*policy.Request, error) {
	...
	reqQP.Set("api-version", "2022-06-13")
...
// VMHealthCheck - Get VM Health details
// If the operation fails it returns an *azcore.ResponseError type.
// Generated from API version 2022-06-13
// resourceGroupName - The resource group name.
// vmName - The name of the virtual machine.
// options - AssignmentsClientVMHealthCheckOptions contains the optional parameters for the AssignmentsClient.VMHealthCheck
// method.
func (client *AssignmentsClient) VMHealthCheck(ctx context.Context, resourceGroupName string, vmName string, options *AssignmentsClientVMHealthCheckOptions) (AssignmentsClientVMHealthCheckResponse, error) {
	req, err := client.vmHealthCheckCreateRequest(ctx, resourceGroupName, vmName, options)
	if err != nil {
		return AssignmentsClientVMHealthCheckResponse{}, err
	}
	resp, err := client.pl.Do(req)
	if err != nil {
		return AssignmentsClientVMHealthCheckResponse{}, err
	...
	if !runtime.HasStatusCode(resp, http.StatusOK) {
		return AssignmentsClientVMHealthCheckResponse{}, runtime.NewResponseError(resp)
	}
	return client.vmHealthCheckHandleResponse(resp)
}

// vmHealthCheckCreateRequest creates the VMHealthCheck request.
func (client *AssignmentsClient) vmHealthCheckCreateRequest(ctx context.Context, resourceGroupName string, vmName string, options *AssignmentsClientVMHealthCheckOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Compute/virtualMachines/{vmName}/providers/Microsoft.GuestConfiguration/guestConfigurationAssignments/diagnostics/healthcheck"
	if client.subscriptionID == "" {
		return nil, errors.New("parameter client.subscriptionID cannot be empty")
	}
	urlPath = strings.ReplaceAll(urlPath, "{subscriptionId}", url.PathEscape(client.subscriptionID))
...
// vmHealthCheckHandleResponse handles the VMHealthCheck response.
func (client *AssignmentsClient) vmHealthCheckHandleResponse(resp *http.Response) (AssignmentsClientVMHealthCheckResponse, error) {
	result := AssignmentsClientVMHealthCheckResponse{}
	if err := runtime.UnmarshalAsJSON(resp, &result.VMHealthCheckProperties); err != nil {
		return AssignmentsClientVMHealthCheckResponse{}, err
	}
	return result, nil
}
