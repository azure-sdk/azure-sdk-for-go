...
- Type of `MetricAlertResource.Properties` has been changed from `*MetricAlertProperties` to `*MetricAlertsPropertiesBase`
- Type of `MetricAlertResourcePatch.Properties` has been changed from `*MetricAlertPropertiesPatch` to `*MetricAlertResourcePatchProperties`
...
- Operation `*MetricAlertsClient.NewListByResourceGroupPager` does not support pagination anymore, use `*MetricAlertsClient.ListByResourceGroup` instead.
- Operation `*MetricAlertsClient.NewListBySubscriptionPager` does not support pagination anymore, use `*MetricAlertsClient.ListBySubscription` instead.
...
- New enum type `MetricAlertResourceKind` with values `MetricAlertResourceKindMetric`, `MetricAlertResourceKindQuery`
- New enum type `MetricAlertRuleCriteriaOperator` with values `MetricAlertRuleCriteriaOperatorEquals`, `MetricAlertRuleCriteriaOperatorGreaterThan`, `MetricAlertRuleCriteriaOperatorGreaterThanOrEqual`, `MetricAlertRuleCriteriaOperatorLessThan`, `MetricAlertRuleCriteriaOperatorLessThanOrEqual`, `MetricAlertRuleCriteriaOperatorNotEquals`
- New struct `ErrorResponseError`
- New struct `IncidentReceiver`
- New struct `IncidentServiceConnection`
- New struct `MetricAlertResourcePatchProperties`
- New struct `MetricAlertRuleAction`
- New struct `MetricAlertRuleCriteria`
- New struct `MetricAlertsMetricProperties`
- New struct `MetricAlertsPropertiesBase`
- New struct `MetricAlertsQueryProperties`
- New struct `MetricQueryCriteria`
- New struct `QueryResolveConfiguration`
...
- New field `Kind` in struct `MetricAlertResource`
