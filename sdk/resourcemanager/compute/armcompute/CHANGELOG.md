...
- New field `TreatFailureAsDeploymentFailure` in struct `VirtualMachineRunCommandProperties`
- New field `ScriptURIManagedIdentity` in struct `VirtualMachineRunCommandScriptSource`
- New field `PriorityMixPolicy` in struct `VirtualMachineScaleSetUpdateProperties`
- New field `SpotRestorePolicy` in struct `VirtualMachineScaleSetUpdateProperties`
