...
type IacProfilesServer struct {
	...
	// QuickTemplate is the fake for method IacProfilesClient.QuickTemplate
	// HTTP status codes to indicate success: http.StatusOK
	QuickTemplate func(ctx context.Context, resourceGroupName string, iacProfileName string, parameters armdevhub.QuickTemplateRequest, options *armdevhub.IacProfilesClientQuickTemplateOptions) (resp azfake.Responder[armdevhub.IacProfilesClientQuickTemplateResponse], errResp azfake.ErrorResponder)
...
func (i *IacProfilesServerTransport) Do(req *http.Request) (*http.Response, error) {
	...
	case "IacProfilesClient.QuickTemplate":
		resp, err = i.dispatchQuickTemplate(req)
...
func (i *IacProfilesServerTransport) dispatchQuickTemplate(req *http.Request) (*http.Response, error) {
	if i.srv.QuickTemplate == nil {
		return nil, &nonRetriableError{errors.New("fake for method QuickTemplate not implemented")}
	}
	const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.DevHub/iacProfiles/(?P<iacProfileName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/quickTemplate`
	regex := regexp.MustCompile(regexStr)
	matches := regex.FindStringSubmatch(req.URL.EscapedPath())
	if matches == nil || len(matches) < 3 {
		return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
	}
	body, err := server.UnmarshalRequestAsJSON[armdevhub.QuickTemplateRequest](req)
	if err != nil {
		return nil, err
	}
	resourceGroupNameParam, err := url.PathUnescape(matches[regex.SubexpIndex("resourceGroupName")])
	...
	respr, errRespr := i.srv.QuickTemplate(req.Context(), resourceGroupNameParam, iacProfileNameParam, body, nil)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
	respContent := server.GetResponseContent(respr)
