...
type WorkflowVersionsServer struct {
	...
	// ListCallbackURL is the fake for method WorkflowVersionsClient.ListCallbackURL
	// HTTP status codes to indicate success: http.StatusOK
	ListCallbackURL func(ctx context.Context, resourceGroupName string, workflowName string, versionID string, triggerName string, options *armlogic.WorkflowVersionsClientListCallbackURLOptions) (resp azfake.Responder[armlogic.WorkflowVersionsClientListCallbackURLResponse], errResp azfake.ErrorResponder)
}
...
func (w *WorkflowVersionsServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	...
	go func() {
		...
		if !intercepted {
			...
			case "WorkflowVersionsClient.ListCallbackURL":
...
func (w *WorkflowVersionsServerTransport) dispatchListCallbackURL(req *http.Request) (*http.Response, error) {
	if w.srv.ListCallbackURL == nil {
		return nil, &nonRetriableError{errors.New("fake for method ListCallbackURL not implemented")}
	}
	const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.Logic/workflows/(?P<workflowName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/versions/(?P<versionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/(?P<triggerName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/listCallbackUrl`
	regex := regexp.MustCompile(regexStr)
	matches := regex.FindStringSubmatch(req.URL.EscapedPath())
	...
	var options *armlogic.WorkflowVersionsClientListCallbackURLOptions
	if !reflect.ValueOf(body).IsZero() {
		options = &armlogic.WorkflowVersionsClientListCallbackURLOptions{
			Parameters: &body,
		}
	}
