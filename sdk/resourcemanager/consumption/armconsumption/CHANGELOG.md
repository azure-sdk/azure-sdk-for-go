...
- New value `LotSourceAwardedCredit`, `LotSourceServiceSpecificCredit` added to enum type `LotSource`
- New enum type `LotType` with values `LotTypeBillingCorrectionCredit`, `LotTypeFreeTrial`, `LotTypeGoodwillCredit`, `LotTypeMACC`, `LotTypeMonetaryCommitment`, `LotTypeOutageCredit`, `LotTypeSSC`, `LotTypeSubscriptionInterruptionCredit`
...
- New field `ExtendedProperties`, `IsEstimatedBalance`, `LotSource`, `LotType`, `OrganizationType`, `UsedAmount` in struct `LotProperties`
- New field `ExtendedProperties` in struct `LotsClientListByBillingAccountOptions`
