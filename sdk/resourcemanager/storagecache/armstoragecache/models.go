...
type AutoExportJobPropertiesStatus struct {
	...
	// READ-ONLY; Files that have been exported in current iteration.
	CurrentIterationFilesExported *int64

	// READ-ONLY; Files failed to export in current iteration.
...
// AutoImportJob - An auto import job instance. Follows Azure Resource Manager standards: https://github.com/Azure/azure-resource-manager-rpc/blob/master/v1.0/resource-api-reference.md
...
// AutoImportJobProperties - Properties of the auto import job.
type AutoImportJobProperties struct {
	...
	// will disable the current active auto import job. By default it is set to
	// 'Enable'.
	...
	// How the auto import job will handle conflicts. For example, if the auto import job is trying to bring in a directory, but
	// a file is at that path, how it handles it. Fail indicates that the auto import
	// job should stop immediately and not do anything with the conflict. Skip indicates that it should pass over the conflict.
	// OverwriteIfDirty causes the auto import job to delete and re-import the file or
	// directory if it is a conflicting type, is dirty, or is currently released. OverwriteAlways extends OverwriteIfDirty to
	// include releasing files that had been restored but were not dirty. Please
	// reference https://learn.microsoft.com/en-us/azure/azure-managed-lustre/ for a thorough explanation of these resolution
	// modes.
	...
	// READ-ONLY; The status of the auto import job.
	Status *AutoImportJobPropertiesStatus
}

// AutoImportJobPropertiesStatus - The status of the auto import job.
type AutoImportJobPropertiesStatus struct {
	// READ-ONLY; The storage account blob change feed status of the auto import job.
	BlobSyncEvents *AutoImportJobPropertiesStatusBlobSyncEvents

	// READ-ONLY; Number of directories imported during full scan.
	ImportedDirectories *int32

	// READ-ONLY; Number of files imported during full scan.
	ImportedFiles *int32

	// READ-ONLY; Number of symlinks imported during full scan.
	ImportedSymlinks *int32

	// READ-ONLY; Number of preexisting directories during full scan.
	PreexistingDirectories *int32

	// READ-ONLY; Number of preexisting files during full scan.
	PreexistingFiles *int32

	// READ-ONLY; Number of preexisting symlinks during full scan.
	PreexistingSymlinks *int32

	// READ-ONLY; Rate of blob import during full scan.
	RateOfBlobImport *int32

	// READ-ONLY; Rate of blobs walked during full scan.
	RateOfBlobWalk *int32

	// READ-ONLY; Date and time of when the full scan ended.
	ScanEndTime *time.Time

	// READ-ONLY; Date and time of when the currently running full scan began.
	ScanStartTime *time.Time

	// READ-ONLY; The state of the auto import operation.
	State *AutoImportJobState

	// READ-ONLY; Total number of blobs imported during full scan.
	TotalBlobsImported *int32

	// READ-ONLY; Total number of blobs walked during full scan.
	TotalBlobsWalked *int32

	// READ-ONLY; Total conflicts encountered during full scan.
	TotalConflicts *int32

	// READ-ONLY; Total errors encountered during full scan.
	TotalErrors *int32
}

// AutoImportJobPropertiesStatusBlobSyncEvents - The storage account blob change feed status of the auto import job.
type AutoImportJobPropertiesStatusBlobSyncEvents struct {
	// READ-ONLY; Number of deletions during auto import.
	Deletions *int32

	// READ-ONLY; Number of directories imported during auto import.
	ImportedDirectories *int32

	// READ-ONLY; Number of files imported during auto import.
	ImportedFiles *int32

	// READ-ONLY; Number of symlinks imported during auto import.
	ImportedSymlinks *int32

	// READ-ONLY; Date and time of the last Change Feed event consumed.
	LastChangeFeedEventConsumedTime *time.Time

	// READ-ONLY; Date and time when last fully synchronized.
	LastTimeFullySynchronized *time.Time

	// READ-ONLY; Number of preexisting directories during auto import.
	PreexistingDirectories *int32

	// READ-ONLY; Number of preexisting files during auto import.
	PreexistingFiles *int32

	// READ-ONLY; Number of preexisting symlinks during auto import.
	PreexistingSymlinks *int32

	// READ-ONLY; Rate of blob import during auto import.
	RateOfBlobImport *int32

	// READ-ONLY; Total number of blobs imported during auto import.
	TotalBlobsImported *int32

	// READ-ONLY; Total conflicts encountered during auto import.
	TotalConflicts *int32

	// READ-ONLY; Total errors encountered during auto import.
	TotalErrors *int32
}

// AutoImportJobUpdate - An auto import job update instance.
type AutoImportJobUpdate struct {
...
// AutoImportJobsListResult - Result of the request to list auto import jobs. It contains a list of auto import jobs and a
// URL link to get the next set of results.
type AutoImportJobsListResult struct {
...
type ImportJobProperties struct {
	...
	// the current active import job. By default it is set to 'Active'.
...
type ImportJobPropertiesStatus struct {
	...
	// READ-ONLY; New or modified directories that have been imported into the filesystem.
	ImportedDirectories *int64

	// READ-ONLY; New or modified files that have been imported into the filesystem.
	ImportedFiles *int64

	// READ-ONLY; Newly added symbolic links into the filesystem.
	...
	// READ-ONLY; Directories that already exist in the filesystem and have not been modified.
	PreexistingDirectories *int64

	// READ-ONLY; Files that already exist in the filesystem and have not been modified.
	PreexistingFiles *int64

	// READ-ONLY; Symbolic links that already exist in the filesystem and have not been modified.
