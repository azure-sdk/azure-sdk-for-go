...
- New field `CitusVersion` in struct `ServerProperties`
- New field `EnablePublicIPAccess` in struct `ServerProperties`
- New field `FullyQualifiedDomainName` in struct `ServerProperties`
- New field `HaState` in struct `ServerProperties`
- New field `IsReadOnly` in struct `ServerProperties`
- New field `PostgresqlVersion` in struct `ServerProperties`
- New field `Role` in struct `ServerProperties`
