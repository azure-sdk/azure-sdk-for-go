...
// QuickTemplate - Create quick start template
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2024-07-01-preview
//   - resourceGroupName - The name of the resource group. The name is case insensitive.
//   - iacProfileName - The name of the IacProfile.
//   - options - IacProfilesClientQuickTemplateOptions contains the optional parameters for the IacProfilesClient.QuickTemplate
//     method.
func (client *IacProfilesClient) QuickTemplate(ctx context.Context, resourceGroupName string, iacProfileName string, parameters QuickTemplateRequest, options *IacProfilesClientQuickTemplateOptions) (IacProfilesClientQuickTemplateResponse, error) {
	var err error
	const operationName = "IacProfilesClient.QuickTemplate"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.quickTemplateCreateRequest(ctx, resourceGroupName, iacProfileName, parameters, options)
	if err != nil {
		return IacProfilesClientQuickTemplateResponse{}, err
	}
	httpResp, err := client.internal.Pipeline().Do(req)
	if err != nil {
		return IacProfilesClientQuickTemplateResponse{}, err
	...
	resp, err := client.quickTemplateHandleResponse(httpResp)
	return resp, err
}

// quickTemplateCreateRequest creates the QuickTemplate request.
func (client *IacProfilesClient) quickTemplateCreateRequest(ctx context.Context, resourceGroupName string, iacProfileName string, parameters QuickTemplateRequest, options *IacProfilesClientQuickTemplateOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.DevHub/iacProfiles/{iacProfileName}/quickTemplate"
	if client.subscriptionID == "" {
		return nil, errors.New("parameter client.subscriptionID cannot be empty")
	}
	urlPath = strings.ReplaceAll(urlPath, "{subscriptionId}", url.PathEscape(client.subscriptionID))
...
// quickTemplateHandleResponse handles the QuickTemplate response.
func (client *IacProfilesClient) quickTemplateHandleResponse(resp *http.Response) (IacProfilesClientQuickTemplateResponse, error) {
	result := IacProfilesClientQuickTemplateResponse{}
	if err := runtime.UnmarshalAsJSON(resp, &result.PrLinkResponse); err != nil {
		return IacProfilesClientQuickTemplateResponse{}, err
	}
	return result, nil
