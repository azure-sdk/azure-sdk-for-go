...
func (r ResourceProvisionPayload) MarshalJSON() ([]byte, error) {
	objectMap := make(map[string]any)
	populateByteArray(objectMap, "baseProfile", r.BaseProfile, func() any {
		return runtime.EncodeByteArray(r.BaseProfile, runtime.Base64StdFormat)
	})
	populate(objectMap, "resourceCount", r.ResourceCount)
	populateByteArray(objectMap, "resourceOverrides", r.ResourceOverrides, func() any {
		encodedValue := make([]string, len(r.ResourceOverrides))
		for i := 0; i < len(r.ResourceOverrides); i++ {
			encodedValue[i] = runtime.EncodeByteArray(r.ResourceOverrides[i], runtime.Base64StdFormat)
		}
		return encodedValue
	})
	...
	for key, val := range rawMsg {
		...
		case "baseProfile":
			if val != nil && string(val) != "null" {
				err = runtime.DecodeByteArray(string(val), &r.BaseProfile, runtime.Base64StdFormat)
			}
			delete(rawMsg, key)
		case "resourceCount":
		...
		case "resourceOverrides":
			...
			err = unpopulate(val, "ResourceOverrides", &encodedValue)
			if err == nil && len(encodedValue) > 0 {
				r.ResourceOverrides = make([][]byte, len(encodedValue))
				for i := 0; i < len(encodedValue) && err == nil; i++ {
					err = runtime.DecodeByteArray(encodedValue[i], &r.ResourceOverrides[i], runtime.Base64StdFormat)
				}
			}
			delete(rawMsg, key)
		case "resourcePrefix":
