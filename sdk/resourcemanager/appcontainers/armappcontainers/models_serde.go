...
// MarshalJSON implements the json.Marshaller interface for type PatchPropertiesProperties.
func (p PatchPropertiesProperties) MarshalJSON() ([]byte, error) {
	objectMap := make(map[string]any)
	populate(objectMap, "maxConcurrentSessions", p.MaxConcurrentSessions)
	populate(objectMap, "secrets", p.Secrets)
	return json.Marshal(objectMap)
}

// UnmarshalJSON implements the json.Unmarshaller interface for type PatchPropertiesProperties.
func (p *PatchPropertiesProperties) UnmarshalJSON(data []byte) error {
	var rawMsg map[string]json.RawMessage
	if err := json.Unmarshal(data, &rawMsg); err != nil {
		return fmt.Errorf("unmarshalling type %T: %v", p, err)
	}
	for key, val := range rawMsg {
		...
		case "maxConcurrentSessions":
			err = unpopulate(val, "MaxConcurrentSessions", &p.MaxConcurrentSessions)
			delete(rawMsg, key)
		case "secrets":
