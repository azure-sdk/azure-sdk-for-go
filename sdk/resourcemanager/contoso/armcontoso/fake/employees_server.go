...
type EmployeesServer struct {
	...
	// HTTP status codes to indicate success: http.StatusOK
	Update func(ctx context.Context, resourceGroupName string, employeeName string, properties armcontoso.EmployeeUpdate, options *armcontoso.EmployeesClientUpdateOptions) (resp azfake.Responder[armcontoso.EmployeesClientUpdateResponse], errResp azfake.ErrorResponder)
}
...
func (e *EmployeesServerTransport) dispatchUpdate(req *http.Request) (*http.Response, error) {
	...
	body, err := server.UnmarshalRequestAsJSON[armcontoso.EmployeeUpdate](req)
	if err != nil {
		return nil, err
	}
	resourceGroupNameParam, err := url.PathUnescape(matches[regex.SubexpIndex("resourceGroupName")])
