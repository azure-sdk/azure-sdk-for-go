...
func (s *SQLPoolBlobAuditingPoliciesServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	...
	go func() {
		...
		var res result
		if sqlPoolBlobAuditingPoliciesServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = sqlPoolBlobAuditingPoliciesServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
			...
			case "SQLPoolBlobAuditingPoliciesClient.NewListBySQLPoolPager":
				res.resp, res.err = s.dispatchNewListBySQLPoolPager(req)
			default:
...
// set this to conditionally intercept incoming requests to SQLPoolBlobAuditingPoliciesServerTransport
var sqlPoolBlobAuditingPoliciesServerTransportInterceptor interface {
