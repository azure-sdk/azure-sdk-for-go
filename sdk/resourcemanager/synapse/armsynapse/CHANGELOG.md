...
- Type of `ErrorResponseAutoGenerated.AdditionalInfo` has been changed from `[]*ErrorAdditionalInfo` to `[]*ErrorAdditionalInfoAutoGenerated`
...
- Field `LinkedAccessCheckOnTargetResource` of struct `ManagedVirtualNetworkSettings` has been removed
- Field `IsStorageSecondaryKeyInUse` of struct `ServerBlobAuditingPolicyProperties` has been removed

### Features Added

- Type of `WorkspaceProperties.ExtraProperties` has been changed from `map[string]any` to `any`
- New struct `ErrorAdditionalInfoAutoGenerated`
...
- New struct `SKUGet`
- New struct `SQLPoolGet`
- New struct `SQLPoolResourcePropertiesGet`
- New struct `VulnerabilityAssessmentRecurringScansPropertiesAutoGenerated`
- New field `Location` in struct `KustoPoolPrivateLinkResources`
- New field `Tags` in struct `KustoPoolPrivateLinkResources`
- New field `LanguageExtensionImageName` in struct `LanguageExtension`
- New field `ProvisioningState` in struct `ManagedIntegrationRuntimeTypeProperties`
- New field `Kind` in struct `SQLPool`
- New field `CatalogCollation` in struct `SQLPoolResourceProperties`
- New field `CurrentServiceObjectiveName` in struct `SQLPoolResourceProperties`
- New field `DefaultSecondaryLocation` in struct `SQLPoolResourceProperties`
