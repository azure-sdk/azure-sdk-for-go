...
//   - options - ResourceSyncRulesClientUpdateOptions contains the optional parameters for the ResourceSyncRulesClient.Update
//     method.
func (client *ResourceSyncRulesClient) Update(ctx context.Context, resourceGroupName string, resourceName string, childResourceName string, parameters PatchableResourceSyncRule, options *ResourceSyncRulesClientUpdateOptions) (ResourceSyncRulesClientUpdateResponse, error) {
	var err error
	const operationName = "ResourceSyncRulesClient.Update"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.updateCreateRequest(ctx, resourceGroupName, resourceName, childResourceName, parameters, options)
	if err != nil {
		return ResourceSyncRulesClientUpdateResponse{}, err
	}
	httpResp, err := client.internal.Pipeline().Do(req)
	if err != nil {
		return ResourceSyncRulesClientUpdateResponse{}, err
...
func (client *ResourceSyncRulesClient) updateCreateRequest(ctx context.Context, resourceGroupName string, resourceName string, childResourceName string, parameters PatchableResourceSyncRule, _ *ResourceSyncRulesClientUpdateOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.ExtendedLocation/customLocations/{resourceName}/resourceSyncRules/{childResourceName}"
	if client.subscriptionID == "" {
		return nil, errors.New("parameter client.subscriptionID cannot be empty")
	}
...
func (client *ResourceSyncRulesClient) updateHandleResponse(resp *http.Response) (ResourceSyncRulesClientUpdateResponse, error) {
	result := ResourceSyncRulesClientUpdateResponse{}
	if err := runtime.UnmarshalAsJSON(resp, &result.ResourceSyncRule); err != nil {
		return ResourceSyncRulesClientUpdateResponse{}, err
	}
	return result, nil
