...
// CodeExecutionConfiguration - Code execution configuration of the Session.
type CodeExecutionConfiguration struct {
...
type ContainerAppsBuildConfiguration struct {
	...
	PreBuildSteps []*PreBuildStepAutoGenerated
}
...
type ContainerAppsBuildProperties struct {
	...
	DestinationContainerRegistry *ContainerRegistryWithCustomImageAutoGenerated
...
// ContainerRegistryWithCustomImageAutoGenerated - Container registry that the final image will be uploaded to.
type ContainerRegistryWithCustomImageAutoGenerated struct {
	// READ-ONLY; Login server of the container registry that the final image should be uploaded to.
	Server *string

	// READ-ONLY; Full name that the final image should be uploaded as, including both image name and tag.
	Image *string
}
...
// CustomContainerConfiguration - Custom container configuration.
type CustomContainerConfiguration struct {
...
// EnvironmentVariableCollection - Environment variables to update.
type EnvironmentVariableCollection struct {
	// Collection of environment variables
	EnvironmentVariables []*EnvironmentVariablesItem
}

// EnvironmentVariablesItem - An item of the environment variables collection.
type EnvironmentVariablesItem struct {
	// The key of the item.
	Key *string
...
// HTTPGetAutoGenerated - Model representing a http get request.
type HTTPGetAutoGenerated struct {
	// READ-ONLY; URL to make HTTP GET request against.
	URL *string

	// READ-ONLY; Name of the file that the request should be saved to.
	FileName *string

	// READ-ONLY; List of headers to send with the request.
	Headers []*string
}
...
// LogLevelSetting - Log level settings for java workloads.
...
// PreBuildStepAutoGenerated - Model representing a pre-build step.
type PreBuildStepAutoGenerated struct {
	// Http get request to send before the build.
	HTTPGet *HTTPGetAutoGenerated

	// READ-ONLY; Description of the pre-build step.
	Description *string

	// READ-ONLY; List of custom commands to run.
	Scripts []*string
}
...
// SSHConfiguration - SSH configuration of the Session.
...
// SecretAutoGenerated - Secret definition.
type SecretAutoGenerated struct {
	// Secret Name.
	Name *string
...
// SessionAuthConfiguration - Session auth configuration.
type SessionAuthConfiguration struct {
...
// SessionCollection - Session collection ARM resource.
...
// SessionPoolCollection - Session Pool collection ARM resource.
...
// SessionPoolPatchProperties - Container App SessionPool patchable properties.
type SessionPoolPatchProperties struct {
	// SessionPool resource specific patchable properties.
	Properties *SessionPoolPatchPropertiesProperties
}

// SessionPoolPatchPropertiesProperties - SessionPool resource specific patchable properties.
type SessionPoolPatchPropertiesProperties struct {
...
// SessionProperties - Session resource specific properties.
type SessionProperties struct {
	...
	RequestedDurationInSeconds *int32
