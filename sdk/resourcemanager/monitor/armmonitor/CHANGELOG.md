...
- Type of `ErrorContract.Error` has been changed from `*ErrorResponse` to `*ErrorDetail`
- Type of `ErrorResponseAutoGenerated2.Error` has been changed from `*ErrorDetailAutoGenerated` to `*ErrorDetailAutoGenerated2`
...
- New field `Description`, `Publisher` in struct `OperationDisplay`
