...
// GetBlobReferenceSAS - Get a SAS for a blob reference associated with the model
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2023-10-01
//   - resourceGroupName - The name of the resource group. The name is case insensitive.
...
//   - modelName - Model name. This is case-sensitive.
//   - version - Version identifier. This is case-sensitive.
//   - body - blob reference request object
//   - options - RegistryModelVersionsClientGetBlobReferenceSASOptions contains the optional parameters for the RegistryModelVersionsClient.GetBlobReferenceSAS
//     method.
func (client *RegistryModelVersionsClient) GetBlobReferenceSAS(ctx context.Context, resourceGroupName string, registryName string, modelName string, version string, body BlobReferenceSASRequestDto, options *RegistryModelVersionsClientGetBlobReferenceSASOptions) (RegistryModelVersionsClientGetBlobReferenceSASResponse, error) {
	var err error
	req, err := client.getBlobReferenceSASCreateRequest(ctx, resourceGroupName, registryName, modelName, version, body, options)
	if err != nil {
		return RegistryModelVersionsClientGetBlobReferenceSASResponse{}, err
	}
	httpResp, err := client.internal.Pipeline().Do(req)
	if err != nil {
		return RegistryModelVersionsClientGetBlobReferenceSASResponse{}, err
...
func (client *RegistryModelVersionsClient) getBlobReferenceSASCreateRequest(ctx context.Context, resourceGroupName string, registryName string, modelName string, version string, body BlobReferenceSASRequestDto, options *RegistryModelVersionsClientGetBlobReferenceSASOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.MachineLearningServices/registries/{registryName}/models/{modelName}/versions/{version}/getBlobReferenceSAS"
	if client.subscriptionID == "" {
		return nil, errors.New("parameter client.subscriptionID cannot be empty")
	}
	urlPath = strings.ReplaceAll(urlPath, "{subscriptionId}", url.PathEscape(client.subscriptionID))
...
func (client *RegistryModelVersionsClient) getBlobReferenceSASHandleResponse(resp *http.Response) (RegistryModelVersionsClientGetBlobReferenceSASResponse, error) {
	result := RegistryModelVersionsClientGetBlobReferenceSASResponse{}
	if err := runtime.UnmarshalAsJSON(resp, &result.BlobReferenceSASResponseDto); err != nil {
		return RegistryModelVersionsClientGetBlobReferenceSASResponse{}, err
	}
	return result, nil
}
