...
func (s StorageMonitorConfiguration) MarshalJSON() ([]byte, error) {
	objectMap := make(map[string]any)
	populate(objectMap, "eventSubscriptionName", s.EventSubscriptionName)
	populate(objectMap, "namespaceResourceId", s.NamespaceResourceID)
...
func (s *StorageMonitorConfiguration) UnmarshalJSON(data []byte) error {
	...
	for key, val := range rawMsg {
		var err error
		switch key {
		case "eventSubscriptionName":
			err = unpopulate(val, "EventSubscriptionName", &s.EventSubscriptionName)
			delete(rawMsg, key)
		case "namespaceResourceId":
			err = unpopulate(val, "NamespaceResourceID", &s.NamespaceResourceID)
			delete(rawMsg, key)
		case "topicName":
