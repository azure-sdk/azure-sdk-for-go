...
func (client *Client) ValidateMetric(ctx context.Context, experimentMetricID string, body ExperimentMetric, options *ValidateMetricOptions) (ValidateMetricResponse, error) {
	var err error
	const operationName = "Client.ValidateMetric"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.validateMetricCreateRequest(ctx, experimentMetricID, body, options)
	if err != nil {
...
func (client *Client) validateMetricCreateRequest(ctx context.Context, experimentMetricID string, body ExperimentMetric, options *ValidateMetricOptions) (*policy.Request, error) {
	host := "{endpoint}"
	host = strings.ReplaceAll(host, "{endpoint}", client.endpoint)
