...
func (s *SharedKeysServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	...
	go func() {
		...
		var res result
		if sharedKeysServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = sharedKeysServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
			case "SharedKeysClient.GetSharedKeys":
				res.resp, res.err = s.dispatchGetSharedKeys(req)
			case "SharedKeysClient.Regenerate":
				res.resp, res.err = s.dispatchRegenerate(req)
			default:
...
// set this to conditionally intercept incoming requests to SharedKeysServerTransport
var sharedKeysServerTransportInterceptor interface {
