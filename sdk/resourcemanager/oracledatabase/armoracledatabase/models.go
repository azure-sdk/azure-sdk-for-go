...
// AutonomousDatabaseBackupUpdate - AutonomousDatabaseBackup resource definition
...
type CloudExadataInfrastructureProperties struct {
	...
	// Oracle uses these email addresses to send notifications about planned and
	// unplanned software maintenance updates, information about system hardware, and other information needed by administrators.
	// Up to 10 email addresses can be added to the customer contacts for a cloud
...
type CloudVMClusterProperties struct {
	...
	// (-). Underscores (_) are not permitted. The cluster name can be no longer than
	// 11 characters and is not case sensitive.
	ClusterName *string
	...
	// (database redo logs, archive logs, and recovery manager backups). Accepted
	// values are 35, 40, 60 and 80. The default is 80 percent assigned to DATA storage. See Storage Configuration [/Content/Database/Concepts/exaoverview.htm#Exadata]
	// in the Exadata documentation for
	...
	// typically used for load balancing and are not assigned to any interface. Oracle
	// Clusterware directs the requests to the appropriate nodes in the cluster. Note: For a single-node DB system, this list
	// is empty.
	ScanIPIDs []*string
	...
	// and maintains one VIP address for each node in the Exadata Cloud Service instance to
	// enable failover. If one node fails, the VIP is reassigned to another active node in the cluster. Note: For a single-node
...
type DbActionResponse struct {
	// READ-ONLY; ExascaleDbNode provisioning state
	ProvisioningState *AzureResourceProvisioningState
}
...
type ExadbVMClusterProperties struct {
	...
	// [/Content/Database/References/timezones.htm].
	TimeZone *string
	...
	// SCAN IP addresses are typically used for load balancing and are not assigned to any
	// interface. Oracle Clusterware directs the requests to the appropriate nodes in the cluster. Note: For a single-node DB
	...
	// Ready Services (CRS) creates and maintains one VIP address for each node in the Exadata
	// Cloud Service instance to enable failover. If one node fails, the VIP is reassigned to another active node in the cluster.
...
type MaintenanceWindow struct {
	...
	// - 0 - represents time slot 0:00 - 3:59 UTC - 4 - represents time slot 4:00 - 7:59
	// UTC - 8 - represents time slot 8:00 - 11:59 UTC - 12 - represents time slot 12:00 - 15:59 UTC - 16 - represents time slot
	...
	// not days of the week. For example, to allow maintenance during the 2nd week of the month (from the 8th day to the 14th
	// day of the month), use the value 2. Maintenance cannot be scheduled for the fifth
	// week of months that contain more than 28 days. Note that this parameter works in conjunction with the daysOfWeek and hoursOfDay
	// parameters to allow you to specify specific days of the week and hours
