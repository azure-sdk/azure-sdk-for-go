...
// ProxyList - Get the proxy properties of a Container App Job.
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2023-08-01-preview
//   - resourceGroupName - The name of the resource group. The name is case insensitive.
//   - jobName - Job Name
//   - options - JobsClientProxyListOptions contains the optional parameters for the JobsClient.ProxyList method.
func (client *JobsClient) ProxyList(ctx context.Context, resourceGroupName string, jobName string, options *JobsClientProxyListOptions) (JobsClientProxyListResponse, error) {
	var err error
	req, err := client.proxyListCreateRequest(ctx, resourceGroupName, jobName, options)
	if err != nil {
		return JobsClientProxyListResponse{}, err
	}
	httpResp, err := client.internal.Pipeline().Do(req)
	if err != nil {
		return JobsClientProxyListResponse{}, err
	...
	resp, err := client.proxyListHandleResponse(httpResp)
	return resp, err
}

// proxyListCreateRequest creates the ProxyList request.
func (client *JobsClient) proxyListCreateRequest(ctx context.Context, resourceGroupName string, jobName string, options *JobsClientProxyListOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.App/jobs/{jobName}/detectorProperties/"
	if client.subscriptionID == "" {
		return nil, errors.New("parameter client.subscriptionID cannot be empty")
	}
	urlPath = strings.ReplaceAll(urlPath, "{subscriptionId}", url.PathEscape(client.subscriptionID))
...
// proxyListHandleResponse handles the ProxyList response.
func (client *JobsClient) proxyListHandleResponse(resp *http.Response) (JobsClientProxyListResponse, error) {
	result := JobsClientProxyListResponse{}
	if err := runtime.UnmarshalAsJSON(resp, &result.JobsCollection); err != nil {
		return JobsClientProxyListResponse{}, err
	}
	return result, nil
