...
type NodeTypesServer struct {
	...
	// HTTP status codes to indicate success: http.StatusOK, http.StatusAccepted
	BeginStartFaultSimulation func(ctx context.Context, resourceGroupName string, clusterName string, nodeTypeName string, parameters armservicefabricmanagedclusters.FaultSimulationContentClassification, options *armservicefabricmanagedclusters.NodeTypesClientBeginStartFaultSimulationOptions) (resp azfake.PollerResponder[armservicefabricmanagedclusters.NodeTypesClientStartFaultSimulationResponse], errResp azfake.ErrorResponder)
...
func (n *NodeTypesServerTransport) dispatchBeginStartFaultSimulation(req *http.Request) (*http.Response, error) {
	...
	if beginStartFaultSimulation == nil {
		...
		body, err := unmarshalFaultSimulationContentClassification(raw)
		if err != nil {
			return nil, err
		}
		resourceGroupNameParam, err := url.PathUnescape(matches[regex.SubexpIndex("resourceGroupName")])
