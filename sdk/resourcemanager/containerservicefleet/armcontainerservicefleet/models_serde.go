...
// MarshalJSON implements the json.Marshaller interface for type GenerateUpdateRunRequest.
func (g GenerateUpdateRunRequest) MarshalJSON() ([]byte, error) {
	objectMap := make(map[string]any)
	populate(objectMap, "updateSpecification", g.UpdateSpecification)
	return json.Marshal(objectMap)
}

// UnmarshalJSON implements the json.Unmarshaller interface for type GenerateUpdateRunRequest.
func (g *GenerateUpdateRunRequest) UnmarshalJSON(data []byte) error {
	var rawMsg map[string]json.RawMessage
	if err := json.Unmarshal(data, &rawMsg); err != nil {
		return fmt.Errorf("unmarshalling type %T: %v", g, err)
	}
	for key, val := range rawMsg {
		...
		case "updateSpecification":
			err = unpopulate(val, "UpdateSpecification", &g.UpdateSpecification)
			delete(rawMsg, key)
		}
		if err != nil {
			return fmt.Errorf("unmarshalling type %T: %v", g, err)
