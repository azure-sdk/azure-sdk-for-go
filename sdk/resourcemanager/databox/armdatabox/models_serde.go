...
// MarshalJSON implements the json.Marshaller interface for type CustomerInterventionStageProgress.
func (c CustomerInterventionStageProgress) MarshalJSON() ([]byte, error) {
	objectMap := make(map[string]interface{})
	populate(objectMap, "actions", c.Actions)
	populate(objectMap, "error", c.Error)
	return json.Marshal(objectMap)
...
// UnmarshalJSON implements the json.Unmarshaller interface for type CustomerInterventionStageProgress.
func (c *CustomerInterventionStageProgress) UnmarshalJSON(data []byte) error {
	var rawMsg map[string]json.RawMessage
	if err := json.Unmarshal(data, &rawMsg); err != nil {
		return fmt.Errorf("unmarshalling type %T: %v", c, err)
	}
